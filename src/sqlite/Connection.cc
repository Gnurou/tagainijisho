...
bool Connection::connect(const QString &dbFile, OpenFlags flags) {
    ...
    int res = sqlite3_open_v2(dbFile.toUtf8().data(), &_handler,
    ...
    // Register our tokenizers (extensions can be handled by SQLite's auto
    // mechanism, not tokenizers as they need the connection to be opened
    sqlite3ext_register_tokenizers(_handler);
    // Configure the connection
    exec("pragma encoding=\"UTF-16le\"");
    if (!(flags & JournalInFile))
...
#endif
    if (!connected()) {
        // Will put the appropriate error (library routine called out of
        // sequence) because the handler is null
        updateError();
        return false;
    }
...
const Error &Connection::lastError() const { return _lastError; }
...
#ifdef DEBUG_TRANSACTIONS
    if (res)
        qDebug("Transaction #%d started on connection %p", _tr_count++, this);
    else
        qDebug("Failed to start transaction #%d on connection %p", _tr_count, this);
#endif
    return res;
}
...
#ifdef DEBUG_TRANSACTIONS
    if (res)
        qDebug("Transaction #%d commited on connection %p", --_tr_count, this);
    else
        qDebug("Failed to commit transaction #%d on connection %p", _tr_count - 1, this);
#endif
    return res;
}
...
#ifdef DEBUG_TRANSACTIONS
    if (res)
        qDebug("Rollbacking transaction #%d on connection %p (commit will follow)", _tr_count - 1,
               this);
    else
        qDebug("Failed to rollback transaction #%d on connection %p", _tr_count - 1, this);
#endif
    if (!res)
        return false;
    res = commit();
...
void Connection::interrupt() { sqlite3_interrupt(_handler); }
