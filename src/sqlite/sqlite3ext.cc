...
/**
 * Returns a pseudo-random value which is biased by the parameter given (which
 * must be between 0 and 100). The bigger the parameter, the biggest chances the
 * generated has to be low.
 */
...
static void uniquecount_aggr_step(sqlite3_context *context, int nParams, sqlite3_value **values) {
    uniquecount_aggr *aggr_struct = static_cast<uniquecount_aggr *>(
        sqlite3_aggregate_context(context, sizeof(uniquecount_aggr)));
    if (!aggr_struct->_set)
        aggr_struct->_set = new QSet<int>();
    for (int i = 0; i < nParams; i++) {
        if (sqlite3_value_type(values[i]) == SQLITE_NULL)
            continue;
...
static void fts_uncompress(sqlite3_context *context, int argc, sqlite3_value **argv) {
    QByteArray data(static_cast<const char *>(sqlite3_value_blob(argv[0])),
...
static int ignoreDelim(ignore_tokenizer *t, unsigned char c) { return c < 0x80 && t->delim[c]; }

/*
** Create a new tokenizer instance.
*/
static int ignoreCreate(int argc, const char *const *argv, sqlite3_tokenizer **ppTokenizer) {
    ignore_tokenizer *t;
...
** *ppCursor.
*/
static int ignoreOpen(sqlite3_tokenizer *pTokenizer,      /* The tokenizer */
...
*/
static int ignoreNext(sqlite3_tokenizer_cursor *pCursor, /* Cursor returned by simpleOpen */
...
static const sqlite3_tokenizer_module ignoreTokenizerModule = {
    0, ignoreCreate, ignoreDestroy, ignoreOpen, ignoreClose, ignoreNext,
};
...
static int katakanaDelim(katakana_tokenizer *t, unsigned char c) { return c < 0x80 && t->delim[c]; }

/*
** Create a new tokenizer instance.
*/
static int katakanaCreate(int argc, const char *const *argv, sqlite3_tokenizer **ppTokenizer) {
...
** *ppCursor.
*/
static int katakanaOpen(sqlite3_tokenizer *pTokenizer,      /* The tokenizer */
                        const char *pInput, int nBytes,     /* String to be tokenized */
                        sqlite3_tokenizer_cursor **ppCursor /* OUT: Tokenization cursor */
) {
...
** have been opened by a prior call to simpleOpen().
*/
static int katakanaNext(sqlite3_tokenizer_cursor *pCursor, /* Cursor returned by simpleOpen */
                        const char **ppToken,              /* OUT: *ppToken is the token text */
                        int *pnBytes,                      /* OUT: Number of bytes in token */
                        int *piStartOffset,                /* OUT: Starting offset of token */
                        int *piEndOffset,                  /* OUT: Ending offset of token */
...
static const sqlite3_tokenizer_module katakanaTokenizerModule = {
    0, katakanaCreate, katakanaDestroy, katakanaOpen, katakanaClose, katakanaNext,
};
...
void sqlite3ext_init() { sqlite3_auto_extension((void (*)())load_extensions); }
...
int sqlite3ext_register_functions(sqlite3 *handler) {
    ...
    sqlite3_create_function(handler, "uniquecount", -1, SQLITE_UTF8, 0, 0, uniquecount_aggr_step,
                            uniquecount_aggr_finalize);
