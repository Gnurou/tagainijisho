...
public:
    ...
    _QSqlCachedResult::ValueCache cache;
    int rowCacheEnd;
    int colCount;
    bool forwardOnly;
    bool atEnd;
...
_QSqlCachedResult::_QSqlCachedResult(const QSqlDriver * db): QSqlResult (db)
{
    d = new QSqlCachedResultPrivate();
}

_QSqlCachedResult::~_QSqlCachedResult()
{
    delete d;
}

void _QSqlCachedResult::init(int colCount)
{
    d->init(colCount, isForwardOnly());
}

bool _QSqlCachedResult::fetch(int i)
{
    if ((!isActive()) || (i < 0))
        return false;
    if (at() == i)
...
bool _QSqlCachedResult::fetchNext()
{
    if (d->canSeek(at() + 1)) {
        setAt(at() + 1);
        return true;
...
bool _QSqlCachedResult::fetchPrevious()
{
    return fetch(at() - 1);
}

bool _QSqlCachedResult::fetchFirst()
{
...
bool _QSqlCachedResult::fetchLast()
{
    if (d->atEnd) {
        if (d->forwardOnly)
            return false;
...
QVariant _QSqlCachedResult::data(int i)
{
...
bool _QSqlCachedResult::isNull(int i)
{
    int idx = d->forwardOnly ? i : at() * d->colCount + i;
    if (i > d->colCount || i < 0 || at() < 0 || idx >= d->rowCacheEnd)
        return true;
...
void _QSqlCachedResult::cleanup()
{
...
void _QSqlCachedResult::clearValues()
{
    setAt(QSql::BeforeFirstRow);
    d->rowCacheEnd = 0;
    d->atEnd = false;
...
bool _QSqlCachedResult::cacheNext()
{
    if (d->atEnd)
        return false;
...
int _QSqlCachedResult::colCount() const
{
    return d->colCount;
}

_QSqlCachedResult::ValueCache &_QSqlCachedResult::cache()
{
    return d->cache;
}
