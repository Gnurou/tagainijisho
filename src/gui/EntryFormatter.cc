...
PreferenceItem<bool> EntryFormatter::shortDescShowJLPT("mainWindow/detailedView",
                                                       "shortEntryShowJLPT", false);
...
EntryFormatter::EntryFormatter(const QString &_cssFile, const QString &_htmlFile, QObject *parent)
    : QObject(parent) {
    // Try to load custom html file
    QString htmlFile;
    if (!_htmlFile.isEmpty())
        htmlFile = lookForFile(_htmlFile);
    if (htmlFile.isEmpty())
        htmlFile = lookForFile("detailed_default.html");
    if (htmlFile.isEmpty())
    ...
    _css += QString(".subinfo {\n\tbackground-color: %1;\n}\n")
                .arg(QPalette().color(QPalette::Dark).name());
    _css += QString(".subinfo .title {\n\tbackground-color: %1;\n}\n")
                .arg(QPalette().color(QPalette::Midlight).name());
    _css += QString(".subinfo .contents {\n\tbackground-color: %1;\n}\n")
                .arg(QPalette().color(QPalette::Midlight).name());
    _css += QString(".notes {\n\tbackground-color: %1; \n\tcolor: %2;\n}\n")
                .arg(QPalette().color(QPalette::ToolTipBase).name())
                .arg(QPalette().color(QPalette::ToolTipText).name());
...
bool EntryFormatter::registerFormatter(const int entryType, EntryFormatter *formatter) {
    if (_formatters.contains(entryType))
        return false;
...
void EntryFormatter::drawInfo(const ConstEntryPointer &entry, QPainter &painter, QRectF &rectangle,
...
void EntryFormatter::draw(const ConstEntryPointer &entry, QPainter &painter,
                          const QRectF &rectangle, QRectF &usedSpace, const QFont &textFont) const {
    painter.save();
...
QString EntryFormatter::colorTriplet(const QColor &color) {
    return QString("#%1%2%3")
        .arg(QString::number(color.red(), 16), 2, '0')
        .arg(QString::number(color.green(), 16), 2, '0')
        .arg(QString::number(color.blue(), 16), 2, '0');
}
...
QString EntryFormatter::autoFormat(const QString &str) const {
    ...
    while (pos < str.size()) {
        ...
        if (TextTools::isKanjiChar(str[pos]))
            nextChar = Kanji;
        else if (TextTools::isKanaChar(str[pos]))
            nextChar = Kana;
        else
            nextChar = Romaji;
...
QString EntryFormatter::entryTitle(const ConstEntryPointer &entry) const {
    ...
    if (entry->trained()) {
        title = QString("<span style=\"background-color:%1\">%2</span>")
                    .arg(colorTriplet(scoreColor(*entry)))
                    .arg(title);
    }
    return title;
}
...
QString EntryFormatter::buildSubInfoLine(const QString &title, const QString &content) {
    return QString("<table class=\"subinfo\"><tr><td class=\"title\" "
                   "valign=\"center\">%1:</td><td class=\"contents\"> "
                   "%2</td></tr></table>")
        .arg(title)
        .arg(content);
}
...
QString EntryFormatter::buildSubInfoBlock(const QString &title, const QString &content) {
    return QString("<table width=\"100%\"class=\"subinfo\"><tr><td "
                   "class=\"title\">%1</td></tr><tr><td "
                   "class=\"contents\">%2</td></tr></table>")
        .arg(title)
        .arg(content);
}
...
QString EntryFormatter::formatLists(const ConstEntryPointer &entry) const {
    if (!entry->lists().isEmpty()) {
        ...
        foreach (quint64 rowid, entry->lists()) {
            ...
            url.setQuery(query);
            ret << QString("<img src=\"listicon\"/>   <a href=\"%1\">%2</a><br />")
                       .arg(QString(url.toEncoded()))
                       .arg(autoFormat(label));
        }
        return ret.join(" ");
    } else
        return "";
...
QString EntryFormatter::formatTags(const ConstEntryPointer &entry) const {
    if (!entry->tags().isEmpty()) {
        ...
        foreach (const Tag &tag, entry->tags()) {
            ...
            url.setQuery(query);
            ret += QString("<a href=\"%1\">%2</a>")
                       .arg(QString(url.toEncoded()))
                       .arg(autoFormat(tag.name()));
        }
        return ret;
    } else
        return "";
...
QString EntryFormatter::formatTrainingData(const ConstEntryPointer &entry) const {
    ...
    if (entry->trained()) {
        QStringList ret;
        ret << tr("Studied since %1.")
                   .arg(entry->dateAdded().date().toString(Qt::DefaultLocaleShortDate));
        ret << tr("Score: %1.").arg(entry->score());
        if (entry->dateLastTrain().isValid()) {
            ret << tr("Last trained on %1.")
                       .arg(entry->dateLastTrain().date().toString(Qt::DefaultLocaleShortDate));
            if (entry->dateLastMistake().isValid()) {
                ret << tr("Last mistaken on %1.")
                           .arg(entry->dateLastMistake().date().toString(
