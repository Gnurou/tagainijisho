...
SavedSearchTreeItem::SavedSearchTreeItem(int setId, int position, bool isFolder,
                                         const QString &label)
    : QTreeWidgetItem(isFolder ? FolderType : SavedSearchType), _setId(setId), _position(position),
      _parentCopy(0) {
...
SavedSearchTreeItem::SavedSearchTreeItem(const SavedSearchTreeItem &other)
...
void SavedSearchesTreeWidget::dropEvent(QDropEvent *event) { QAbstractItemView::dropEvent(event); }

QMimeData *SavedSearchesTreeWidget::mimeData(const QList<QTreeWidgetItem *> items) const {
    QByteArray ba;
    QDataStream ds(&ba, QIODevice::WriteOnly);
    foreach (QTreeWidgetItem *item, items) {
        SavedSearchTreeItem *clone =
            new SavedSearchTreeItem(*static_cast<SavedSearchTreeItem *>(item));
...
bool SavedSearchesTreeWidget::dropMimeData(QTreeWidgetItem *parent, int index,
    ...
    while (!ds.atEnd()) {
        ...
        quint64 tmp;
        ds >> tmp;
        movedItem = (SavedSearchTreeItem *)tmp;
        ...
        if (index > 0 && (prevParent == newParent && index > prevPosition))
            newPosition--;
...
#define QUERY(q)                                                                                   \
    {                                                                                              \
        if (!query.exec(q))                                                                        \
            qDebug() << "Query failed" << query.lastError().message();                             \
    }
            // Update the positions of the items after the one we removed
            QUERY(QString("UPDATE sets SET position = position - 1 where parent %1 "
                          "and position > %2")
                      .arg(!prevParentId ? "is null" : QString("= %1").arg(prevParentId))
                      .arg(prevPosition));
            ...
            foreach (SavedSearchTreeItem *sibling, siblings) {
                ...
                if (sibPos > prevPosition)
                    sibling->setPosition(sibPos - 1);
            }
            // Update the positions of the items after the one we add
            QUERY(QString("UPDATE sets SET position = position + 1 where "
                          "parent %1 and "
                          "position >= %2")
                      .arg(!newParentId ? "is null" : QString("= %1").arg(newParentId))
                      .arg(newPosition));
            siblings = childsOf(newParent);
            foreach (SavedSearchTreeItem *sibling, siblings) {
                // Do not update the item
                if (sibling == movedItem)
                    continue;
                int sibPos = sibling->position();
                if (sibPos >= newPosition)
                    sibling->setPosition(sibPos + 1);
            }
            // Move our item
            QUERY(QString("UPDATE sets SET position = %1, parent = %2 where "
                          "rowid = %3")
                      .arg(newPosition)
                      .arg(!newParentId ? "null" : QString("%1").arg(newParentId))
                      .arg(movedItem->setId()));
        }
        // Now put our item to its new position
        if (!newParent)
            insertTopLevelItem(index, movedItem);
        else
            newParent->insertChild(index, movedItem);
#undef QUERY
    }
    return true;
}
...
void SavedSearchesTreeWidget::deleteSavedSearch(SavedSearchTreeItem *item) {
    ...
    query.prepare(QString("UPDATE sets SET position = position - 1 where parent %1 and "
                          "position > %2")
                      .arg(!parent ? "is null" : QString("= %1").arg(parent->setId()))
                      .arg(item->position()));
    if (!query.exec()) {
        qDebug() << "Error executing query:" << query.lastError().message();
        return;
    ...
    foreach (SavedSearchTreeItem *sibling, siblings)
        if (sibling->position() > item->position())
            sibling->setPosition(sibling->position() - 1);
    delete item;
}

void SavedSearchesTreeWidget::deleteSelection() {
    if (QMessageBox::question(this, tr("Confirm deletion"),
                              tr("Are you sure you want to delete the selected search/folder?"),
...
QList<SavedSearchTreeItem *> SavedSearchesTreeWidget::childsOf(SavedSearchTreeItem *parent) {
    ...
    if (!parent)
        for (int i = 0; i < topLevelItemCount(); i++)
            siblings << static_cast<SavedSearchTreeItem *>(topLevelItem(i));
    else
        for (int i = 0; i < parent->childCount(); i++)
            siblings << static_cast<SavedSearchTreeItem *>(parent->child(i));
    return siblings;
}
...
void SavedSearchesTreeWidget::populateRoot() {
    ...
    query.exec("SELECT rowid, position, state IS NULL, label FROM sets WHERE "
               "parent IS NULL ORDER BY position");
    while (query.next()) {
        SavedSearchTreeItem *item = new SavedSearchTreeItem(
            query.valueInt(0), query.valueInt(1), query.valueBool(2), query.valueString(3));
        if (item->isFolder()) {
...
void SavedSearchesTreeWidget::populateFolder(SavedSearchTreeItem *parent) const {
    ...
    query.prepare("SELECT rowid, position, state IS NULL, label FROM sets WHERE "
