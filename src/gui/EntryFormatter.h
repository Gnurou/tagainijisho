...
class EntryFormatter : public QObject {
  ...
  protected:
     ...
     */
    void drawInfo(const ConstEntryPointer &entry, QPainter &painter, QRectF &rectangle,
                  const QFont &textFont = QFont()) const;

    EntryFormatter(const QString &_cssFile = "", const QString &_htmlFile = "",
                   QObject *parent = 0);
  ...
  public:
     ...
     *
     * @return true if the formatter has been registered successfully, false if
     * a formatter was already registered for this type of entry.
     */
    static bool registerFormatter(const int entryType, EntryFormatter *formatter);
    /**
     * Remove a previously registered formatter.
     *
     * @return true if the formatter has been removed successfully, false if
     * there were no formatter registered for this type of entry.
     */
    static bool removeFormatter(const int entryType);
    static const EntryFormatter *getFormatter(const int entryType) {
        return _formatters[entryType];
    }
    static const EntryFormatter *getFormatter(const ConstEntryPointer &entry) {
        return _formatters[entry->type()];
    }
};
