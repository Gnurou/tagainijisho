...
EntriesViewHelper::EntriesViewHelper(QAbstractItemView *client, EntryDelegateLayout *delegateLayout,
                                     bool workOnSelection, bool viewOnly)
    : EntryMenu(client), _client(client), _entriesMenu(), _workOnSelection(workOnSelection),
      _actionPrint(QIcon(":/images/icons/print.png"), tr("&Print..."), 0),
      _actionPrintPreview(QIcon(":/images/icons/print.png"), tr("Print p&review..."), 0),
      _actionPrintBooklet(QIcon(":/images/icons/print.png"), tr("Print &booklet..."), 0),
      _actionPrintBookletPreview(QIcon(":/images/icons/print.png"), tr("Booklet pre&view..."), 0),
      _actionExportTab(QIcon(":/images/icons/document-export.png"), tr("Export as &TSV..."), 0),
      _actionExportJs(QIcon(":/images/icons/document-export.png"), tr("Export as &HTML..."), 0),
      prefRefs(MAX_PREF), _contextMenu() {
    ...
    if (!delegateLayout)
        delegateLayout = new EntryDelegateLayout(this);
    ...
    connect(this, SIGNAL(tagsHistorySelected(const QStringList &)), this,
            SLOT(addTags(const QStringList &)));
...
void EntriesViewHelper::setPreferenceHandler(Preference pref, PreferenceRoot *ref) {
    if (prefRefs[pref])
        disconnect(prefRefs[pref], SIGNAL(valueChanged(QVariant)), _delegateLayout,
                   SLOT(updateConfig(QVariant)));
    prefRefs[pref] = ref;
    if (ref) {
        _delegateLayout->setProperty(ref->name().toLatin1().constData(), ref->variantValue());
        connect(prefRefs[pref], SIGNAL(valueChanged(QVariant)), _delegateLayout,
                SLOT(updateConfig(QVariant)));
    }
}
...
void addToClipboard(const QString &text) {
    ...
    QApplication::clipboard()->setText(clipboardText.isEmpty() ? text
                                                               : clipboardText + "\n" + text);
}

struct StudyHandler : BatchHandler {
    void apply(const EntryPointer &e) const { e->addToTraining(); }
};
...
void EntriesViewHelper::studySelected() { applyOnSelection(StudyHandler()); }

struct UnstudyHandler : BatchHandler {
    void apply(const EntryPointer &e) const { e->removeFromTraining(); }
};

void EntriesViewHelper::unstudySelected() { applyOnSelection(UnstudyHandler()); }

struct MarkAsKnownHandler : BatchHandler {
    void apply(const EntryPointer &e) const { e->setAlreadyKnown(); }
};

void EntriesViewHelper::markAsKnown() { applyOnSelection(MarkAsKnownHandler()); }

struct ResetTrainingHandler : BatchHandler {
    void apply(const EntryPointer &e) const { e->resetScore(); }
};

void EntriesViewHelper::resetTraining() { applyOnSelection(ResetTrainingHandler()); }
    ...
    foreach (const QModelIndex &idx, indexes) {
        ...
        if (!entry) {
            ...
            for (int i = 0; i < childsCount; i++)
                childs << idx.model()->index(i, 0, idx);
...
QModelIndexList EntriesViewHelper::getEntriesToProcess(bool limitToSelection) {
    ...
    if (_workOnSelection || limitToSelection)
        selIndexes = client()->selectionModel()->selectedIndexes();
    else
        for (int i = 0; i < client()->model()->rowCount(QModelIndex()); i++) {
...
void EntriesViewHelper::print() {
    QPrinter printer;
    if (!askForPrintOptions(printer, tr("Print")))
        return;
    EntriesPrinter(client()).print(getEntriesToProcess(printer.printRange() & QPrinter::Selection),
                                   &printer);
}

void EntriesViewHelper::printPreview() {
    QPrinter printer;
    if (!workOnSelection() && !askForPrintOptions(printer, tr("Print preview")))
        return;
    EntriesPrinter(client()).printPreview(
        getEntriesToProcess(printer.printRange() & QPrinter::Selection), &printer);
}

void EntriesViewHelper::printBooklet() {
    QPrinter printer;
    if (!askForPrintOptions(printer, tr("Booklet print")))
        return;
    EntriesPrinter(client()).printBooklet(
        getEntriesToProcess(printer.printRange() & QPrinter::Selection), &printer);
}

void EntriesViewHelper::printBookletPreview() {
    QPrinter printer;
    if (!workOnSelection() && !askForPrintOptions(printer, tr("Booklet print preview")))
        return;
    EntriesPrinter(client()).printBookletPreview(
        getEntriesToProcess(printer.printRange() & QPrinter::Selection), &printer);
}

void EntriesViewHelper::tabExport() {
    QString exportFile =
        QFileDialog::getSaveFileName(0, tr("Export to tab-separated file..."), "export.tsv");
    if (exportFile.isEmpty())
        return;
    QFile outFile(exportFile);
    if (!outFile.open(QIODevice::WriteOnly)) {
        QMessageBox::warning(0, tr("Cannot write file"),
                             QString(tr("Unable to write file %1.")).arg(exportFile));
        return;
    }
    ...
    foreach (const QModelIndex &idx, entries) {
        ...
        if (outFile.write(QString("%1\t%2\t%3\n")
                              ...
                              .arg(meanings.join(", "))
                              .toUtf8()) == -1) {
            QMessageBox::warning(0, tr("Error writing file"),
                                 QString(tr("Error while writing file %1.")).arg(exportFile));
            return;
        }
    }
...
void EntriesViewHelper::jsExport() {
    QString exportFile =
        QFileDialog::getSaveFileName(0, tr("Export to HTML flashcard file..."), "flashcard.html");
    if (exportFile.isEmpty())
        return;
    QFile outFile(exportFile);
    if (!outFile.open(QIODevice::WriteOnly)) {
    ...
    if (!tmplFile.open(QIODevice::ReadOnly)) {
        QMessageBox::warning(0, tr("Cannot open template file"),
                             QString(tr("Unable to open template file!")).arg(exportFile));
        return;
    }
    ...
    foreach (const ConstEntryPointer &entry, realEntries) {
        ...
        QString meaning;
        if (readings.size() > 0)
            reading = escapeQuotes(readings.join(", "));
        if (meanings.size() == 1)
            meaning = escapeQuotes(meanings[0]);
        else {
            int cpt = 1;
            foreach (const QString &str, meanings)
    ...
    if (obj == client()) {
        ...
        case QEvent::ContextMenu: {
            ...
            if (numSelectedEntries < 250) {
                ...
                foreach (const EntryPointer &entry, _selectedEntries)
                    cSelectedEntries << entry;
        ...
        case QEvent::MouseButtonPress: {
            ...
            if (selectedIndexes.size() != 1 || !selectedIndexes.contains(clickedIndex))
                return false;
            // If we arrive here we know we have clicked on the uniquely
            // selected item. Left button and no modifier? In any case clear the
            // current selection, so that the selection signal is emitted
            // anyway.
...
QToolBar *EntriesViewHelper::defaultToolBar(QWidget *parent) {
    ...
    toolBar->setStyleSheet("QToolBar { background: none; border-style: none; "
