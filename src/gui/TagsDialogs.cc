...
//	tagsCompleter->setModelSorting(QCompleter::CaseInsensitivelySortedModel);
	...
	connect(this, SIGNAL(cursorPositionChanged(int, int)),
		this, SLOT(checkCompletion()));
	connect(this, SIGNAL(textChanged(const QString &)),
		this, SLOT(onTextChanged(const QString &)));
	connect(tagsCompleter, SIGNAL(activated(const QString &)),
		this, SLOT(completeText(const QString &)));
}
...
void TagsLineInput::onTextChanged(const QString &newText)
{
	...
	foreach (const QString &tag, tags) {
		...
		else if (!Tag::knownTagsModel()->contains(tag)) return;
	}
	emit allValidTags();
}
...
void TagsLineInput::checkCompletion()
{
	...
	// Do not display anything if there is no text or if we are in the middle of a word
	if (curText.isEmpty() ||
		(curText.size() > cursorPosition() &&
		 !(curText[cursorPosition()].category() & (QChar::Separator_Space | QChar::Mark_NonSpacing)))) {
	...
	// We have a potential completion to perform, let's isolate the interesting portion
...
void TagsLineInput::completeText(const QString &completion)
{
	...
	curText.insert(cursorPosition(), completion.right(completion.size() - tagsCompleter->completionPrefix().size()) + " ");
	setText(curText);
}

TagsInputDialog::TagsInputDialog(const QString &title, const QString &message, const QStringList &initialTags, bool selectAll, QWidget *parent) : QDialog(parent)
{
	...
	if (selectAll) lineEdit->selectAll();
...
bool TagsDialogs::setTagsDialog(const QList<Entry *> entries, QWidget *parent)
{
	...
	for (int i = 1; i < entries.size(); i++) {
		...
		foreach(const Tag &tag, tags) if (!split.contains(tag.name())) {
			identical = false;
			continue;
		}
	}
	if (!identical) split.clear();
	...
	do {
		...
		QString tags;
		QString winTitle;
		if (entries.size() == 1) winTitle = tr("Set tags for %1").arg(entries[0]->name());
		else winTitle = tr("Set tags");
		TagsInputDialog dialog(winTitle, tr("Enter a space-separated list of tags for ") + (entries.size() > 1 ? tr("these entries:") : tr("this entry:")), split, true, parent);
		if (dialog.exec() == QDialog::Accepted) tags = dialog.getText();
		else return false;
...
bool TagsDialogs::addTagsDialog(const QList<Entry *> entries, QWidget *parent)
{
	...
	do {
		...
		QString tags;
		QString winTitle;
		if (entries.size() == 1) winTitle = tr("Add tags to %1").arg(entries[0]->name());
		else winTitle = tr("Add tags");
		TagsInputDialog dialog(winTitle, tr("Enter a space-separated list of tags to add to ") + (entries.size() > 1 ? tr("these entries:") : tr("this entry:")), QStringList(), false, parent);
		if (dialog.exec() == QDialog::Accepted) tags = dialog.getText();
		else return false;
