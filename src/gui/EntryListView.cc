...
PreferenceItem<bool> EntryListView::smoothScrollingSetting("mainWindow/lists", "smoothScrolling",
                                                           true);
...
PreferenceItem<QString> EntryListView::kanjiFontSetting("mainWindow/lists", "kanjiFont",
                                                        QFont("Helvetica", 15).toString());
PreferenceItem<int> EntryListView::displayModeSetting("mainWindow/lists", "displayMode",
                                                      EntryDelegateLayout::OneLine);

EntryListView::EntryListView(QWidget *parent, EntryDelegateLayout *delegateLayout, bool viewOnly)
    : QTreeView(parent), _helper(this, delegateLayout, true, viewOnly),
      _newListAction(QIcon(":/images/icons/document-new.png"), tr("New list..."), 0),
      _rightClickNewListAction(_newListAction.icon(), _newListAction.text(), 0),
      _deleteSelectionAction(QIcon(":/images/icons/delete.png"), tr("Delete"), 0),
      _renameListAction(QIcon(), tr("Rename list..."), 0),
      _goUpAction(QIcon(":/images/icons/go-up.png"), tr("Go up"), 0) {
    ...
    // If the view is editable, the helper menu shall be augmented with edit
    // options
    if (!viewOnly) {
    ...
    // If no delegate has been given, consider this view is ruled by the default
    // settings
    if (!delegateLayout) {
    ...
    connect(&smoothScrollingSetting, SIGNAL(valueChanged(QVariant)), &_helper,
...
void EntryListView::selectionChanged(const QItemSelection &selected,
                                     const QItemSelection &deselected) {
    QTreeView::selectionChanged(selected, deselected);

    // Update the status of the right-click new list action
    QModelIndexList selection(selected.indexes());
    _rightClickNewListAction.setEnabled(
        selection.size() <= 1 &&
        (selection.size() == 0 || !selection[0].data(Entry::EntryRole).isValid()));
    _renameListAction.setEnabled(selection.size() == 1 &&
                                 !selection[0].data(Entry::EntryRole).isValid());
...
void EntryListView::newList(QModelIndex parent) {
    ...
    if (!model()->insertRows(idx, 1, parent)) {
        QMessageBox::information(this, tr("Unable to create list"),
                                 tr("A database error occured while trying to add the list."));
        return;
    }
...
void EntryListView::rightClickNewList() {
    ...
    if (selection.isEmpty())
        newList();
    else if (selection.size() == 1 && !selection[0].data(Entry::EntryRole).isValid())
        newList(selection[0]);
}
...
bool EntryListView::deleteEntries(const QModelIndex &index) {
    ...
    // Delete all children
    while (nRows--)
        if (!deleteEntries(index.model()->index(0, 0, index)))
            return false;
...
void EntryListView::deleteSelectedItems() {
    if (QMessageBox::question(this, tr("Confirm deletion"),
                              tr("This will delete the selected lists items and lists, including "
                                 "all their children. Continue?"),
                              QMessageBox::Yes | QMessageBox::No,
                              QMessageBox::No) != QMessageBox::Yes)
        return;
    QModelIndexList selection(selectionModel()->selectedIndexes());

    // Build a persistent list of all the indexes to remove as we are going to
    // mess with their order
    ...
    // TODO progress bar
    if (!EntryListCache::connection()->transaction())
        goto failure_1;
    foreach (const QPersistentModelIndex &index, perList) {
        if (!deleteEntries(index))
            goto failure_2;
    }
    if (!EntryListCache::connection()->commit())
        goto failure_2;
...
failure_1:
    QMessageBox::information(this, tr("Removal failed"),
                             tr("A database error has occured while trying to remove the selected "
                                "items:\n\n%1\n\n Some of them may be remaining.")
                                 .arg(Database::lastError().message()));
}

void EntryListView::goUp() { setRootIndex(rootIndex().parent()); }

void EntryListView::setRootIndex(const QModelIndex &idx) {
    // Entries can not become roots
    if (idx.data(Entry::EntryRefRole).isValid())
        return;
...
void EntryListView::editSelectedList() {
    ...
    if (selection.size() == 1) {
        ...
        if (!idx.data(Entry::EntryRole).isValid())
            edit(idx);
    }
}
