...
void messageHandler(QtMsgType type, const QMessageLogContext &context, const QString &msg) {
    ...
    case QtCriticalMsg:
        ...
        break;
    case QtFatalMsg:
        // TODO here a message should be emitted and caught by the main thread
        // in order to display the message box. This is because fatal errors may
        // occur in auxiliary threads too. QMessageBox::critical(0, "Tagaini
        // Jisho fatal error", msg);
...
void checkConfigurationVersion() {
    // Are we running the program for the first time or updating from a previous
    // version?
    if (configVersion.value() >= CONFIG_VERSION)
        return;

    QSettings settings;
    // If there is no key, then we are running the program for the first time
    ...
    if (!settings.allKeys().isEmpty()) {
        ...
        case 1:
            ...
            if (settings.contains("defaultFont"))
                settings.setValue("mainWindow/defaultFont", settings.value("defaultFont"));
            if (settings.contains("guiLanguage"))
                settings.setValue("mainWindow/guiLanguage", settings.value("guiLanguage"));
            if (settings.contains("autoCheckUpdates"))
                settings.setValue("mainWindow/autoCheckUpdates",
                                  settings.value("autoCheckUpdates"));
            if (settings.contains("autoCheckBetaUpdates"))
                settings.setValue("mainWindow/autoCheckBetaUpdates",
                                  settings.value("autoCheckBetaUpdates"));
            if (settings.contains("updateCheckInterval"))
                settings.setValue("mainWindow/updateCheckInterval",
        ...
        case 4:
            if (settings.contains("mainWindow/guiLanguage"))
                settings.setValue("preferredLanguages", settings.value("mainWindow/guiLanguage"));
        ...
        default:
            // If we arrive here, this means we are running an pre-tracking
            // version - do nothing in that case
            break;
        }
    }
    ...
    if (legacyDataFile.exists()) {
        ...
        QMessageBox::information(
            ...
            QCoreApplication::translate(
                "main.cc", "Welcome to Tagaini Jisho %1! Your user data has successfully "
                           "been "
                           "migrated from your old version of Tagaini and you should find "
                           "it "
                           "as it was.\n\nIf you still have an old version of Tagaini "
                           "installed, please note that changes made in Tagaini %1 will "
                           "not "
                           "be visible on it and vice-versa. It is recommended that you "
                           "uninstall your old Tagaini version after checking that "
                           "everything "
                           "is working here.")
                .arg(VERSION));
    }
...
int main(int argc, char *argv[]) {
    ...
    // Ensure the EntriesCache is instanciated in the main thread - that way we
    // won't have to switch threads every time an Entry is deleted
    EntriesCache::init();
    ...
    foreach (const QString &arg, args) {
        if (arg == "--temp-db")
            temporaryDB = true;
        else if (arg.startsWith("--user-db="))
            userDBFile = arg.mid(10);
    }
    ...
    // Deleting the main window ensures there is no other DB query running in
    // the GUI thread
    delete mainWindow;
    ...
    // Plugins must be deleted after the main window in case the latter still
    // uses them in a background thread
