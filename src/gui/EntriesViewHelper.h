...
class EntriesViewHelper : public EntryMenu {
  ...
  private:
    ...
    QAction _actionPrint, _actionPrintPreview, _actionPrintBooklet, _actionPrintBookletPreview,
        _actionExportTab, _actionExportJs;
  ...
  public:
    /**
     * @arg workOnSelection defines how the print and export actions will
     * performed. If set, printing and export will only be available if a
     * selection is active, and will perform on this selection exclusively. If
     * not set (the default), these actions will take place on the whole model,
     * with an option to restrict them to the current selection.
     *
     * @arg viewOnly sets whether the view this helper acts on is not editable.
     * If so, options that let the displayed entries be modified will be omited.
     */
    EntriesViewHelper(QAbstractItemView *client, EntryDelegateLayout *delegateLayout = 0,
                      bool workOnSelection = false, bool viewOnly = false);
