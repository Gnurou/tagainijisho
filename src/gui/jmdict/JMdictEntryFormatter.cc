...
PreferenceItem<int> JMdictEntryFormatter::maxHomophonesToDisplay("jmdict", "maxHomophonesToDisplay",
                                                                 5);
PreferenceItem<bool>
    JMdictEntryFormatter::displayStudiedHomophonesOnly("jmdict", "displayStudiedHomophonesOnly",
                                                       false);
PreferenceItem<int> JMdictEntryFormatter::maxHomographsToDisplay("jmdict", "maxHomographsToDisplay",
                                                                 5);
PreferenceItem<bool>
    JMdictEntryFormatter::displayStudiedHomographsOnly("jmdict", "displayStudiedHomographsOnly",
                                                       false);
...
PreferenceItem<bool> JMdictEntryFormatter::printOnlyStudiedKanjis("jmdict",
                                                                  "printOnlyStudiedKanjis", false);
PreferenceItem<int> JMdictEntryFormatter::maxDefinitionsToPrint("jmdict", "maxDefinitionsToPrint",
                                                                0);
PreferenceItem<QString>
    JMdictEntryFormatter::exampleSentencesService("jmdict", "exampleSentencesService", "");
...
QString JMdictEntryFormatter::getVerbBuddySql(const QString &matchPattern, quint64 pos, int id) {
    const QString queryFindVerbBuddySql("select distinct " QUOTEMACRO(
        JMDICTENTRY_GLOBALID) ", jmdict.entries.id "
                              "from jmdict.entries "
                              "join jmdict.kanji on jmdict.kanji.id = "
                              "jmdict.entries.id "
                              "join jmdict.senses on jmdict.senses.id = "
                              "jmdict.entries.id "
...
QString JMdictEntryFormatter::getHomophonesSql(const QString &reading, int id, int maxToDisplay,
                                               bool studiedOnly) {
    const QString queryFindHomophonesSql("select distinct " QUOTEMACRO(
        JMDICTENTRY_GLOBALID) ", jmdict.entries.id from jmdict.entries "
                              "join jmdict.kana on jmdict.kana.id = "
                              "jmdict.entries.id "
                              "%4join training on training.id = "
                              "jmdict.entries.id and training.type "
                              "= " QUOTEMACRO(JMDICTENTRY_GLOBALID) " "
                                                                    "join jmdict.senses on "
                                                                    "jmdict.senses.id = "
                                                                    "jmdict.entries.id "
                                                                    ...
                                                                    "match '\"%1\"') "
                                                                    "and "
                                                                    "jmdict.kana.priority "
                                                                    "= "
                                                                    "0 "
                                                                    "and "
                                                                    "jmdict.senses.misc0 "
                                                                    "& %5 == 0 "
                                                                    "and jmdict.entries.id "
                                                                    "!= %3 "
                                                                    "order by "
...
QString JMdictEntryFormatter::getHomographsSql(const QString &writing, int id, int maxToDisplay,
                                               bool studiedOnly) {
    const QString queryFindHomographsSql("select distinct " QUOTEMACRO(
        JMDICTENTRY_GLOBALID) ", jmdict.entries.id from jmdict.entries "
                              "join jmdict.kanji on jmdict.kanji.id = "
                              "jmdict.entries.id "
                              "%4join training on training.id = "
...
JMdictEntryFormatter::JMdictEntryFormatter(QObject *parent)
    : EntryFormatter("detailed_jmdict.css", "detailed_jmdict.html", parent) {
    _exampleSentencesServices["Tatoeba"] = tatoebaTemplate;
    _exampleSentencesServices["Jisho.org"] = jishoTemplate;
}

void JMdictEntryFormatter::drawCustom(const ConstEntryPointer &_entry, QPainter &painter,
                                      const QRectF &rectangle, QRectF &usedSpace,
                                      const QFont &textFont, int headerPrintSize, bool printKanjis,
                                      bool printOnlyStudiedKanjis,
                                      int maxDefinitionsToPrint) const {
    ...
    if (printKanjis) {
        ...
        foreach (const QChar &c, writing) {
            if (TextTools::isKanjiChar(c) && !usedKanjis.contains(c)) {
                usedKanjis << c;
                ConstKanjidic2EntryPointer kanji(
                    KanjiEntryRef(c.unicode()).get().staticCast<const Kanjidic2Entry>());
                if (!kanji)
                    continue;
                if (printOnlyStudiedKanjis && !kanji->trained())
                    continue;
...
QString JMdictEntryFormatter::formatHead(const ConstEntryPointer &_entry) const {
    ...
    const QList<KanjiReading> &kanjis = entry->getKanjiReadings();
    const QList<KanaReading> &kanas = entry->getKanaReadings();
    if (!kanjis.isEmpty() && !entry->writtenInKana())
        return kanjis[0].getReading();
    else if (!kanas.isEmpty())
...
QString JMdictEntryFormatter::formatAltReadings(const ConstEntryPointer &_entry) const {
    ...
    if (kanjis.isEmpty())
        ...
        foreach (qint32 idx, kanjis[0].getKanaReadings())
            altReadings << kanas[idx];
...
static QString senseProps(const QSet<QString> &entities, const QString &tag,
                          const QMap<QString, QPair<QString, quint16>> &map) {
    QStringList ret;
    for (const auto &entity : entities) {
        QString translated =
            QCoreApplication::translate("JMdictLongDescs", map[entity].first.toLatin1());
        translated.replace(0, 1, translated[0].toUpper());
        ret << QString("<a href=\"%1\" title=\"%3\">%2</a>")
                   .arg(QString("longdesc://%1#%2").arg(tag).arg(entity))
                   .arg(entity)
                   .arg(translated);
    }
...
static QString senseProps(const Sense &sense) {
    ...
    ret.removeAll("");
    if (!ret.isEmpty())
        return QString("<span class=\"senseProps\">%1</span>").arg(ret.join(", "));
    else
        return "";
}
...
QString JMdictEntryFormatter::formatSenses(const ConstEntryPointer &_entry) const {
    ...
    if (senses.isEmpty()) {
        ...
        ret += tr("<p>This entry has been removed from the JMdict and cannot be "
                  "displayed.</p>");
        ret += tr("<p>Please find a replacement and remove this entry from "
                  "your study "
                  "list, tags, and notes. To help you find a replacement, the "
                  "legacy "
                  "content of this entry is visible <a href=\"%1\">here</a>.")
                   .arg(QString("http://www.edrdg.org/jmdictdb/cgi-bin/"
                                "entr.py?svc=jmdict&sid=&q=%1")
                            .arg(entry->id()));
        ret += "</div>";
    }
    ...
    foreach (const Sense *sense, senses) {
        ...
        foreach (const Gloss &gloss, glosses) {
            ...
            if (str.isEmpty())
                continue;
            str[0] = str[0].toUpper();
            gls << QString("<span class=\"glossbody\"><img src=\"flag:%1\"/> "
                           "%2.</span>")
                       .arg(gloss.lang())
                       .arg(str.toHtmlEscaped());
        }
        ret += gls.join("<br/>");
        ...
        if (!sense->stagK().isEmpty()) {
            foreach (int idx, sense->stagK()) {
                ...
                if (!kList.isEmpty()) {
                    ...
                    foreach (qint32 idx2, kList)
                        kanasList << kanas[idx2].getReading();
                    str += " (" + kanasList.join(",") + ")";
                }
                senseHeaders << str;
            }
        ...
        if (!senseHeaders.isEmpty()) {
            ...
            ret += QString("<span class=\"altSenseReadings\"> [ %1 ]</span>")
                       .arg(autoFormat(senseHeaders.join(", ")));
    ...
    if (showKanjis.value() && !jEntry->getKanjiReadings().isEmpty()) {
        ...
        for (int i = 0; i < reading.size(); ++i) {
            if (TextTools::isKanjiChar(reading, i)) {
                QString k(reading[i]);
                if (reading[i].isHighSurrogate())
                    k += reading[++i];
                ConstKanjidic2EntryPointer kanji =
                    KanjiEntryRef(TextTools::singleCharToUnicode(k)).get();
                if (kanji) {
                    const EntryFormatter *formatter(EntryFormatter::getFormatter(kanji));
                    contents += QString("<div class=\"kanjiinfo\">%1</div>")
                                    .arg(formatter->shortDesc(kanji));
                    // TODO
...
QList<DetailedViewJob *> JMdictEntryFormatter::jobVerbBuddy(const ConstEntryPointer &_entry,
...
QList<DetailedViewJob *> JMdictEntryFormatter::jobHomophones(const ConstEntryPointer &_entry,
                                                             const QTextCursor &cursor) const {
    ConstJMdictEntryPointer entry(_entry.staticCast<const JMdictEntry>());
    QList<DetailedViewJob *> ret;
    if (maxHomophonesToDisplay.value())
        ret << new FindHomophonesJob(entry, maxHomophonesToDisplay.value(),
                                     displayStudiedHomophonesOnly.value(), cursor);
    return ret;
}

QList<DetailedViewJob *> JMdictEntryFormatter::jobHomographs(const ConstEntryPointer &_entry,
                                                             const QTextCursor &cursor) const {
    ConstJMdictEntryPointer entry(_entry.staticCast<const JMdictEntry>());
    QList<DetailedViewJob *> ret;
    if (maxHomographsToDisplay.value())
        ret << new FindHomographsJob(entry, maxHomographsToDisplay.value(),
                                     displayStudiedHomographsOnly.value(), cursor);
    return ret;
}

FindVerbBuddyJob::FindVerbBuddyJob(const ConstJMdictEntryPointer &verb, const QString &pos,
    ...
    while (!TextTools::isKanjiChar(matchPattern[matchPattern.size() - 1]) &&
           !matchPattern.isEmpty()) {
    ...
    if (!matchPattern.size() || !kanaPattern.size())
        return;
...
void FindVerbBuddyJob::result(EntryPointer entry) {
    ...
    if (writing.size() < lastKanjiPos)
        return;
    // Ensure we have no kanji after lastKanjiPos
    for (int i = lastKanjiPos; i < writing.size(); i++)
        if (TextTools::isKanjiChar(writing[i]))
            return;
    // Candidates which kana reading do not even match a single character are
    // not considered
    {
        QList<qint32> readingIndexes(jEntry->getKanjiReadings()[0].getKanaReadings());
        if (readingIndexes.isEmpty())
            return;
        if (jEntry->getKanaReadings()[readingIndexes[0]].getReading()[0] != kanaPattern[0])
            return;
    }
    ...
    // 2) We match longer and better than the current candidate to the original
    // reading
    {
        ...
        bool exit = false;
        do {
            if (bestMatchLength >= firstReading.size() ||
                candidateMatchLength >= firstReading.size())
                break;
            if (bestMatchReading[bestMatchLength] == firstReading[bestMatchLength])
                bestMatchLength++;
            else
                exit = true;
            if (candidateReading[candidateMatchLength] == firstReading[candidateMatchLength])
        ...
        // We match better if the phoneme after the match is in the same row as
        // the phoneme of the buddy
        else if (candidateMatchLength == bestMatchLength &&
                 candidateReading.size() > candidateMatchLength &&
                 firstReading.size() > candidateMatchLength) {
            int firstReadingRow = TextTools::kanasTableRow(firstReading[candidateMatchLength]);
            if (TextTools::kanasTableRow(candidateReading[candidateMatchLength]) ==
                firstReadingRow) {
                bestMatch = jEntry;
                return;
            }
    ...
    if (qAbs(writing.size() - initialLength) <
        qAbs(bestMatch->writings()[0].size() - initialLength)) {
        bestMatch = jEntry;
        return;
    }
}
...
void FindVerbBuddyJob::completed() {
    ...
    if (formatter)
        cursor().insertHtml(QString("<br/>%1").arg(EntryFormatter::buildSubInfoLine(
            QString(searchedPos == "vt"   ? tr("Transitive buddy")
                    : searchedPos == "vi" ? tr("Intransitive buddy")
                                          : tr("Buddy")),
            formatter->shortDesc(bestMatch))));
}

FindHomophonesJob::FindHomophonesJob(const ConstJMdictEntryPointer &entry, int maxToDisplay,
                                     bool studiedOnly, const QTextCursor &cursor)
    : DetailedViewJob(cursor), gotResults(false) {
    QString s;
    // No reading? Must be a katakana entry, take the writing in this case.
    if (!entry->readings().isEmpty())
        s = entry->readings()[0];
    else if (!entry->writings().isEmpty())
        s = entry->writings()[0];
    else
        return;
...
void FindHomophonesJob::firstResult() {
    cursor().insertHtml(
        QString("<br/>%1").arg(EntryFormatter::buildSubInfoBlock(tr("Homophones"), "")));
...
FindHomographsJob::FindHomographsJob(const ConstJMdictEntryPointer &entry, int maxToDisplay,
                                     bool studiedOnly, const QTextCursor &cursor)
    : DetailedViewJob(cursor), gotResults(false) {
    // No writing? Nothing we can possibly do here.
    if (entry->writings().isEmpty())
        return;
    _sql = JMdictEntryFormatter::getHomographsSql(entry->writings()[0], entry->id(), maxToDisplay,
                                                  studiedOnly);
}

void FindHomographsJob::firstResult() {
    cursor().insertHtml(
        QString("<br/>%1").arg(EntryFormatter::buildSubInfoBlock(tr("Homographs"), "")));
    _cursor.movePosition(QTextCursor::PreviousBlock);
}
