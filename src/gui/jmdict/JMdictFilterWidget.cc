...
JMdictFilterWidget::JMdictFilterWidget(QWidget *parent) : SearchFilterWidget(parent, "wordsdic") {
    _propsToSave << "containedKanjis" << "containedComponents" << "studiedKanjisOnly"
                 << "kanaOnlyWords" << "pos" << "dial" << "field" << "misc";
    QVBoxLayout *mainLayout = new QVBoxLayout(this);
    {
        ...
        connect(_containedKanjis, SIGNAL(textChanged(const QString &)), this,
                SLOT(commandUpdate()));
        connect(_containedComponents, SIGNAL(textChanged(const QString &)), this,
                SLOT(commandUpdate()));
    ...
    }
    {
        ...
        connect(&JMdictEntrySearcher::miscPropertiesFilter, SIGNAL(valueChanged(QVariant)), this,
                SLOT(updateMiscFilteredProperties()));
...
QString JMdictFilterWidget::currentCommand() const {
    ...
    if (!_posList.isEmpty())
        ret += " :pos=" + _posList.join(",");
    if (!_dialList.isEmpty())
        ret += " :dial=" + _dialList.join(",");
    if (!_fieldList.isEmpty())
        ret += " :field=" + _fieldList.join(",");
    if (!_miscList.isEmpty())
        ret += " :misc=" + _miscList.join(",");
    return ret;
}
...
QString JMdictFilterWidget::currentTitle() const {
    ...
    if (_studiedKanjisCheckBox->isChecked()) {
        if (!kanjis.isEmpty() || !comps.isEmpty())
            contains += tr(", studied kanji only");
        else
            contains += tr(" with studied kanji");
    }
    if (_kanaOnlyCheckBox->isChecked()) {
        if (!kanjis.isEmpty() || !comps.isEmpty() || _studiedKanjisCheckBox->isChecked())
            contains += tr(", including kana words");
        else
            contains += tr(" using kana only");
    }
    ...
    if (!ret.isEmpty())
        ret = tr("Vocabulary") + ret;
    else
        ret = tr("Vocabulary");
    return ret;
}

void JMdictFilterWidget::__onPropertyTriggered(QAction *action, QStringList &list,
                                               QPushButton *button) {
...
QActionGroup *
JMdictFilterWidget::addCheckableProperties(const QMap<QString, QPair<QString, quint16>> &map,
                                           QMenu *menu) {
    QMap<QString, QString> strMap;
    QStringList strList;
    for (const auto &tag : map.keys()) {
        QString translated =
            QCoreApplication::translate("JMdictLongDescs", map[tag].first.toLatin1());
