...
JMdictPreferences::JMdictPreferences(QWidget *parent)
    : PreferencesWindowCategory(tr("Vocabulary entries"), parent) {
    setupUi(this);
...
void JMdictPreferences::refresh() {
    ...
    for (int i = 1; i < exampleSentencesServiceBox->count(); i++)
        if (exampleSentencesServiceBox->itemText(i) ==
            JMdictEntryFormatter::exampleSentencesService.value()) {
...
void JMdictPreferences::applySettings() {
    ...
    if (exampleSentencesServiceBox->currentIndex() == 0)
        ...
        JMdictEntryFormatter::exampleSentencesService.set(
            exampleSentencesServiceBox->currentText());
...
void JMdictPreferences::updatePrintPreview() {
    ...
    const JMdictEntryFormatter *formatter =
        static_cast<const JMdictEntryFormatter *>(EntryFormatter::getFormatter(previewEntry));
    ...
    formatter->drawCustom(
        ...
        usedSpace, QFont(), headerPrintSize->value(), printKanjis->isChecked(),
        printOnlyStudiedKanjis->isChecked(), maxDefinitionsToPrint->value());
