...
JMdictGUIPlugin::JMdictGUIPlugin()
    : Plugin("JMdictGUI"), _flashJL(0), _flashJS(0), _flashTL(0), _flashTS(0), _linkhandler(0),
      _filter(0), _trainer(0) {}

JMdictGUIPlugin::~JMdictGUIPlugin() {}

bool JMdictGUIPlugin::onRegister() {
    // Check if the JMdict plugin is loaded
    if (!Plugin::pluginExists("JMdict"))
        return false;
    // Register the formatter
    if (!EntryFormatter::registerFormatter(JMDICTENTRY_GLOBALID, &JMdictEntryFormatter::instance()))
        return false;
...
void JMdictLinkHandler::handleUrl(const QUrl &url, DetailedView *view) {
    ...
    if (url.authority() == "pos") {
        translated = QCoreApplication::translate(
            "JMdictLongDescs", JMdictPlugin::posMap()[url.fragment()].first.toLatin1());
    } else if (url.authority() == "misc") {
        translated = QCoreApplication::translate(
            "JMdictLongDescs", JMdictPlugin::miscMap()[url.fragment()].first.toLatin1());
    } else if (url.authority() == "dialect") {
        translated = QCoreApplication::translate(
            "JMdictLongDescs", JMdictPlugin::dialMap()[url.fragment()].first.toLatin1());
    } else if (url.authority() == "field") {
        translated = QCoreApplication::translate(
            "JMdictLongDescs", JMdictPlugin::fieldMap()[url.fragment()].first.toLatin1());
    } else
        return;
    QToolTip::showText(QCursor::pos(), translated.replace(0, 1, translated[0].toUpper()), 0,
                       QRect());
}
...
void JMdictGUIPlugin::trainingJapaneseSet() {
    ...
    if (!stat) {
        QMessageBox::information(MainWindow::instance(), tr("Nothing to train"),
                                 tr("There are no vocabulary entries in this set to train on."));
        return;
    }
...
void JMdictGUIPlugin::trainerDeleted() { _trainer = 0; }
