...
QModelIndex EntryListModel::indexFromList(quint64 listId, quint64 position) const {
    ...
    // FIXME Qt is wrong here - internalId() returns a qint64, so this function
    // should take a qint64 too!
...
QModelIndex EntryListModel::parent(const QModelIndex &idx) const {
    ...
    if (list.listId() == 0)
        return QModelIndex();
    // We need to construct an index by determining the parent list of idx and
    // the position of idx within it
    else {
...
int EntryListModel::rowCount(const QModelIndex &index) const {
    ...
    if (!index.isValid())
        return EntryListCache::get(0)->size();
    else {
        ...
        if (!cEntry.isList())
            ...
            return EntryListCache::get(cEntry.id)->size();
    }
}
...
QVariant EntryListModel::data(const QModelIndex &index, int role) const {
    ...
    case Qt::EditRole: {
        if (cEntry.isList())
            return EntryListCache::get(cEntry.id)->label();
    ...
    case Qt::BackgroundRole: {
        ...
        EntryPointer entry(cEntry.entryRef().get());
        if (!entry || !entry->trained())
            return QVariant();
        else
            return EntryFormatter::scoreColor(*entry);
    }
    case Qt::FontRole: {
        if (cEntry.isList()) {
            QFont font;
...
bool EntryListModel::setData(const QModelIndex &index, const QVariant &value, int role) {
    if (role == Qt::EditRole && index.isValid()) {
        ...
        if (cEntry.isList()) {
            ...
            if (list.setLabel(value.toString()))
                return true;
        }
    }
    return false;
...
// TODO How to handle the beginInsertRows if the transaction failed and
// endInsertRows is not called?
bool EntryListModel::insertRows(int row, int count, const QModelIndex &parent) {
    ...
    if (!parent.isValid())
        parentList = EntryListCache::get(0);
    else {
        const EntryListData &cEntry = INDEXDATA(parent);
        Q_ASSERT(cEntry.isList());
...
QMimeData *EntryListModel::mimeData(const QModelIndexList &indexes) const {
    ...
    foreach (const QModelIndex &index, indexes) {
        if (index.isValid()) {
            ...
            const EntryListData &cEntry = INDEXDATA(index);
            if (!cEntry.isList())
                entriesStream << cEntry.entryRef();
            // TODO in case of a list, add all the items the list contains
        }
    }
    if (!entriesEncodedData.isEmpty())
        mimeData->setData("tagainijisho/entry", entriesEncodedData);
    if (!itemsEncodedData.isEmpty())
        mimeData->setData("tagainijisho/listitem", itemsEncodedData);
...
bool EntryListModel::dropMimeData(const QMimeData *data, Qt::DropAction action, int row, int column,
    ...
    if (data->hasFormat("tagainijisho/listitem")) {
        ...
        while (!ds.atEnd()) {
            ...
            // Decrease the destination index if we are removing an item in the
            // destination list with an index inferior to that of the drop
            if (iRef.list == &list && iRef.row < origRow)
                --row;
        }
        ...
        // Insert all the nodes into the destination list and prepare persistent
        // indexes for update
        for (int i = 0; i < iRefs.size(); i++) {
            ...
            while (it.hasNext()) {
                ...
                if (idx.internalId() == list.listId() && idx.row() >= origRow)
                    updatedPIndexes[idx].row++;
            }
        }
    ...
    // add the entries to the list
        ...
        foreach (const EntryRef &entry, entries) {
            ...
            if (entry.isLoaded()) {
                entry.get()->addToList(EntryListCache::getRowIdFromIndex(
                    QPair<const EntryList *, quint32>(&list, row + cpt)));
            }
            cpt++;
        }
        if (!EntryListCache::connection()->commit())
