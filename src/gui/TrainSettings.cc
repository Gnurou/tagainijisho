...
TrainSettings::TrainSettings(QWidget *parent) : QDialog(parent) {
    ...
    connect(buttonBox->button(QDialogButtonBox::RestoreDefaults), SIGNAL(clicked()), this,
...
void TrainSettings::checkMinScoreValue(int newMaxValue) {
    if (newMaxValue < minScore->value())
        minScore->setValue(newMaxValue);
}

void TrainSettings::checkMaxScoreValue(int newMinValue) {
    if (newMinValue > maxScore->value())
        maxScore->setValue(newMinValue);
}
...
QString TrainSettings::buildQueryString(int entryType) {
    ...
    if (maxScore < maxScorePref.defaultValue())
        queryString += QString(" and score <= %1").arg(maxScore);
...
QString TrainSettings::buildOrderString(const QString &scoreColumn) {
    ...
    if (bias == TrainSettings::BIAS_SCORE)
        return QString("ORDER BY biased_random(%1) DESC").arg(scoreColumn);
    else
