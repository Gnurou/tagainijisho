...
StudyFilterWidget::StudyFilterWidget(QWidget *parent) : SearchFilterWidget(parent) {
    _propsToSave << "studyState" << "studyMinScore" << "studyMaxScore" << "studyMinDate"
                 << "studyMaxDate" << "trainMinDate" << "trainMaxDate" << "mistakenMinDate"
                 << "mistakenMaxDate";
    ...
    connect(studyBox, SIGNAL(toggled(bool)), this, SLOT(commandUpdate()));
    {
        ...
        if (query.next()) {
            infDate = QDateTime::fromSecsSinceEpoch(query.valueUInt(0)).date();
        }
        query.exec("select max(dateAdded) from training");
        if (query.next()) {
            supDate = QDateTime::fromSecsSinceEpoch(query.valueUInt(0)).date();
        }

        _studyMinDate = new RelativeDateEdit(studyBox);
        connect(_studyMinDate, SIGNAL(dateSelected(const RelativeDate &)), this,
                SLOT(commandUpdate()));
        connect(_studyMinDate, SIGNAL(dateChanged(const RelativeDate &)), this,
                SLOT(delayedCommandUpdate()));
        _studyMaxDate = new RelativeDateEdit(studyBox);
        connect(_studyMaxDate, SIGNAL(dateSelected(const RelativeDate &)), this,
                SLOT(commandUpdate()));
        connect(_studyMaxDate, SIGNAL(dateChanged(const RelativeDate &)), this,
                SLOT(delayedCommandUpdate()));
    ...
    connect(trainBox, SIGNAL(toggled(bool)), this, SLOT(commandUpdate()));
    {
        ...
        connect(_trainMinDate, SIGNAL(dateSelected(const RelativeDate &)), this,
                SLOT(commandUpdate()));
        connect(_trainMinDate, SIGNAL(dateChanged(const RelativeDate &)), this,
                SLOT(delayedCommandUpdate()));
        _trainMaxDate = new RelativeDateEdit(trainBox);
        connect(_trainMaxDate, SIGNAL(dateSelected(const RelativeDate &)), this,
                SLOT(commandUpdate()));
        connect(_trainMaxDate, SIGNAL(dateChanged(const RelativeDate &)), this,
                SLOT(delayedCommandUpdate()));
    ...
    connect(mistakeBox, SIGNAL(toggled(bool)), this, SLOT(commandUpdate()));
    {
        ...
        connect(_mistakenMinDate, SIGNAL(dateSelected(const RelativeDate &)), this,
                SLOT(commandUpdate()));
        connect(_mistakenMinDate, SIGNAL(dateChanged(const RelativeDate &)), this,
                SLOT(delayedCommandUpdate()));

        _mistakenMaxDate = new RelativeDateEdit(mistakeBox);
        connect(_mistakenMaxDate, SIGNAL(dateSelected(const RelativeDate &)), this,
                SLOT(commandUpdate()));
        connect(_mistakenMaxDate, SIGNAL(dateChanged(const RelativeDate &)), this,
                SLOT(delayedCommandUpdate()));
    ...
    QWidget *entriesBox = new QWidget(this);
    {
        ...
        connect(studiedEntriesButton, SIGNAL(toggled(bool)), this,
                SLOT(onRadioButtonToggled(bool)));
        nonStudiedEntriesButton = new QRadioButton(tr("Not studied"), entriesBox);
        connect(nonStudiedEntriesButton, SIGNAL(toggled(bool)), this,
                SLOT(onRadioButtonToggled(bool)));
...
QString StudyFilterWidget::currentCommand() const {
    ...
    if (studiedEntriesButton->isChecked()) {
        ...
        if (!minTrainDateString.isEmpty() || !maxTrainDateString.isEmpty())
            ret += QString(" :lasttrained=\"%1\",\"%2\"")
                       .arg(minTrainDateString)
                       .arg(maxTrainDateString);
        ...
        if (!minMistakenDateString.isEmpty() || !maxMistakenDateString.isEmpty())
            ret += QString(" :mistaken=\"%1\",\"%2\"")
                       .arg(minMistakenDateString)
                       .arg(maxMistakenDateString);
    } else if (nonStudiedEntriesButton->isChecked())
        ret += " :nostudy";
...
void StudyFilterWidget::checkMinSpinBoxValue(int newMaxValue) {
    if (newMaxValue < minSpinBox->value())
        minSpinBox->setValue(newMaxValue);
}

void StudyFilterWidget::checkMaxSpinBoxValue(int newMinValue) {
    if (newMinValue > maxSpinBox->value())
        maxSpinBox->setValue(newMinValue);
}
...
QString StudyFilterWidget::currentTitle() const {
    ...
    if (studiedEntriesButton->isChecked()) {
        ...
        if (!maxTrainDateString.isEmpty()) {
            if (minTrainDateString.isEmpty())
                newTitle += tr(", trained until %1").arg(maxTrainDateString);
            else
                newTitle += tr(" until %1").arg(maxTrainDateString);
        }
        ...
        if (!maxMistakenDateString.isEmpty()) {
            if (minMistakenDateString.isEmpty())
                newTitle += tr(", mistaken until %1").arg(maxMistakenDateString);
            else
                newTitle += tr(" until %1").arg(maxMistakenDateString);
        }
    } else if (nonStudiedEntriesButton->isChecked())
        newTitle = tr("Not studied");
    else
        newTitle = tr("Study");
