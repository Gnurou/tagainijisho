...
/// State version of the main window - should be increated every time the docks
/// or statusbars change
...
DockTitleBar::DockTitleBar(QWidget *widget, QDockWidget *parent) : QWidget(parent) {
    ...
    hLayout->addStretch();
    QIcon icon(static_cast<QApplication *>(QApplication::instance())
                   ->style()
...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), _clipboardEnabled(false) {
    ...
    // fixed in Qt 4.5.
    connect(_updateChecker, SIGNAL(updateAvailable(const QString &)), this,
            SLOT(updateAvailable(const QString &)), Qt::QueuedConnection);
    connect(_betaUpdateChecker, SIGNAL(updateAvailable(const QString &)), this,
            SLOT(betaUpdateAvailable(const QString &)), Qt::QueuedConnection);
...
void MainWindow::setupSearchWidget() {
    ...
    QWidget *filtersToolBar =
        static_cast<QBoxLayout *>(searchWidget()->layout())->takeAt(0)->widget();
    ...
    connect(searchWidget()->resultsView(), SIGNAL(entrySelected(EntryPointer)), detailedView(),
            SLOT(display(EntryPointer)));
...
void MainWindow::setupListWidget() {
    ...
    connect(_entryListWidget->entryListView(), SIGNAL(entrySelected(EntryPointer)), detailedView(),
            SLOT(display(EntryPointer)));
...
void MainWindow::setupClipboardSearchShortcut() {
    ...
    QAction *_enableClipboardInputAction = new QAction(
        QIcon(":/images/icons/clipboard-search.png"), tr("Auto-search on clipboard content"), this);
    _enableClipboardInputAction->setCheckable(true);
    connect(_enableClipboardInputAction, SIGNAL(toggled(bool)), this,
            SLOT(enableClipboardInput(bool)));
...
void MainWindow::updateCheck() {
    if (autoCheckUpdates.value()) {
        ...
        if (!dt.isValid() ||
            dt.addDays(updateCheckInterval.value()) <= QDateTime::currentDateTime()) {
            _updateChecker->checkForUpdates();
            if (autoCheckBetaUpdates.value())
                _betaUpdateChecker->checkForUpdates();
    ...
    if (!donationReminderDisplayed.value()) {
        ...
        if (QDateTime::currentDateTime() > firstRunTime.value().addDays(14)) {
            QMessageBox messageBox(QMessageBox::Information,
                                   tr("Please consider supporting Tagaini Jisho"),
                                   tr("You have now been using Tagaini Jisho for a short while. "
                                      "If you "
                                      "like it, please show your support by making a donation "
                                      "that "
                                      "expresses your appreciation of this software. While "
                                      "Tagaini is "
                                      "free, its development has a cost and your support will "
                                      "have a "
                                      "positive influence on its future. You can donate via "
                                      "Paypal or "
                                      "credit card.\n\nThis message is a one-time reminder and "
                                      "will not "
                                      "appear anymore - you can still donate later by choosing "
                                      "the "
                                      "corresponding option in the Help menu."),
...
void MainWindow::exportUserData() {
    ...
    QFile(to).remove(); // QFile::copy does not overwrite, so delete it first
                        // (if existent).
    if (!dataFile.copy(to)) {
        QMessageBox::critical(this, tr("Export user data..."),
                              tr("Error: Could not export database! Please verify that you have "
...
void MainWindow::importUserData() {
    ...
    QMessageBox::StandardButton ret =
        QMessageBox::warning(this, tr("About to replace user data!"),
                             tr("<b>WARNING:</b> All the user data (study list, tags, notes, "
                                "training, ...) is going to be replaced by the selected user data "
                                "file. Are you <b>sure</b> you want to do this?"),
    ...
    ret = QMessageBox::information(
        this, tr("Please restart Tagaini Jisho"),
        tr("The imported data will be available the next time you start "
           "Tagaini "
           "Jisho. All changes performed from now will be lost. Do you want to "
           "exit Tagaini Jisho now?"),
...
void MainWindow::donate() { QDesktopServices::openUrl(QUrl("https://www.tagaini.net/donate")); }
...
#error No version defined - the -DVERSION=<version> flag must be set!
#endif
    QString message =
        QString("<p>Copyright (C) 2008-2022 Alexandre Courbot.</p>"
                "<p align=\"center\"><a "
                "href=\"https://www.tagaini.net\">https://www.tagaini.net</a></"
                "p><p>This "
                "program comes with ABSOLUTELY NO WARRANTY. This is free software, and "
                "you are welcome to redistribute it under the conditions of the <a "
                "href=\"https://www.gnu.org/licenses/gpl-3.0.html\">GNU General Public "
                "License, version 3.0</a>, or (at your option) any later "
                "version.</p><hr/>");
    ...
    credits += "<p>Fugue Icons Copyright (C) 2010 <a "
               "href=\"https://p.yusukekamiyamane.com/\">Yusuke Kamiyamane</a>, "
               "licensed under a <a "
    ...
    aboutDialogUI.logo->setPixmap(
        aboutDialogUI.logo->pixmap(Qt::ReturnByValue).scaledToWidth(75, Qt::SmoothTransformation));
    ...
    QString authors = "<h2>Authors</h2>"
                      "<p><b>Alexandre Courbot</b> project lead, programming.</p>"
                      "<p><b>Axel Bodart</b> CMake improvements, web site guru, "
                      "Mac release manager.</p>"
                      ...
                      "<p>All the translators on <a "
                      "href=\"https://www.transifex.net/projects/p/"
                      "tagaini-jisho\">Transifex</a>.</p>"
                      "<p>All the persons who donated and contributed directly "
                      "or indirectly to this software!</p>"
                      "";
...
void MainWindow::openUrl(const QUrl &url) { QDesktopServices::openUrl(url); }
...
void MainWindow::manual() {
    ...
    #else
            QString f =
    QDir(QCoreApplication::applicationDirPath()).filePath("doc/manual.html");
    #endif
    */
...
void MainWindow::updateAvailable(const QString &version) {
    QMessageBox messageBox(QMessageBox::Information, tr("An update is available!"),
                           tr("Version %1 of Tagaini Jisho is available. Do you "
...
void MainWindow::betaUpdateAvailable(const QString &version) {
    QMessageBox messageBox(QMessageBox::Information, tr("A development update is available!"),
                           tr("Development version %1 of Tagaini Jisho is "
...
void MainWindow::populateMenu(QMenu *menu, int parentId) {
    ...
    query.exec(QString("SELECT label, state, rowid FROM sets WHERE parent %1 ORDER BY "
                       "position")
                   .arg(parentId == 0 ? "is null" : QString("= %1").arg(parentId)));
    while (query.next()) {
        // A folder
        if (query.valueIsNull(1)) {
            QMenu *subMenu =
                menu->addMenu(QIcon(":/images/icons/folder.png"), query.valueString(0));
        ...
        // A set
            ...
            QAction *action =
                menu->addAction(query.valueString(0), this, SLOT(onSavedSearchSelected()));
...
void MainWindow::populateSavedSearchesMenu() {
    ...
    foreach (QAction *action, _rootActions)
        delete action;
    _rootActions.clear();
    foreach (QMenu *menu, _rootMenus)
        delete menu;
    _rootMenus.clear();
...
void MainWindow::populateSubMenu() {
    ...
    menu->addSeparator();
    QAction *action =
        menu->addAction(tr("Save current search here..."), this, SLOT(newSavedSearch()));
...
void MainWindow::newSavedSearch() {
    ...
    bool ok;
    QString setName(QInputDialog::getText(this, tr("Save current search"),
                                          tr("Please enter a name for this search:"),
                                          QLineEdit::Normal, tr("Unnamed"), &ok));
    if (!ok)
        return;
    ...
    SQLite::Query query(Database::connection());
    query.prepare(QString("INSERT INTO sets VALUES(NULL, %2, ifnull((SELECT max(position) "
                          "+ 1 FROM sets WHERE parent %1), 0), ?, ?)")
                      .arg(!parentId ? "is null" : QString("= %1").arg(parentId))
                      .arg(!parentId ? "null" : QString::number(parentId)));
...
void MainWindow::newSavedSearchesFolder() {
    ...
    bool ok;
    QString setName(QInputDialog::getText(this, tr("New folder"),
                                          tr("Please enter a name for this folder"),
                                          QLineEdit::Normal, tr("Unnamed folder"), &ok));
    if (!ok)
        return;

    SQLite::Query query(Database::connection());
    query.prepare(QString("INSERT INTO sets VALUES(NULL, %2, ifnull((SELECT max(position) "
                          "+ 1 FROM sets WHERE parent %1), 0), ?, null)")
                      .arg(!parentId ? "is null" : QString("= %1").arg(parentId))
                      .arg(!parentId ? "null" : QString::number(parentId)));
    query.bindValue(setName);
    // TODO error handling
...
void MainWindow::onSavedSearchSelected() {
    ...
    if (_searchDockWidget->isHidden())
        _searchDockWidget->setHidden(false);
}
...
void MainWindow::display(const QModelIndex &clicked) {
    ...
    if (view == searchWidget()->resultsView() &&
        !searchWidget()->resultsView()->selectionModel()->isSelected(clicked))
        return;
    else if (view == _entryListWidget->entryListView() &&
             !_entryListWidget->entryListView()->selectionModel()->isSelected(clicked))
        return;
    EntryPointer entry(clicked.data(Entry::EntryRole).value<EntryPointer>());
    // Do not redisplay an entry if it is already shown
    if (_detailedView->detailedView()->entryView()->entry() == entry)
        return;
    if (entry)
        _detailedView->detailedView()->display(entry);
}

void MainWindow::resetSearch() { searchWidget()->resetSearch(); }

void MainWindow::focusTextSearch() {
    TextFilterWidget *tWidget =
        qobject_cast<TextFilterWidget *>(searchWidget()->getSearchFilter("searchtext"));
    if (!tWidget)
        return;
    searchWidget()->searchFilters()->showWidget(tWidget);
    // showWidget is supposed to do this already, but somehow this is still
    // necessary.
    tWidget->setFocus();
    tWidget->searchField()->lineEdit()->selectAll();
}

void MainWindow::focusResultsList() { searchWidget()->resultsView()->setFocus(); }

void MainWindow::enableClipboardInput(bool enable) {
    QClipboard *clipboard = QApplication::clipboard();
    if (enable && !_clipboardEnabled) {
        _clipboardEnabled = true;
        connect(clipboard, SIGNAL(changed(QClipboard::Mode)), this,
                SLOT(onClipboardChanged(QClipboard::Mode)));
    } else if (!enable && _clipboardEnabled) {
        disconnect(clipboard, SIGNAL(changed(QClipboard::Mode)), this,
                   SLOT(onClipboardChanged(QClipboard::Mode)));
...
void MainWindow::onClipboardChanged(QClipboard::Mode mode) {
    ...
    foreach (QChar c, cText) {
        ...
        if (lastCharSpace) {
            ...
            if (isJapanese && lastCharJapanese)
                cText2 += c;
            ...
            cText2 += c;
        if (!isSpace)
            lastCharJapanese = isJapanese;
        lastCharSpace = isSpace;
    }
    if (cText2.isEmpty() || cText2 == tfw->text())
        return;
    tfw->setText(cText2);
}
...
void MainWindow::fitToScreen(QWidget *widget) {
    ...
    if (screenRect.top() > popupRect.top())
