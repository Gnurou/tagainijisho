...
PreferenceItem<QString> DetailedViewFonts::kanjiHeaderFont("mainWindow/detailedView",
                                                           "kanjiHeaderFont",
                                                           QFont("Helvetica", 28).toString());
PreferenceItem<QString> DetailedViewFonts::kanaHeaderFont("mainWindow/detailedView",
                                                          "kanaHeaderFont", "");

PreferenceItem<bool> DetailedView::smoothScrolling("mainWindow/detailedView", "smoothScrolling",
                                                   true);
...
void DetailedView::registerEventFilter(QObject *obj) {
    _eventFilters << obj;
    foreach (DetailedView *view, _instances)
        view->viewport()->installEventFilter(obj);
}

void DetailedView::removeEventFilter(QObject *obj) {
    foreach (DetailedView *view, _instances)
        view->viewport()->removeEventFilter(obj);
    _eventFilters.remove(obj);
}

DetailedView::DetailedView(QWidget *parent)
    : QTextBrowser(parent), _kanjiClickable(true), _historyEnabled(true), _dragEntryRef(0, 0),
      _dragStarted(false), _history(historySize.value()), _entryView(0), _jobsRunner(this) {
    // Add the default handlers if not already done (first instanciation)
    if (!DetailedViewLinkManager::getHandler(_entryHandler.scheme()))
        DetailedViewLinkManager::registerHandler(&_entryHandler);
    if (!DetailedViewLinkManager::getHandler(_tagsLinkHandler.scheme()))
        DetailedViewLinkManager::registerHandler(&_tagsLinkHandler);
    if (!DetailedViewLinkManager::getHandler(_listLinkHandler.scheme()))
        DetailedViewLinkManager::registerHandler(&_listLinkHandler);
    // Is the fonts manager instance already running?
    if (!DetailedViewFonts::_instance)
        DetailedViewFonts::_instance = new DetailedViewFonts();
    connect(DetailedViewFonts::_instance, SIGNAL(fontsHaveChanged()), this, SLOT(redisplay()));

    // Add the registered event filters
    foreach (QObject *obj, _eventFilters)
    ...
    connect(this, SIGNAL(anchorClicked(const QUrl &)), DetailedViewLinkManager::instance(),
            SLOT(handleUrl(const QUrl &)));
    ...
    _historyPrevAction =
        new QAction(QIcon(":/images/icons/go-previous.png"), tr("Previous entry"), this);
...
void DetailedView::_display(const EntryPointer &entry, bool update) {
    ...
    if (!formatter)
        qWarning("%s %d: %s", __FILE__, __LINE__, "No formatter found for entry!");
    else {
        ...
        css += QString("\n%1 {\n%2}\n")
                   .arg(".kana")
                   .arg(DetailedViewFonts::CSS(DetailedViewFonts::Kana));
        ...
        while (!(matchPos = document()->find(funcMatch, pos)).isNull()) {
            ...
            QMetaObject::invokeMethod(
                const_cast<EntryFormatter *>(formatter),
                QString("job" + jobClass).toLatin1().constData(), Qt::DirectConnection,
                Q_RETURN_ARG(QList<DetailedViewJob *>, jobs), Q_ARG(ConstEntryPointer, entry),
                Q_ARG(QTextCursor, matchPos));
...
void DetailedView::clear() {
    ...
    document()->addResource(QTextDocument::ImageResource, QUrl("tagicon"),
                            QPixmap(":/images/icons/tags.png"));
    document()->addResource(QTextDocument::ImageResource, QUrl("listicon"),
                            QPixmap(":/images/icons/list.png"));
    document()->addResource(QTextDocument::ImageResource, QUrl("moreicon"),
                            QPixmap(":/images/icons/zoom-in.png"));
}

void DetailedView::setKanjiClickable(bool clickable) { _kanjiClickable = clickable; }
...
void DetailedView::addBackgroundJob(DetailedViewJob *job) { _jobsRunner.addJob(job); }
...
void DetailedView::mousePressEvent(QMouseEvent *e) {
    ...
    if (e->button() == Qt::LeftButton && anchor.startsWith("entry://")) {
        ...
        _dragStarted = true;
        _dragStartPos = e->pos();
        _dragEntryRef = EntryRef(QUrlQuery(url).queryItemValue("type").toInt(),
                                 QUrlQuery(url).queryItemValue("id").toInt());
        e->accept();
    } else {
        _dragStarted = false;
...
DetailedViewJobRunner::DetailedViewJobRunner(DetailedView *view, QObject *parent)
    : QObject(parent), _view(view), _currentJob(0), _ignoreJobs(false) {
    ...
    connect(_aQuery, SIGNAL(result(EntryPointer)), this, SLOT(onResult(EntryPointer)));
    connect(_aQuery, SIGNAL(completed()), this, SLOT(onCompleted()));
    connect(_aQuery, SIGNAL(aborted()), this, SLOT(onAborted()));
    connect(_aQuery, SIGNAL(error(const QString &)), this, SLOT(onError(const QString &)));
}
...
void DetailedViewJobRunner::addJob(DetailedViewJob *job) { _jobs << job; }
...
void DetailedViewJobRunner::runNextJob() {
    ...
    // Since we are only using the Query from here, it should always be
    // available!
    if (_currentJob->sql().isEmpty())
        runNextJob();
    else {
...
void DetailedViewJobRunner::abortAllJobs() {
    ...
    foreach (DetailedViewJob *job, _jobs)
        delete job;
    _jobs.clear();
...
void DetailedViewJobRunner::onResult(EntryPointer entry) {
    ...
    // Workaround for the fact QTextEdit does not seem to like when we change
    // the displayed text document so often - without this scheduled repaint
    // graphical glitches tend to appear on the first time a given entry is
    // displayed.
...
DetailedViewJob::DetailedViewJob(const QString &sql, const QTextCursor &cursor)
    : _sql(sql), _cursor(cursor) {
    __init();
}

DetailedViewJob::DetailedViewJob(const QTextCursor &cursor) : _cursor(cursor) { __init(); }
...
void DetailedViewFonts::_setFont(FontRole role, const QFont &font) { _font[role] = font; }

void DetailedViewFonts::_fontsChanged() { emit fontsHaveChanged(); }
...
QString DetailedViewFonts::_HTML(FontRole role) const {
    ...
    if (!font.family().isEmpty())
        ret += QString(" font-family: %1;").arg(font.family());
    if (color(role) != QApplication::palette().color(QPalette::Text))
...
void EntryMenuHandler::handleUrl(const QUrl &url, DetailedView *view) {
    EntryPointer entry(EntryRef(QUrlQuery(url).queryItemValue("type").toInt(),
                                QUrlQuery(url).queryItemValue("id").toInt())
                           .get());
    if (entry)
        MainWindow::instance()->detailedView()->display(entry);
}
...
void TagsLinkHandler::handleUrl(const QUrl &url, DetailedView *view) {
    ...
    TagsFilterWidget *extender =
        qobject_cast<TagsFilterWidget *>(mainWindow->searchWidget()->getSearchFilter("tagssearch"));
    if (!extender)
        return;

    if (QUrlQuery(url).hasQueryItem("reset"))
        mainWindow->searchWidget()->searchBuilder()->reset();
