...
Kanjidic2FilterWidget::Kanjidic2FilterWidget(QWidget *parent)
    : SearchFilterWidget(parent, "kanjidic") {
    _propsToSave << "strokeCount" << "isStrokeRange" << "maxStrokeCount" << "radicals"
                 << "components" << "unicode" << "skip" << "fourCorner" << "grades";
    ...
    QGroupBox *gradeGroupBox = new QGroupBox(tr("School grade"), this);
    {
        ...
        for (int i = 1; i <= 10; i++) {
            if (i == 7)
                continue;
            action = actionGroup->addAction(QCoreApplication::translate(
                "Kanjidic2GUIPlugin", Kanjidic2GUIPlugin::kanjiGrades[i].toLatin1()));
...
void showSelector(QWidget *selector, QWidget *focusWidget) {
    selector->move(focusWidget->mapToGlobal(QPoint(
        focusWidget->rect().left() + (focusWidget->rect().width() - selector->rect().width()) / 2,
    ...
    if ((watched == _radicals || watched == _components)) {
        ...
        case QEvent::FocusIn: {
            ...
            if (focusWidget == _radicals && !_radKSelector) {
                ...
                KanjiSelectorValidator *selectorValidator =
                    new KanjiSelectorValidator(_radKSelector, _radicals);
            ...
            if (focusWidget == _radicals)
                selector = _radKSelector;
            else if (focusWidget == _components)
                selector = _compKSelector;
            // If the selector has been created, setup its properties
            if (justCreated) {
                selector->setWindowFlags(Qt::Tool | Qt::FramelessWindowHint |
        ...
        case QEvent::FocusOut:
            if (focusWidget == _radicals &&
                !(QApplication::widgetAt(QCursor::pos()) != _radKSelector))
                _radKSelector->hide();
            else if (focusWidget == _components &&
                     !(QApplication::widgetAt(QCursor::pos()) != _compKSelector))
                _compKSelector->hide();
            break;
        // Show the selector is the line edit has focus
        case QEvent::Enter:
            if (focusWidget == _radicals && _radicals->hasFocus())
                showSelector(_radKSelector, _radicals);
            else if (focusWidget == _components && _components->hasFocus())
                showSelector(_compKSelector, _components);
            break;
        ...
        case QEvent::Leave:
            if (focusWidget == _radicals && _radicals->hasFocus() &&
                ...
                _compKSelector->hide();
            break;
        // If ESC is pressed while a radical/component line edit has focus, hide
        // its selector
        ...
        switch (event->type()) {
        // If the mouse leaves a selector for something else than its associated
        // line edit, hide it
        case QEvent::Leave:
            if (watched == _radKSelector && QApplication::widgetAt(QCursor::pos()) != _radicals)
                _radKSelector->hide();
            else if (watched == _compKSelector &&
                     QApplication::widgetAt(QCursor::pos()) != _components)
                _compKSelector->hide();
            break;
        default:
            break;
...
void Kanjidic2FilterWidget::onStrokeRangeToggled(bool checked) {
    // We will need to update the query if we uncheck the strokes checkbox and
    // maxStrokes != minStrokes != 0.
    bool needUpdate = (!checked && _maxStrokeCountSpinBox->value() != 0 &&
                       _maxStrokeCountSpinBox->value() != _strokeCountSpinBox->value());
    _maxStrokeCountSpinBox->setVisible(checked);
    _maxStrokeCountSpinBox->setEnabled(checked);
    // Do not allow the command update to be called as the state did not really
    // change
...
void Kanjidic2FilterWidget::onStrokeRangeChanged() {
    if (_maxStrokeCountSpinBox->isEnabled()) {
        ...
        if (sender() == _strokeCountSpinBox && minValue > maxValue)
            _maxStrokeCountSpinBox->setValue(minValue);
        else if (sender() == _maxStrokeCountSpinBox && maxValue < minValue)
            _strokeCountSpinBox->setValue(maxValue);
    }
...
QString Kanjidic2FilterWidget::currentCommand() const {
    ...
    if ((!_maxStrokeCountSpinBox->isEnabled() || maxStrokes == 0 || maxStrokes == minStrokes) &&
        minStrokes > 0) {
    ...
    if (_unicode->value())
        ret += QString(" :unicode=%1").arg(_unicode->text());
    if (_skip1->value() || _skip2->value() || _skip3->value())
        ret += QString(" :skip=%1").arg(skip());
    if (_fcTopLeft->currentIndex() > 0 || _fcTopRight->currentIndex() > 0 ||
        _fcBotLeft->currentIndex() > 0 || _fcBotRight->currentIndex() > 0 ||
        _fcExtra->currentIndex() > 0)
        ret += QString(" :fourcorner=%1").arg(fourCorner());
    if (!_gradesList.isEmpty())
        ret += " :grade=" + _gradesList.join(",");
    return ret;
}
...
QString Kanjidic2FilterWidget::currentTitle() const {
    ...
    if ((!_maxStrokeCountSpinBox->isEnabled() || maxStrokes == 0 || maxStrokes == minStrokes) &&
        ...
        if (minStrokes == 0)
            ret += tr(", strokes<=%1").arg(maxStrokes);
        else if (maxStrokes == 0)
            ret += tr(", strokes>=%1").arg(minStrokes);
        else
            ret += tr(", %1-%2 strokes").arg(minStrokes).arg(maxStrokes);
    }
    if (_unicode->value())
        ret += tr(", unicode: %1").arg(_unicode->text());
    if (_skip1->value() || _skip2->value() || _skip3->value())
        ret += tr(", skip: %1").arg(skip());
    if (_fcTopLeft->currentIndex() > 0 || _fcTopRight->currentIndex() > 0 ||
        _fcBotLeft->currentIndex() > 0 || _fcBotRight->currentIndex() > 0 ||
        _fcExtra->currentIndex() > 0)
        ret += tr(", 4c: %1").arg(fourCorner());
    if (!_gradesList.isEmpty())
        ret += tr(", grade: %1").arg(_gradesList.join(","));
    if (!ret.isEmpty())
        ret = tr("Kanji") + ret;
    else
        ret = tr("Kanji");
...
void Kanjidic2FilterWidget::onGradeTriggered(QAction *action) {
    ...
    if (!_gradesList.isEmpty())
        _gradeButton->setText(tr("Grade:") + _gradesList.join(","));
    else
        _gradeButton->setText(tr("Grade"));
    commandUpdate();
}
...
void Kanjidic2FilterWidget::setGrades(const QStringList &list) {
    ...
    foreach (QAction *action, _gradeButton->menu()->actions()) {
        if (action->isChecked())
            action->trigger();
        if (list.contains(action->property("Agrade").toString()))
            action->trigger();
    }
}
...
QString Kanjidic2FilterWidget::skip() const {
    return QString("%1-%2-%3")
        .arg(_skip1->value() ? QString::number(_skip1->value()) : "?")
        .arg(_skip2->value() ? QString::number(_skip2->value()) : "?")
        .arg(_skip3->value() ? QString::number(_skip3->value()) : "?");
}
...
void Kanjidic2FilterWidget::setSkip(const QString &value) {
    ...
    bool ok;
    int t, c1, c2;
    t = skipList[0].toInt(&ok);
    if (!ok && skipList[0] != "?")
        return;
    c1 = skipList[1].toInt(&ok);
    if (!ok && skipList[1] != "?")
        return;
    c2 = skipList[2].toInt(&ok);
    if (!ok && skipList[2] != "?")
        return;
    _skip1->setValue(t);
    _skip2->setValue(c1);
...
QString Kanjidic2FilterWidget::fourCorner() const {
    return QString("%1%2%3%4.%5")
        .arg(_fcTopLeft->currentIndex() > 0 ? QString::number(_fcTopLeft->currentIndex() - 1) : "?")
