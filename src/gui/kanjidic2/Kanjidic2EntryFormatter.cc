...
PreferenceItem<bool> Kanjidic2EntryFormatter::showDictionaries("kanjidic", "showDictionaries",
                                                               false);
PreferenceItem<bool> Kanjidic2EntryFormatter::showStrokesNumber("kanjidic", "showStrokesNumber",
                                                                true);
...
PreferenceItem<bool> Kanjidic2EntryFormatter::showOnlyStudiedVocab("kanjidic",
                                                                   "showOnlyStudiedVocab", false);
PreferenceItem<int> Kanjidic2EntryFormatter::maxCompoundsToDisplay("kanjidic",
                                                                   "maxParentKanjiToDisplay", 30);
PreferenceItem<bool> Kanjidic2EntryFormatter::showOnlyStudiedCompounds("kanjidic",
                                                                       "showOnlyStudiedComponents",
                                                                       false);

PreferenceItem<bool> Kanjidic2EntryFormatter::tooltipShowScore("kanjidic", "tooltipShowScore",
                                                               false);
PreferenceItem<bool> Kanjidic2EntryFormatter::tooltipShowUnicode("kanjidic", "tooltipShowUnicode",
                                                                 false);
PreferenceItem<bool> Kanjidic2EntryFormatter::tooltipShowSKIP("kanjidic", "tooltipShowSKIP", false);
PreferenceItem<bool> Kanjidic2EntryFormatter::tooltipShowFourCorner("kanjidic",
                                                                    "tooltipShowFourCorner", false);
PreferenceItem<bool> Kanjidic2EntryFormatter::tooltipShowHeisig("kanjidic", "tooltipShowHeisig",
                                                                false);
PreferenceItem<bool> Kanjidic2EntryFormatter::tooltipShowJLPT("kanjidic", "tooltipShowJLPT", false);
PreferenceItem<bool> Kanjidic2EntryFormatter::tooltipShowGrade("kanjidic", "tooltipShowGrade",
                                                               false);
PreferenceItem<bool> Kanjidic2EntryFormatter::tooltipShowStrokesNumber("kanjidic",
                                                                       "tooltipShowStrokesNumber",
                                                                       false);
PreferenceItem<bool> Kanjidic2EntryFormatter::tooltipShowFrequency("kanjidic",
                                                                   "tooltipShowFrequency", false);
...
PreferenceItem<bool>
    Kanjidic2EntryFormatter::printOnlyStudiedComponents("kanjidic", "printOnlyStudiedComponents",
                                                        false);
PreferenceItem<int> Kanjidic2EntryFormatter::maxWordsToPrint("kanjidic", "maxWordsToPrint", 5);
PreferenceItem<bool> Kanjidic2EntryFormatter::printOnlyStudiedVocab("kanjidic",
                                                                    "printOnlyStudiedVocab", false);
PreferenceItem<bool> Kanjidic2EntryFormatter::printStrokesNumbers("kanjidic", "printStrokesNumbers",
                                                                  false);
PreferenceItem<int> Kanjidic2EntryFormatter::strokesNumbersSize("kanjidic", "strokesNumbersSize",
                                                                6);
...
QString Kanjidic2EntryFormatter::getQueryUsedInWordsSql(int kanji, int limit, bool onlyStudied) {
    const QString queryUsedInWordsSql("select distinct " QUOTEMACRO(
        JMDICTENTRY_GLOBALID) ", jmdict.entries.id "
                              "from jmdict.entries "
                              "join jmdict.kanjiChar on jmdict.kanjiChar.id = "
                              "jmdict.entries.id "
                              "join jmdict.senses as senses on senses.id = "
                              "jmdict.entries.id "
                              "and senses.priority = 0 "
                              "and senses.misc0 & %4 = 0 "
                              "%3join training on training.id = "
                              "jmdict.entries.id and training.type "
                              "= " QUOTEMACRO(JMDICTENTRY_GLOBALID) " "
                                                                    "left join jmdict.jlpt "
                                                                    "on jmdict.entries.id "
                                                                    "= "
                                                                    "jmdict.jlpt.id "
                                                                    "where "
                                                                    "jmdict.kanjiChar."
    ...
    return queryUsedInWordsSql.arg(kanji)
        .arg(limit)
        .arg(onlyStudied ? "" : "left ")
        .arg(JMdictEntrySearcher::miscFilterMask()[0] |
             (1 << JMdictPlugin::miscMap()["uk"].second));
}

QString Kanjidic2EntryFormatter::getQueryUsedInKanjiSql(int kanji, int limit, bool onlyStudied) {
    const QString queryUsedInKanjiSql("select distinct " QUOTEMACRO(
        KANJIDIC2ENTRY_GLOBALID) ", ks1.kanji "
                                 "from kanjidic2.strokeGroups as ks1 "
                                 "join kanjidic2.entries on ks1.isRoot == 1 "
                                 "and "
                                 "ks1.kanji = entries.id "
                                 "%3join training on training.type "
                                 "= " QUOTEMACRO(KANJIDIC2ENTRY_GLOBALID) " and "
                                                                          "training.id "
                                                                          "= entries.id "
                                                                          "where "
                                                                          "(ks1.element = "
...
Kanjidic2EntryFormatter::Kanjidic2EntryFormatter(QObject *parent)
    : EntryFormatter("detailed_kanjidic2.css", "detailed_kanjidic2.html", parent) {}

void Kanjidic2EntryFormatter::draw(const ConstEntryPointer &entry, QPainter &painter,
                                   const QRectF &rectangle, QRectF &usedSpace,
                                   const QFont &textFont) const {
    drawCustom(entry.staticCast<const Kanjidic2Entry>(), painter, rectangle, usedSpace, textFont);
}

void Kanjidic2EntryFormatter::drawCustom(const ConstKanjidic2EntryPointer &entry, QPainter &painter,
                                         const QRectF &rectangle, QRectF &usedSpace,
                                         const QFont &textFont, int printSize, bool printWithFont,
                                         bool printMeanings, bool printOnyomi, bool printKunyomi,
                                         bool printComponents, bool printOnlyStudiedComponents,
                                         int maxWordsToPrint, bool printOnlyStudiedVocab,
    ...
    while (QFontMetrics(kanjiFont, painter.device()).boundingRect(entry->kanji()).width() >
           leftArea.width())
    ...
    if (printComponents) {
        ...
        foreach (const KanjiComponent *c, entry->rootComponents()) {
            ...
            if (printOnlyStudiedComponents && !original->trained())
                continue;
            QString s = original->kanji();
            if (original != element)
                s += tr(" (%1) ").arg(element->kanji());
            ...
            if (!printOnlyStudiedComponents && original->trained())
                painter.drawLine(
                    textBB.topLeft() + QPoint(0, metrics.ascent() + metrics.underlinePos()),
                    textBB.topRight() + QPoint(0, metrics.ascent() + metrics.underlinePos()));
            leftArea.setTop(textBB.bottom());
        }
    }
    ...
    QString str;
    rightArea.setTopLeft(
        QPointF(rectangle.left() + leftArea.width() + leftArea.width() / 20.0, rectangle.top()));
    // Print the meanings
    if (printMeanings) {
        ...
        if (!str.isEmpty())
            str[0] = str[0].toUpper();
        str = QFontMetrics(painter.font(), painter.device())
                  .elidedText(str, Qt::ElideRight, (int)rightArea.width());
    ...
    if (printOnyomi) {
        ...
        if (!onRead.isEmpty()) {
            ...
            str = " " + onRead.join(", ");
            str = QFontMetrics(painter.font(), painter.device())
                      .elidedText(str, Qt::ElideRight, (int)tempBox.width());
    ...
    if (maxWordsToPrint) {
        ...
        while (query.next()) {
            ...
            QString str = QFontMetrics(painter.font(), painter.device())
                              .elidedText(jmEntry->shortVersion(Entry::TinyVersion), Qt::ElideRight,
                                          (int)rightArea.width());
            textBB = painter.boundingRect(rightArea, Qt::AlignLeft, str);
            painter.drawText(textBB, Qt::AlignLeft, str);
            rightArea.setTop(textBB.bottom());
...
void Kanjidic2EntryFormatter::showToolTip(const ConstKanjidic2EntryPointer entry,
                                          const QPoint &pos) const {
    QString writing(DetailedViewFonts::HTML(DetailedViewFonts::KanjiHeader) + entry->kanji() +
                    "</span>"),
        alt;
    ...
    if (tooltipShowScore.value() && entry->trained()) {
        ...
        s += QString("<div background-color: #%1%2%3;\">%4</div> ")
                 .arg(QString::number(scoreColor.red(), 16))
                 .arg(QString::number(scoreColor.green(), 16))
                 .arg(QString::number(scoreColor.blue(), 16))
                 .arg(writing);
    } else
        s += writing;
    s += alt;
...
#define CLOSE_TAB                                                                                  \
    ...
    if (entry->grade() != -1 && tooltipShowGrade.value()) {
        QString body(tr("<b>Grade:</b> %1")
                         .arg(QCoreApplication::translate(
                             "Kanjidic2GUIPlugin",
                             Kanjidic2GUIPlugin::kanjiGrades[entry->grade()].toLatin1())));
        CLOSE_TAB;
    }
    ...
    if (tooltipShowUnicode.value()) {
        QString body(
            tr("<b>Unicode:</b> U+%1")
                .arg(
                    QString::number(TextTools::singleCharToUnicode(entry->kanji()), 16).toUpper()));
        CLOSE_TAB;
    }
...
ConstKanjidic2EntryPointer
Kanjidic2EntryFormatter::getShapeEntry(const KanjiComponent *comp) const {
    ConstKanjidic2EntryPointer component(KanjiEntryRef(comp->unicode()).get());
    return component;
}

ConstKanjidic2EntryPointer
Kanjidic2EntryFormatter::getMeaningEntry(const KanjiComponent *comp) const {
    ConstKanjidic2EntryPointer component;
    // If the component has an original, get the meaning from it!
    if (!comp->original().isEmpty())
        component = KanjiEntryRef(TextTools::singleCharToUnicode(comp->original())).get();
    else
        component = KanjiEntryRef(comp->unicode()).get();
    return component;
}
...
QString Kanjidic2EntryFormatter::shortDesc(const ConstKanjidic2EntryPointer &shape,
                                           const ConstKanjidic2EntryPointer &entry) const {
    QString ret;

    QString str(entry->kanji());
    if (shape != entry)
        str += QString(" (%1) ").arg(shape->kanji());
    if (!entry->meanings().isEmpty())
        str += ": " + entry->meaningsString();
    ret += autoFormat(str);
    ...
    ret += QString(" <a href=\"entry://?type=%1&id=%2\"><img src=\"moreicon\"/></a>")
               .arg(entry->type())
               .arg(entry->id());
    if (entry->trained()) {
        ret = QString("<span style=\"background-color:%1\">%2</span>")
                  .arg(colorTriplet(EntryFormatter::scoreColor(*entry)))
                  .arg(ret);
    }
    return ret;
}
...
QString Kanjidic2EntryFormatter::formatGrade(const ConstEntryPointer &_entry) const {
    ...
    if (entry->grade() != -1 && showGrade.value()) {
        return QString("<b>%1:</b> %2")
            .arg(tr("Grade"))
            .arg(QCoreApplication::translate(
                "Kanjidic2GUIPlugin", Kanjidic2GUIPlugin::kanjiGrades[entry->grade()].toLatin1()));
    }
    return "";
}
...
QString Kanjidic2EntryFormatter::formatVariations(const ConstEntryPointer &_entry) const {
    if (showVariations.value()) {
        ConstKanjidic2EntryPointer entry(_entry.staticCast<const Kanjidic2Entry>());
        SQLite::Query query(Database::connection());
        query.prepare("select distinct element from kanjidic2.strokeGroups where "
                      "original = ?");
...
QString Kanjidic2EntryFormatter::formatUnicode(const ConstEntryPointer &_entry) const {
    ...
    if (showUnicode.value()) {
        return QString("<b>%1:</b> U+%2")
            .arg(tr("Unicode"))
            .arg(QString::number(TextTools::singleCharToUnicode(entry->kanji()), 16).toUpper());
    }
    return "";
}
...
QString Kanjidic2EntryFormatter::formatComponents(const ConstEntryPointer &_entry) const {
    ...
    if (!components.isEmpty() && showComponents.value()) {
        ...
        foreach (const KanjiComponent *component, components) {
            if (listedComps.contains(component->unicode()))
                continue;
...
QString Kanjidic2EntryFormatter::formatDictionaries(const ConstEntryPointer &_entry) const {
    ...
    if (!dictionaries.isEmpty() && showDictionaries.value()) {
        QString table = QString("<table width=\"100%\" class=\"subinfo\"><tr><td "
...
QList<DetailedViewJob *> Kanjidic2EntryFormatter::jobUsedInKanji(const ConstEntryPointer &_entry,
                                                                 const QTextCursor &cursor) const {
    ConstKanjidic2EntryPointer entry(_entry.staticCast<const Kanjidic2Entry>());
    QList<DetailedViewJob *> ret;
    if (maxCompoundsToDisplay.value())
        ret << new ShowUsedInKanjiJob(entry->kanji(), cursor);
    return ret;
}

QList<DetailedViewJob *> Kanjidic2EntryFormatter::jobUsedInWords(const ConstEntryPointer &_entry,
                                                                 const QTextCursor &cursor) const {
    ConstKanjidic2EntryPointer entry(_entry.staticCast<const Kanjidic2Entry>());
    QList<DetailedViewJob *> ret;
    if (maxWordsToDisplay.value())
        ret << new ShowUsedInWordsJob(entry->kanji(), cursor);
    return ret;
}

ShowUsedInKanjiJob::ShowUsedInKanjiJob(const QString &kanji, const QTextCursor &cursor)
    : DetailedViewJob(Kanjidic2EntryFormatter::getQueryUsedInKanjiSql(
                          TextTools::singleCharToUnicode(kanji),
                          Kanjidic2EntryFormatter::maxCompoundsToDisplay.value(),
                          Kanjidic2EntryFormatter::showOnlyStudiedCompounds.value()),
                      cursor),
      _kanji(kanji), gotResults(false) {}

void ShowUsedInKanjiJob::firstResult() {
    cursor().insertHtml(
        QString("<br/>%1").arg(EntryFormatter::buildSubInfoLine(tr("Direct compounds"), "")));
...
void ShowUsedInKanjiJob::completed() {
    if (!gotResults)
        return;
    cursor().insertHtml(QString("<a href=\"component:?reset=true&kanji=%1\" title=\"%4\">%2</a> "
                                "<a href=\"component:?kanji=%1\" title=\"%5\">%3</a>")
                            .arg(_kanji)
                            .arg(tr("All compounds"))
                            .arg(tr("(+)"))
                            .arg(tr("Make a new search using only this filter"))
                            .arg(tr("Add this filter to the current search")));
}

ShowUsedInWordsJob::ShowUsedInWordsJob(const QString &kanji, const QTextCursor &cursor)
    : DetailedViewJob(Kanjidic2EntryFormatter::getQueryUsedInWordsSql(
                          TextTools::singleCharToUnicode(kanji),
                          Kanjidic2EntryFormatter::maxWordsToDisplay.value(),
                          Kanjidic2EntryFormatter::showOnlyStudiedVocab.value()),
                      cursor),
      _kanji(kanji), gotResults(false) {}

void ShowUsedInWordsJob::firstResult() {
    cursor().insertHtml(
        QString("<br/>%1").arg(EntryFormatter::buildSubInfoBlock(tr("Seen in"), "")));
    _cursor.movePosition(QTextCursor::PreviousBlock);
    gotResults = true;
}
...
void ShowUsedInWordsJob::completed() {
    if (!gotResults)
        return;
    cursor().insertHtml(QString("<a href=\"allwords:?reset=true&kanji=%1\" title=\"%4\">%2</a> "
                                "<a href=\"allwords:?kanji=%1\" title=\"%5\">%3</a>")
                            .arg(_kanji)
                            .arg(tr("All words using this kanji"))
                            .arg(tr("(+)"))
                            .arg(tr("Make a new search using only this filter"))
                            .arg(tr("Add this filter to the current search")));
}
