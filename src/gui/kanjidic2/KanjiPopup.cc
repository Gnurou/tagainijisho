...
void KanjiComponentWidget::setComponent(const KanjiComponent *component) {
    ...
    if (component)
        _kanji = KanjiEntryRef(component->element()).get();
    else
        _kanji.clear();
    update();
}
...
void KanjiComponentWidget::paintEvent(QPaintEvent *event) {
    ...
    if (!_component->original().isEmpty())
        kEntry = KanjiEntryRef(_component->original()).get();
    ...
    if (!_component->original().isEmpty()) {
        ...
        if (elt) {
            ...
            if (!meaningsList.isEmpty() && meaningsList[0] != meanings)
                meanings = tr("%1 (drawn as \"%2\")").arg(meanings).arg(meaningsList[0]);
        }
    }

    QRect textRect(QPoint(kanjiSize + 5, 0), size() - QSize(kanjiSize + 5, 0));
    readings =
        fontMetrics().elidedText(kEntry->readings().join(", "), Qt::ElideRight, textRect.width());
    meanings = fontMetrics().elidedText(meanings, Qt::ElideRight, textRect.width());
    if (!meanings.isEmpty())
        meanings[0] = meanings[0].toUpper();
...
KanjiPopup::KanjiPopup(QWidget *parent)
    : QFrame(parent), _history(historySize.value()), entryView() {
    setupUi(this);
    ...
    connect(stroke, SIGNAL(componentHighlighted(const KanjiComponent *)), this,
            SLOT(onComponentHighlighted(const KanjiComponent *)));
    connect(stroke, SIGNAL(componentUnHighlighted(const KanjiComponent *)), this,
            SLOT(onComponentUnHighlighted()));
    connect(stroke, SIGNAL(componentClicked(const KanjiComponent *)), this,
            SLOT(onComponentClicked(const KanjiComponent *)));
    connect(componentsLabel, SIGNAL(linkHovered(QString)), this,
            SLOT(onComponentLinkHovered(QString)));
    connect(componentsLabel, SIGNAL(linkActivated(QString)), this,
            SLOT(onComponentLinkActivated(QString)));
...
void KanjiPopup::showKanji(const Kanjidic2EntryPointer &entry) {
    ...
    if (entry->meanings().isEmpty())
        str = tr("No information about this kanji!");
    else {
    ...
    meaningsLabel->setText(meaningsLabel->fontMetrics().elidedText(str, Qt::ElideRight,
                                                                   meaningsLabel->size().width()));
    ...
    if (!onReadings.isEmpty())
        readingsParts << tr("<b>On:</b> %1")
                             .arg(readingsLabel->fontMetrics().elidedText(
                                 onReadings.join(", "), Qt::ElideRight,
                                 readingsLabel->size().width()));
    if (!kunReadings.isEmpty())
        readingsParts << tr("<b>Kun:</b> %1")
                             .arg(readingsLabel->fontMetrics().elidedText(
                                 kunReadings.join(", "), Qt::ElideRight,
                                 readingsLabel->size().width()));
    ...
    if (autoStartAnim.value())
        stroke->play();
    else
        stroke->setPosition(entry->strokeCount());
}
...
void KanjiPopup::setComponentsLabelText(int highlightPos) {
    ...
    foreach (const KanjiComponent *component, entry->rootComponents()) {
        componentsStrings << QString("<a %3href=\"%1|%2\">%2</a>")
                                 .arg(i)
                                 .arg(component->original().isEmpty() ? component->element()
                                                                      : component->original())
                                 .arg(highlightPos == i ? "class=\"highlighted\" " : "");
        if (!component->original().isEmpty())
            componentsStrings.last() += QString("(<a %3href=\"%1|%2\">%2</a>)")
                                            .arg(i)
                                            .arg(component->element())
                                            .arg(highlightPos == i ? "class=\"highlighted\" " : "");
        ++i;
    }
    ...
    QString cString;
    if (!componentsStrings.isEmpty())
        cString = QString("<style>a.highlighted { background-color: %1; color: "
                          "%2; }</style>")
                      .arg(palette.color(QPalette::Highlight).name())
                      .arg(palette.color(QPalette::HighlightedText).name()) +
                  tr("<b>Components:</b> %2 %1")
                      .arg(componentsStrings.join(" "))
                      .arg(componentsStrings.size() > 3 ? "<br/>" : "");
    ...
    if (!rads.isEmpty()) {
        ...
        foreach (const radType &rad, rads)
            strl << QString("%1 (%2)")
                        .arg(TextTools::unicodeToSingleChar(rad.first))
                        .arg(rad.second);
...
void KanjiPopup::onComponentHighlighted(const KanjiComponent *component) {
    ...
    int pos = -1;
    for (int i = 0; i < entry->rootComponents().size(); ++i)
        if (component == entry->rootComponents()[i]) {
            pos = i;
            break;
        }
...
void KanjiPopup::onComponentClicked(const KanjiComponent *component) {
    Kanjidic2EntryPointer kElement(
        KanjiEntryRef(TextTools::singleCharToUnicode(component->element())).get());
    if (!kElement)
        return;
