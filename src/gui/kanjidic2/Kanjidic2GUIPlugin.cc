...
PreferenceItem<bool> Kanjidic2GUIPlugin::kanjiTooltipEnabled("kanjidic", "kanjiTooltipEnabled",
                                                             true);

Kanjidic2GUIPlugin::Kanjidic2GUIPlugin()
    : Plugin("kanjidic2GUI"), _flashKL(0), _flashKS(0), _flashML(0), _flashMS(0),
      _readingPractice(0), _showKanjiPopup(0), _linkHandler(0), _wordsLinkHandler(0),
      _componentsLinkHandler(0), _filter(0), _trainer(0), _readingTrainer(0), _cAction(0),
      _kAction(0), _dragStarted(false), _dragEntryRef(0) {
    _instance = this;
}
...
bool Kanjidic2GUIPlugin::onRegister() {
    ...
    if (!Plugin::pluginExists("kanjidic2"))
        return false;
    // Register the formatter
    if (!EntryFormatter::registerFormatter(KANJIDIC2ENTRY_GLOBALID,
                                           &Kanjidic2EntryFormatter::instance()))
        return false;
    // Register the link handlers
    _linkHandler = new KanjiLinkHandler();
    if (!DetailedViewLinkManager::registerHandler(_linkHandler))
        return false;
    _wordsLinkHandler = new KanjiAllWordsHandler();
    if (!DetailedViewLinkManager::registerHandler(_wordsLinkHandler))
        return false;
    _componentsLinkHandler = new KanjiAllComponentsOfHandler();
    if (!DetailedViewLinkManager::registerHandler(_componentsLinkHandler))
        return false;

    // Add the main window entries
    MainWindow *mainWindow(MainWindow::instance());
    QMenu *menu = mainWindow->trainMenu();
    QMenu *menu2 = menu->addMenu(tr("Character flashcards"));
    _flashKL = menu2->addAction(tr("From &character, whole study list"));
    _flashKS = menu2->addAction(tr("From &character, current set"));
    ...
    connect(_kanaSelector->kanaView(), SIGNAL(entrySelected(EntryPointer)),
            mainWindow->detailedView(), SLOT(display(EntryPointer)));
    // Toggle action
    ...
    _kAction =
        new KanjiInputPopupAction(new KanjiInputter(new RadicalKanjiSelector(), false, mainWindow),
                                  tr("Radical search input"), mainWindow);
    _kAction->setShortcut(QKeySequence("Ctrl+k"));
    mainWindow->searchMenu()->addAction(_kAction);
    _cAction =
        new KanjiInputPopupAction(new KanjiInputter(new ComponentKanjiSelector(), true, mainWindow),
                                  tr("Component search input"), mainWindow);
...
bool Kanjidic2GUIPlugin::onUnregister() {
    ...
    delete _wordsLinkHandler;
    _wordsLinkHandler = 0;
    DetailedViewLinkManager::removeHandler(_componentsLinkHandler);
    delete _componentsLinkHandler;
    _componentsLinkHandler = 0;
...
void Kanjidic2GUIPlugin::training(YesNoTrainer::TrainingMode mode, const QString &queryString) {
    ...
    if (_trainer && (_trainer->trainingMode() != mode || _trainer->query() != queryString))
        delete _trainer;
    if (!_trainer) {
        restart = true;
...
void Kanjidic2GUIPlugin::trainingKanjiSet() {
    ...
    const QueryBuilder::Statement *stat(
        qBuilder->getStatementForEntryType(KANJIDIC2ENTRY_GLOBALID));
    if (!stat) {
        QMessageBox::information(MainWindow::instance(), tr("Nothing to train"),
                                 tr("There are no kanji entries in this set to train on."));
        return;
    }
...
void Kanjidic2GUIPlugin::trainerDeleted() { _trainer = 0; }
...
bool Kanjidic2GUIPlugin::eventFilter(QObject *obj, QEvent *_event) {
    ...
    case QEvent::MouseMove: {
        ...
        if (pos != -1) {
            ...
            if (!cursor.charFormat().isAnchor()) {
                ...
                if (TextTools::isKanjiChar(c)) {
                    ConstKanjidic2EntryPointer entry(
                        KanjiEntryRef(TextTools::singleCharToUnicode(c)).get());
                    // If kanji are clickable, change the cursor
                    if (view->kanjiClickable()) {
                        view->viewport()->setCursor(QCursor(Qt::PointingHandCursor));
                        // Only show the tooltip if the entry exists in the
                        // database!
                        if (kanjiTooltipEnabled.value() && entry) {
                            const Kanjidic2EntryFormatter *formatter(
                                static_cast<const Kanjidic2EntryFormatter *>(
                                    EntryFormatter::getFormatter(KANJIDIC2ENTRY_GLOBALID)));
    ...
    case QEvent::MouseButtonRelease: {
        ...
        if (e->button() == Qt::LeftButton && _dragStarted) {
            ...
            if (view->kanjiClickable())
                view->fakeClick(QUrl(QString("drawkanji:?kanji=%1").arg(_dragEntryRef.kanji())));
            return true;
        }
        return false;
    }
    case QEvent::ContextMenu: {
        ...
        if (tview.entry()) {
            menu = new QMenu();
            openAction =
                menu->addAction(QIcon("images/icons/zoom-in.png"), tr("Open in detailed view..."));
            menu->addSeparator();
        ...
        if (selected && selected == openAction)
            MainWindow::instance()->detailedView()->display(tview.entry());
        delete menu;
        return true;
    }
    default:
...
void Kanjidic2GUIPlugin::showPopup(const Kanjidic2EntryPointer &entry, const QPoint &pos) {
    ...
    QObject::connect(popup, SIGNAL(requestDisplay(EntryPointer)),
                     MainWindow::instance()->detailedView(), SLOT(display(EntryPointer)));
    ...
    popup->show();
    // Need to display a second time once the window is visible to get the
...
void KanjiAllWordsHandler::handleUrl(const QUrl &url, DetailedView *view) {
    MainWindow *mainWindow = MainWindow::instance();
    EntryTypeFilterWidget *entryType = qobject_cast<EntryTypeFilterWidget *>(
        mainWindow->searchWidget()->getSearchFilter("entrytypeselector"));
    JMdictFilterWidget *extender = qobject_cast<JMdictFilterWidget *>(
        mainWindow->searchWidget()->getSearchFilter("jmdictoptions"));
    if (!entryType || !extender)
        return;
...
void KanjiAllComponentsOfHandler::handleUrl(const QUrl &url, DetailedView *view) {
    ...
    EntryTypeFilterWidget *entryType = qobject_cast<EntryTypeFilterWidget *>(
        mainWindow->searchWidget()->getSearchFilter("entrytypeselector"));
    Kanjidic2FilterWidget *extender = qobject_cast<Kanjidic2FilterWidget *>(
        mainWindow->searchWidget()->getSearchFilter("kanjidicoptions"));
    if (!entryType || !extender)
        return;
