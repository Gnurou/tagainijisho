...
class Kanjidic2EntryFormatter : public EntryFormatter {
  ...
  protected:
    ...
    /**
     * Some components have the shape of a kanji but are actually variations of
     * another one. This function returns the right shape for a component
     * according to the user's settings.
     */
    ConstKanjidic2EntryPointer getShapeEntry(const KanjiComponent *comp) const;
    /**
     * Some components have the shape of a kanji but are actually variations of
     * another one. This function returns the right meaning for a component
     * according to the user's settings.
     */
  ...
  public:
     ...
     */
    virtual QString shortDesc(const ConstKanjidic2EntryPointer &shape,
    ...
    static QString getQueryUsedInWordsSql(int kanji, int limit = maxWordsToDisplay.value(),
                                          bool onlyStudied = showOnlyStudiedVocab.value());
    static QString getQueryUsedInKanjiSql(int kanji, int limit = maxCompoundsToDisplay.value(),
                                          bool onlyStudied = showOnlyStudiedCompounds.value());
    ...
    void drawCustom(const ConstKanjidic2EntryPointer &entry, QPainter &painter,
                    const QRectF &rectangle, QRectF &usedSpace, const QFont &textFont = QFont(),
                    int _printSize = printSize.value(), bool _printWithFont = printWithFont.value(),
                    bool _printMeanings = printMeanings.value(),
                    bool _printOnyomi = printOnyomi.value(),
                    bool _printKunyomi = printKunyomi.value(),
                    bool _printComponents = printComponents.value(),
                    bool _printOnlyStudiedComponents = printOnlyStudiedComponents.value(),
                    int _maxWordsToPrint = maxWordsToPrint.value(),
                    bool _printOnlyStudiedVocab = printOnlyStudiedVocab.value(),
                    bool _printStrokesNumbers = printStrokesNumbers.value(),
                    int _printStrokesNumbersSize = strokesNumbersSize.value(),
                    bool _printGrid = printGrid.value()) const;
  ...
  public slots:
    ...
    virtual QList<DetailedViewJob *> jobUsedInKanji(const ConstEntryPointer &_entry,
                                                    const QTextCursor &cursor) const;
    virtual QList<DetailedViewJob *> jobUsedInWords(const ConstEntryPointer &_entry,
                                                    const QTextCursor &cursor) const;
};
