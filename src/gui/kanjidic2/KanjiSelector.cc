...
ComplementsList::ComplementsList(QWidget *parent)
    : QListWidget(parent), baseFont(font()), labelFont(baseFont), _sscroll(verticalScrollBar()) {
    ...
    connect(this, SIGNAL(itemEntered(QListWidgetItem *)), this,
            SLOT(onItemEntered(QListWidgetItem *)));
}
...
void ComplementsList::onItemEntered(QListWidgetItem *item) {
    ...
    // Kanjidic2EntryFormatter::instance().showToolTip(KanjiEntryRef(item->text()).get(),
    // QCursor::pos());
}

KanjiSelector::KanjiSelector(QWidget *parent)
    : QFrame(parent), _associate(0), _outOfSyncWithAssociate(false),
      _ignoreAssociateSignals(false) {
    setupUi(this);
    connect(_complementsList, SIGNAL(itemSelectionChanged()), this, SLOT(onSelectionChanged()));
    resize(complementsList()->gridSize().width() * 10 +
               complementsList()->verticalScrollBar()->width(),
           complementsList()->gridSize().height() * 7);
}
...
void KanjiSelector::associateTo(QLineEdit *associate) {
    if (_associate) {
        ...
        disconnect(this, SIGNAL(selectionChanged(QSet<uint>)), this,
                   SLOT(updateAssociateFromSelection(QSet<uint>)));
    }
    _associate = associate;
    if (!_associate)
        return;
    connect(_associate, SIGNAL(textChanged(QString)), this, SLOT(onAssociateChanged()));
    connect(this, SIGNAL(selectionChanged(QSet<uint>)), this,
            SLOT(updateAssociateFromSelection(QSet<uint>)));
}
...
void KanjiSelector::updateAssociateFromSelection(QSet<uint> selection) {
    ...
    foreach (uint comp, currentComps)
        if (!selComps.contains(comp))
            compText.remove(TextTools::unicodeToSingleChar(comp));
    QStringList addComps;
    foreach (uint comp, selComps) {
        QString compRepr(TextTools::unicodeToSingleChar(comp));
        if (!compText.contains(compRepr))
            compText += compRepr;
    }
...
QSet<uint> KanjiSelector::getCandidates(const QSet<uint> &selection) {
    ...
    emit startQuery();
    QSet<uint> realSel;
    foreach (uint kanji, selection)
        realSel << complementCode(TextTools::unicodeToSingleChar(kanji));
    QString resQuery(getCandidatesQuery(realSel));
    if (!resQuery.isEmpty()) {
        SQLite::Query query(Database::connection());
        if (!query.exec(resQuery))
            qDebug() << query.lastError().message();
        while (query.next()) {
...
void KanjiSelector::updateComplementsList(const QSet<uint> &selection,
                                          const QSet<uint> &candidates) {
    ...
    if (!compQuery.isEmpty()) {
        ...
        while (query.next()) {
            ...
            if (curKanji == kanji)
                continue;
            curKanji = kanji;
            // Do not display kanji that are already in candidates, excepted if
            // they are part of the current selection
            if (candidates.contains(kanji) && !selection.contains(kanji))
                continue;
...
KanjiSelectorValidator::KanjiSelectorValidator(KanjiSelector *filter, QObject *parent)
    : QValidator(parent), _filter(filter) {}
...
QValidator::State KanjiSelectorValidator::validate(QString &input, int &pos) const {
    ...
    foreach (const compType &comp, complements)
        validChars << comp.second;
    int i = 0;
    while (i < input.size()) {
        ...
        if (isSurrogate && i == input.size() - 1)
            isSurrogate = 0;
        if (!isSurrogate)
            isValid = validChars.contains(c);
        else
            isValid = validChars.contains(QString(c) + input[i + 1]);
        if (!isValid) {
...
QString RadicalKanjiSelector::getCandidatesQuery(const QSet<uint> &selection) const {
    ...
    foreach (uint sel, selection)
        select << QString::number(sel);

    return QString("select r1.kanji from kanjidic2.radicals as r1 join "
                   "kanjidic2.entries as e on r1.kanji = e.id where r1.number in "
                   "(%1) and r1.type is not null group by r1.kanji having "
                   "uniquecount(r1.number) >= %2 order by e.strokeCount, "
                   "e.frequency, e.id")
        .arg(select.join(", "))
        .arg(select.size());
}

QString RadicalKanjiSelector::getComplementsQuery(const QSet<uint> &selection,
                                                  const QSet<uint> &candidates) const {
    if (candidates.isEmpty())
        return "select entries.id, strokeCount from kanjidic2.radicalsList "
               "join "
               "kanjidic2.entries on radicalsList.kanji = entries.id order by "
               "strokeCount, number, radicalsList.rowid";
    else {
        QString selString;
        foreach (uint candidate, candidates) {
            if (selString.isEmpty())
                selString = QString::number(candidate);
            else
                selString += "," + QString::number(candidate);
        }
        return QString("select distinct e.id, strokeCount from kanjidic2.radicals "
                       "as r join kanjidic2.radicalsList as rl on r.number = "
                       "rl.number join kanjidic2.entries as e on rl.kanji = e.id "
                       "where r.kanji in (%1) and r.type is not null order by "
                       "strokeCount, rl.number, rl.rowid")
            .arg(selString);
    }
}
...
QString ComponentKanjiSelector::getCandidatesQuery(const QSet<uint> &selection) const {
    ...
    return QString("select ks1.kanji from kanjidic2.strokeGroups as ks1 left join "
                   "kanjidic2.entries as e on ks1.kanji = e.id where (ks1.element "
                   "in "
                   "(%1) or ks1.original in (%1)) group by ks1.kanji having "
                   "uniquecount(CASE WHEN ks1.element IN (%1) THEN ks1.element "
                   "ELSE "
                   "NULL END, CASE WHEN ks1.original IN (%1) THEN ks1.original "
                   "ELSE "
                   "NULL END) >= %2 order by strokeCount")
        .arg(select.join(", "))
        .arg(select.size());
}

QString ComponentKanjiSelector::getComplementsQuery(const QSet<uint> &selection,
                                                    const QSet<uint> &candidates) const {
    if (selection.isEmpty() && candidates.isEmpty())
        return "select distinct kanji, strokeCount from "
               "kanjidic2.rootComponents "
               "as rc join kanjidic2.entries as e on rc.kanji = e.id order by "
               "strokeCount";
    // Selection but no candidates - just get the selection
        ...
        foreach (uint sel, selection) {
            if (selString.isEmpty())
                selString = QString::number(sel);
            else
                selString += "," + QString::number(sel);
        }
        return QString("select distinct id, strokeCount from kanjidic2.entries "
                       "where id in (%1)")
            .arg(selString);
    } else {
        QString selString;
        ...
        return QString("select distinct ks2.element, strokeCount from "
                       "kanjidic2.strokeGroups as ks join kanjidic2.strokeGroups "
                       "as ks2 on ks.kanji = ks2.kanji left join kanjidic2.entries "
                       "as e on ks2.element = e.id where ks.kanji in (%1) order by "
                       "strokeCount")
            .arg(selString);
    }
}

KanjiInputter::KanjiInputter(KanjiSelector *selector, bool useLineEdit, QWidget *parent)
    : QFrame(parent), _selector(selector) {
    ...
    resize(_selector->complementsList()->gridSize().width() * 10 +
               _selector->complementsList()->verticalScrollBar()->width(),
           _selector->complementsList()->gridSize().height() * 7 + _results->sizeHint().height());
}
...
KanjiInputPopupAction::KanjiInputPopupAction(KanjiInputter *popup, const QString &title,
                                             QWidget *parent)
    : QAction(title, parent), _popup(popup), focusWidget(0) {
    ...
    connect(_popup, SIGNAL(kanjiSelected(QString)), this,
            SLOT(onComponentSearchKanjiSelected(QString)));
}
...
void KanjiInputPopupAction::togglePopup(bool status) {
    if (status) {
        ...
        if (lEdit || (cBox && cBox->isEditable())) {
            focusWidget = fWidget;
            _popup->move(focusWidget->mapToGlobal(
                QPoint(focusWidget->rect().left() +
                           (focusWidget->rect().width() - _popup->rect().width()) / 2,
                       focusWidget->rect().bottom())));
            _popup->reset();
            _popup->show();
...
void KanjiInputPopupAction::onComponentSearchKanjiSelected(const QString &kString) {
    if (focusWidget) {
        ...
        if (lEdit)
            target = lEdit;
        else if (cBox && cBox->isEditable())
            target = cBox->lineEdit();
        if (target)
            target->insert(TextTools::unicodeToSingleChar(kanji));
    }
    _popup->hide();
}
