...
KanjiRenderer::Stroke::Stroke(const KanjiStroke *const stroke)
    : _stroke(stroke), _painterPath(pathFromSVG(stroke->path())) {}
...
KanjiRenderer::KanjiRenderer(ConstKanjidic2EntryPointer kanji) { setKanji(kanji); }
...
void KanjiRenderer::setKanji(ConstKanjidic2EntryPointer kanji) {
    ...
    if (TextTools::isKana(kanji->kanji())) {
        ...
        for (QList<Stroke>::iterator stroke = _strokes.begin(); stroke != _strokes.end();
             ++stroke) {
...
void KanjiRenderer::renderStrokeNumber(const KanjiStroke &stroke, QPainter *painter,
                                       qreal baseSize) {
    QPalette palette;
    ...
    QRectF elipseRect(line.p2() - QPointF(baseSize, baseSize),
                      QSizeF((baseSize * 2), (baseSize * 2)));
...
const KanjiRenderer::Stroke *KanjiRenderer::strokeFor(const KanjiStroke &stroke) const {
    if (!_strokesMap.contains(&stroke))
        return 0;
    else
        return _strokesMap[&stroke];
}
