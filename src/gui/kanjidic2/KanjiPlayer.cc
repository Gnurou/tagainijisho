...
static QList<QColor> colList(QList<QColor>()
                             << Qt::black << QColor(0x0d, 0x5b, 0xa6) << QColor(0xce, 0x34, 0x34)
                             << QColor(0x04, 0x9a, 0x40) << QColor(0xe6, 0xa6, 0x00)
                             << QColor(0xd2, 0x7d, 0x8e) << Qt::blue << Qt::red << Qt::green
                             << Qt::cyan << Qt::magenta << Qt::yellow);

KanjiPlayer::KanjiPlayer(QWidget *parent)
    : QWidget(parent), _timer(), _kanji(0), renderer(), _picture(), _state(STATE_STROKE),
      _showGrid(showGridPref.value()), _showStrokesNumbers(showStrokesNumbersPref.value()),
      _strokesNumbersSize(strokesNumbersSizePref.value()), _highlightedComponent(0) {
...
void KanjiPlayer::updateActionsState() {
    ...
    if (_timer.isActive()) {
        playButton->removeAction(playAction());
        playButton->setDefaultAction(pauseAction());
    } else {
        playButton->removeAction(pauseAction());
        playButton->setDefaultAction(playAction());
    }
...
void KanjiPlayer::highlightComponent(const KanjiComponent *component) {
    if (component == _highlightedComponent)
        return;
    _highlightedComponent = component;
    update();
}

void KanjiPlayer::unHighlightComponent() { highlightComponent(0); }
...
void KanjiPlayer::renderCurrentState() {
    ...
    for (int i = 0; i < _strokesCpt; i++) {
        ...
        if (highlightedComponent() && highlightedComponent()->strokes().contains(&kStrokes[i]))
            ...
            foreach (const KanjiComponent *comp, kComponents) {
    ...
    if (_state == STATE_STROKE && _strokesCpt < renderer.strokes().size()) {
        ...
        const KanjiComponent *parent(0);
        if (highlightedComponent() &&
            highlightedComponent()->strokes().contains(currentStroke.stroke()))
            parent = highlightedComponent();
        else
            foreach (const KanjiComponent *comp, kComponents) {
    ...
    if (showStrokesNumbers()) {
        int strokesMax =
            _strokesCpt +
            (_state == STATE_STROKE && _strokesCpt < renderer.strokes().size() ? 1 : 0);
...
bool KanjiPlayer::eventFilter(QObject *obj, QEvent *event) {
    ...
    if (event->type() == QEvent::MouseMove) {
        ...
        QPointF pos(mEvent->position());
        QPointF fPos(pos.x() * (KANJI_AREA_WIDTH / pictureSize()),
                     pos.y() * (KANJI_AREA_HEIGHT / pictureSize()));
        ...
        foreach (const KanjiRenderer::Stroke &stroke, strokes) {
            ...
            if (nPath.contains(fPos)) {
                foreach (const KanjiComponent *component, kComponents)
                    if (component->strokes().contains(stroke.stroke())) {
                        comp = component;
                        break;
                    }
                if (comp)
...
void KanjiPlayer::setPosition(int strokeNbr) {
    if (strokeNbr > renderer.strokes().size())
        strokeNbr = renderer.strokes().size();
...
void KanjiPlayer::updateStrokesCountLabel() {
    strokeCountLabel->setText(
        QString("%1/%2").arg(_strokesCpt + (_lengthCpt ? 1 : 0)).arg(renderer.strokes().size()));
}
