...
PreferenceItem<QString> KanaView::characterFont("kanjidic2/kanaSelector", "characterFont",
                                                QFont("Helvetica", 15).toString());

KanaModel::KanaModel(QObject *parent)
    : QAbstractTableModel(parent), _showObsolete(false), _mode(Hiragana),
      _kanaTable(&TextTools::hiraganaTable) {
    _font.fromString(KanaView::characterFont.value());
}

int KanaModel::rowCount(const QModelIndex &parent) const { return KANASTABLE_NBROWS; }

int KanaModel::columnCount(const QModelIndex &parent) const { return KANASTABLE_NBCOLS; }
...
void KanaModel::setMode(Mode newMode) {
    ...
    if (_mode == Hiragana)
        ...
        _kanaTable = &TextTools::katakanaTable;
    emit layoutChanged();
}
...
QVariant KanaModel::data(const QModelIndex &index, int role) const {
    ...
    if (c.unicode() == 0 || (!showObsolete() && info.usage == TextTools::KanaInfo::Rare))
        return QVariant();
...
static QStringList vLabels(QStringList()
                           << "" << "K" << "G" << "S" << "Z" << "T" << "D" << "N" << "H" << "B"
                           << "P" << "M" << "Y" << "R" << "W" << "V" << "N");
...
KanaView::KanaView(QWidget *parent, bool viewOnly)
    : QTableView(parent), _helper(this, 0, true, viewOnly) {
    setModel(&_model);
    ...
    for (int i = 0; i < _model.rowCount(); ++i) {
        ...
        for (int j = 0; j < _model.columnCount(); ++j) {
            ...
            if (c.unicode() == 0)
                continue;
            TextTools::KanaInfo info(TextTools::kanaInfo(c));
            if (!showObsolete() && info.usage == TextTools::KanaInfo::Rare)
                continue;
            empty = false;
        }
