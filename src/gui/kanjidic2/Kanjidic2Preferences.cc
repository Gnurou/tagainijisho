...
Kanjidic2Preferences::Kanjidic2Preferences(QWidget *parent)
    : PreferencesWindowCategory(tr("Character entries"), parent) {
    setupUi(this);
    ...
    connect(printComponents, SIGNAL(toggled(bool)), printOnlyStudiedComponents,
    ...
    connect(animationLoopDelay, SIGNAL(valueChanged(int)), _player,
            SLOT(setAnimationLoopDelay(int)));
    connect(showStrokesNumbersButton, SIGNAL(toggled(bool)), _player,
            SLOT(setShowStrokesNumbers(bool)));
    connect(showStrokesNumbersSize, SIGNAL(valueChanged(int)), _player,
            SLOT(setStrokesNumbersSize(int)));
    ...
    connect(_kanaSelectorFont, SIGNAL(fontChanged(QFont)), _kanaSelectorPreview,
            SLOT(setCharacterFont(QFont)));
}
...
void Kanjidic2Preferences::refresh() {
    ...
    printOnlyStudiedComponents->setChecked(
        Kanjidic2EntryFormatter::printOnlyStudiedComponents.value());
    ...
    if (Kanjidic2EntryFormatter::printWithFont.value())
        fontButton->setChecked(true);
    else
        handWritingButton->setChecked(true);
...
void Kanjidic2Preferences::applySettings() {
    ...
    Kanjidic2EntryFormatter::printOnlyStudiedComponents.set(
        printOnlyStudiedComponents->isChecked());
    ...
    if (animSpeedDefault->isChecked())
        KanjiPlayer::animationSpeed.reset();
    else
        KanjiPlayer::animationSpeed.set(animSpeedSlider->value());
    if (animDelayDefault->isChecked())
        KanjiPlayer::delayBetweenStrokes.reset();
    else
        KanjiPlayer::delayBetweenStrokes.set(animDelaySlider->value());
    ...
    if (_kanaSelectorFont->isDefault())
        KanaView::characterFont.reset();
    else
        KanaView::characterFont.set(font.toString());
...
void Kanjidic2Preferences::updatePrintPreview() {
    ...
    const Kanjidic2EntryFormatter *formatter =
        static_cast<const Kanjidic2EntryFormatter *>(EntryFormatter::getFormatter(previewEntry));
    QPainter painter(&previewPic);
    QRectF usedSpace;
    formatter->drawCustom(
        previewEntry, painter,
        QRectF(0, 0, printPreviewScrollArea->viewport()->contentsRect().width() - 20, 300),
        usedSpace, QFont(), kanjiPrintSize->value(), fontButton->isChecked(),
        printMeanings->isChecked(), printOnyomi->isChecked(), printKunyomi->isChecked(),
        printComponents->isChecked(), printOnlyStudiedComponents->isChecked(),
        maxWordsPrint->value(), printOnlyStudiedVocab->isChecked(),
        printStrokesNumbersButton->isChecked(), printStrokesNumbersSize->value(),
        printGrid->isChecked());
...
void Kanjidic2Preferences::onSizeChanged(int newSize) { _player->setPictureSize(newSize); }
