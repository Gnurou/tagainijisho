...
class KanjiGraphicsItem : public QGraphicsSimpleTextItem {
  public:
    KanjiGraphicsItem(const QString &chr, QGraphicsItem *parent)
        : QGraphicsSimpleTextItem(chr, parent) {
  ...
  protected:
    virtual void hoverEnterEvent(QGraphicsSceneHoverEvent *event) {
        Kanjidic2EntryFormatter::instance().showToolTip(KanjiEntryRef(text()).get(),
                                                        QCursor::pos());
    }

    virtual void hoverLeaveEvent(QGraphicsSceneHoverEvent *event) { QToolTip::hideText(); }
};
...
QSize KanjiResultsView::sizeHint() const {
    ...
    return QSize(QWidget::sizeHint().width(),
                 QFontMetrics(font).height() + horizontalScrollBar()->height());
}
...
void KanjiResultsView::onSelectionChanged() {
    ...
    if (selection.isEmpty())
        return;
    emit kanjiSelected(
        static_cast<QGraphicsSimpleTextItem *>(selection[selection.size() - 1])->text());
}

void KanjiResultsView::startReceive() { clear(); }
...
void KanjiResultsView::endReceive() {
    ...
    if (!items.isEmpty())
        centerOn(0.0, ITEM_CENTER(items[0]).y());
}
