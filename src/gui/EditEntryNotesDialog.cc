...
QVariant EntryNotesModel::data(const QModelIndex &index, int role) const {
    ...
    if (role == NoteRole)
        return QVariant::fromValue(&_entry.notes()[index.row()]);
    else if (role == Qt::DisplayRole)
        return _entry.notes()[index.row()].note();
...
QString EditEntryNotesDialogEntryDelegate::displayText(const QVariant &value,
                                                       const QLocale &locale) const {
    return QStyledItemDelegate::displayText(value, locale).section(QChar::LineSeparator, 0, 0);
}

QSize EditEntryNotesDialogEntryDelegate::sizeHint(const QStyleOptionViewItem &option,
                                                  const QModelIndex &index) const {
    return QSize(300, QStyledItemDelegate::sizeHint(option, index).height());
}

EditEntryNotesDialog::EditEntryNotesDialog(Entry &entry, QWidget *parent)
    : QDialog(parent), currentNote(0) {
...
EditEntryNotesDialog::~EditEntryNotesDialog() { windowGeometry.set(saveGeometry()); }

void EditEntryNotesDialog::displayNote(const QItemSelection &selected,
