...
ReadingTrainer::ReadingTrainer(QWidget *parent)
    : QFrame(parent), _goodCount(0), _wrongCount(0), _totalCount(0), query(Database::connection()) {
    ui.setupUi(this);
...
ReadingTrainer::~ReadingTrainer() { windowGeometry.set(saveGeometry()); }
...
void ReadingTrainer::newSession() {
    ...
    // TODO Why the fuck is all that stuff needed to display the message box
    // correctly??
    messageBox.show();
    messageBox.repaint();
    ...
    QString queryString(QString("select k1.id from training cross join jmdict.kanjiChar as k1 on "
                                "training.type = %1 and training.id = k1.kanji cross join "
                                "training as t2 on t2.type = %2 and t2.id = k1.id cross join "
                                "jmdict.entries on entries.id = t2.id cross join jmdict.senses "
                                "on senses.id = k1.id and senses.priority = 0 where k1.priority "
                                "= 0 and (senses.misc0 & %3) = 0")
                            .arg(KANJIDIC2ENTRY_GLOBALID)
                            .arg(JMDICTENTRY_GLOBALID)
                            .arg(1 << JMdictPlugin::miscMap()["uk"].second));
    ...
    if (maxDate.isSet())
        queryString += QString(" and t2.dateLastTrain > %1")
                           .arg(maxDate.date().startOfDay().toSecsSinceEpoch());
    ...
    if (maxScore != TrainSettings::MAXSCORE_DEFAULT)
        queryString += QString(" and t2.score <= %1").arg(maxScore);
...
void ReadingTrainer::train() {
    ...
    if (query.next()) {
        ...
        if (_totalCount == 0)
            QMessageBox::information(this, tr("No matching entries found"),
                                     tr("Unable to find any entry eligible for reading practice. "
                                        "Entries "
                                        "eligible for this training mode are studied vocabulary "
                                        "entries "
                                        "for which all kanji are also studied, and that match the "
                                        "train "
                                        "settings. Please add entries or modify the train settings "
                                        "accordingly if you want to practice this mode."));
        else
            QMessageBox::information(this, tr("No more entries to train"),
                                     tr("There are no more entries to train for the "
                                        "current train settings."));
        close();
    }
}
...
void ReadingTrainer::onShowMeaningChecked(bool checked) {
    ...
    // Do not update the detailed view if it is already displaying the answer to
    // a mistaken entry.
    ...
    if (checked) {
        ...
        QString html(
            filler.fill(filler.extract(formatter->htmlTemplate(), parts), formatter, entry));
