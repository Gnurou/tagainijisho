...
void ReadingTrainer::newSession()
{
	...
	if (minDate.isSet()) queryString += QString(" and (t2.dateLastTrain < %1 OR t2.dateLastTrain is null)").arg(minDate.date().startOfDay().toSecsSinceEpoch());
	RelativeDate maxDate(TrainSettings::maxDatePref.value());
	if (maxDate.isSet()) queryString += QString(" and t2.dateLastTrain > %1").arg(maxDate.date().startOfDay().toSecsSinceEpoch());
