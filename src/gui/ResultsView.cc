...
PreferenceItem<bool> ResultsView::smoothScrollingSetting("mainWindow/resultsView",
...
PreferenceItem<QString> ResultsView::kanjiFontSetting("mainWindow/resultsView", "kanjiFont",
                                                      QFont("Helvetica", 15).toString());
PreferenceItem<int> ResultsView::displayModeSetting("mainWindow/resultsView", "displayMode",
                                                    EntryDelegateLayout::TwoLines);

ResultsView::ResultsView(QWidget *parent, EntryDelegateLayout *delegateLayout, bool viewOnly)
    : QListView(parent), _helper(this, delegateLayout, false, viewOnly) {
...
void ResultsView::selectionChanged(const QItemSelection &selected,
                                   const QItemSelection &deselected) {
    QListView::selectionChanged(selected, deselected);
    emit listSelectionChanged(selected, deselected);
    if (selected.isEmpty()) {
        // Check the current selection - if we have only one index, emit it as
        // selected
        QModelIndexList selection(selectionModel()->selectedIndexes());
        if (selection.size() != 1)
            return;
        EntryPointer entry(
            qvariant_cast<EntryPointer>(model()->data(selection[0], Entry::EntryRole)));
        if (entry)
