...
PreferencesWindowCategory::PreferencesWindowCategory(const QString &name, QWidget *parent)
...
PreferencesWindow::PreferencesWindow(QWidget *parent, Qt::WindowFlags f) : QDialog(parent, f) {
    ...
    foreach (const QMetaObject *objClass, _pluginPanels) {
        category = qobject_cast<PreferencesWindowCategory *>(
            objClass->newInstance(Q_ARG(QWidget *, this)));
        if (!category)
            continue;
        addCategory(category);
    }
...
void PreferencesWindow::applySettings() {
    ...
    foreach (PreferencesWindowCategory *category, cats)
...
static const QList<qps> _prefLangs() {
    ...
    foreach (const QString &lang, langMapping.keys()) {
        // FIXME log(n2)
        int i;
        QString trans = QCoreApplication::translate("GeneralPreferences",
                                                    langMapping[lang].toUtf8().constData());
        for (i = 0; i < ret.size(); i++)
            if (trans <= ret[i].second)
                break;
...
GeneralPreferences::GeneralPreferences(QWidget *parent)
    : PreferencesWindowCategory(tr("General"), parent) {
    setupUi(this);
...
void GeneralPreferences::applySettings() {
    ...
    if (fontChooser->isDefault())
        MainWindow::applicationFont.reset();
    else
        MainWindow::applicationFont.set(font.toString());

    // Application language
    if (guiLanguage->currentIndex() == 0)
        Lang::preferredGUILanguage.reset();
    else
        Lang::preferredGUILanguage.set(
            guiLanguage->itemData(guiLanguage->currentIndex()).toString());
    if (dictLanguage->currentIndex() == 0)
        Lang::preferredDictLanguage.reset();
    else
        Lang::preferredDictLanguage.set(
            dictLanguage->itemData(dictLanguage->currentIndex()).toString());

    // Always show English
    Lang::alwaysShowEnglish.set(alwaysShowEnglish->isChecked());

    // First day of week
    RelativeDate::firstDayOfWeek.set(static_cast<Qt::DayOfWeek>(
        firstDayOfWeek->itemData(firstDayOfWeek->currentIndex()).toInt()));
...
EntryDelegatePreferences::EntryDelegatePreferences(QWidget *parent) : QWidget(parent) {
    ...
    connect(kanjifontChooser, SIGNAL(fontChanged(const QFont &)), this,
            SLOT(setKanjiFont(const QFont &)));
    topLayout->addWidget(kanjifontChooser);

    kFont = QFont();
    kFont.fromString(ResultsView::kanaFontSetting.defaultValue());
    kanafontChooser =
        new PreferencesFontChooser(tr("Readings and alternate writings"), kFont, this);
    connect(kanafontChooser, SIGNAL(fontChanged(const QFont &)), this,
            SLOT(setKanaFont(const QFont &)));
    ...
    connect(romajifontChooser, SIGNAL(fontChanged(const QFont &)), this,
            SLOT(setTextFont(const QFont &)));
    topLayout->addWidget(romajifontChooser);
}

void EntryDelegatePreferences::setPrefsToWatch(PreferenceItem<int> *twoLinesPref,
                                               PreferenceItem<QString> *defaultFontPref,
                                               PreferenceItem<QString> *kanjiFontPref,
                                               PreferenceItem<QString> *kanaFontPref) {
...
void EntryDelegatePreferences::applyFontSetting(PreferencesFontChooser *fontChooser,
                                                PreferenceItem<QString> *prefItem,
                                                const EntryDelegateLayout::FontRole fontRole) {
    const QFont &font = fontChooser->font();
    if (fontChooser->isDefault())
        prefItem->reset();
    else
        prefItem->set(font.toString());
}

void EntryDelegatePreferences::refresh() {
    if (_twoLinesPref->value() == EntryDelegateLayout::OneLine)
        oneLine->click();
    else
        twoLines->click();
...
void EntryDelegatePreferences::applySettings() {
    if (oneLine->isChecked())
        _twoLinesPref->set(EntryDelegateLayout::OneLine);
    else
        _twoLinesPref->set(EntryDelegateLayout::TwoLines);
...
ResultsViewPreferences::ResultsViewPreferences(QWidget *parent)
    : PreferencesWindowCategory(tr("Results view"), parent) {
    setupUi(this);

    // Configure the delegate layout
    entryDelegatePrefs->setPrefsToWatch(
        &ResultsView::displayModeSetting, &ResultsView::textFontSetting,
        &ResultsView::kanjiFontSetting, &ResultsView::kanaFontSetting);
...
ListsViewPreferences::ListsViewPreferences(QWidget *parent)
    : PreferencesWindowCategory(tr("Lists"), parent) {
    setupUi(this);

    // Configure the delegate layout
    entryDelegatePrefs->setPrefsToWatch(
        &EntryListView::displayModeSetting, &EntryListView::textFontSetting,
        &EntryListView::kanjiFontSetting, &EntryListView::kanaFontSetting);
    // Prepare the preview list
    _list = new ResultsList(this);
...
DetailedViewPreferences::DetailedViewPreferences(QWidget *parent)
    : PreferencesWindowCategory(tr("Detailed view"), parent) {
    setupUi(this);
...

    {
        ...
        connect(romajifontChooser, SIGNAL(fontChanged(const QFont &)), detailedExample,
                SLOT(redrawRomajis(const QFont &)));
        ...
        connect(kanaHeaderfontChooser, SIGNAL(fontChanged(const QFont &)), detailedExample,
                SLOT(redrawKanasHeader(const QFont &)));
        ...
        connect(kanjiHeaderfontChooser, SIGNAL(fontChanged(const QFont &)), detailedExample,
                SLOT(redrawKanjisHeader(const QFont &)));
        ...
        connect(kanjifontChooser, SIGNAL(fontChanged(const QFont &)), detailedExample,
                SLOT(redrawKanjis(const QFont &)));
        connect(romajifontChooser, SIGNAL(fontChanged(const QFont &)), kanjifontChooser,
                SLOT(checkDefaultState(const QFont &)));
        ...
        connect(kanafontChooser, SIGNAL(fontChanged(const QFont &)), detailedExample,
                SLOT(redrawKanas(const QFont &)));
        connect(romajifontChooser, SIGNAL(fontChanged(const QFont &)), kanafontChooser,
                SLOT(checkDefaultState(const QFont &)));
...
void DetailedViewPreferences::applyFontSetting(PreferencesFontChooser *fontChooser,
                                               PreferenceItem<QString> *prefItem,
...
void DetailedViewPreferences::applySettings() {
    ...
    // Detailed view fonts
    applyFontSetting(romajifontChooser, &DetailedViewFonts::textFont,
                     DetailedViewFonts::DefaultText);
    applyFontSetting(kanaHeaderfontChooser, &DetailedViewFonts::kanaHeaderFont,
                     DetailedViewFonts::KanaHeader);
    applyFontSetting(kanjiHeaderfontChooser, &DetailedViewFonts::kanjiHeaderFont,
                     DetailedViewFonts::KanjiHeader);
...
void DetailedViewPreferences::updateUI() {
    foreach (DetailedView *view, DetailedView::instances())
        view->setSmoothScrolling(smoothScrolling->isChecked());
    DetailedViewFonts::fontsChanged();
}

DataPreferences::DataPreferences(QWidget *parent)
    : PreferencesWindowCategory(tr("User data"), parent) {
    setupUi(this);
...
void DataPreferences::onWipeUserDataButtonPushed() {
    if (QMessageBox::question(this, tr("Really erase user data?"),
                              tr("This will erase all your user data. Are you "
                                 "sure you want to do this?"),
                              QMessageBox::Yes | QMessageBox::No,
                              QMessageBox::No) == QMessageBox::Yes) {
        if (QMessageBox::question(this, tr("REALLY erase it?"),
                                  tr("Are you absolutely, positively, definitely sure?"),
                                  QMessageBox::Yes | QMessageBox::No,
                                  QMessageBox::No) == QMessageBox::Yes) {
            QFile f(Database::defaultDBFile());
            if (!f.exists()) {
                QMessageBox::information(this, tr("No user database file"),
                                         tr("The user database file has "
                                            "already been erased before."));
            } else if (!f.remove()) {
                QMessageBox::warning(this, tr("Cannot erase user database"),
                                     tr("Unable to erase user database. Please see "
                                        "what is wrong and try to do it manually."));
            } else {
                QMessageBox::information(this, tr("User data erased"),
                                         tr("User data has been erased. "
                                            "Tagaini Jisho will now exit. You "
                                            "will be back to a blank database "
...
PreferencesFontChooser::PreferencesFontChooser(const QString &whatFor, const QFont &defaultFont,
                                               QWidget *parent)
    : QWidget(parent), _defaultFont(defaultFont) {
...
PreferencesDetailedViewExample::PreferencesDetailedViewExample(QWidget *parent)
    : DetailedView(parent) {
