...
ResultsList::ResultsList(QObject *parent)
    : QAbstractListModel(parent), entries(), displayedUntil(0), dbThread(), query(&dbThread) {
...
void ResultsList::addResult(EntryRef entry) { entries << entry; }
...
void ResultsList::startReceive() { timer.start(); }
