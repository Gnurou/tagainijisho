...
class TagsInputDialog : public QDialog {
  ...
  public:
    TagsInputDialog(const QString &title, const QString &message,
                    const QStringList &initialTags = QStringList(), bool selectAll = false,
                    QWidget *parent = 0);
...
class TagsDialogs {
  ...
  public:
     ...
     */
    static bool splitTagsString(const QString &string, QStringList &tagsList,
                                QStringList &invalidTags);

    /**
     * Invokes the dialog to set tags for the list of entries given as
     * parameter. Returns true if the tags have changed, false otherwise.
     */
