...
RelativeDatePopup::RelativeDatePopup(RelativeDate &date, QWidget *parent)
    : QFrame(parent), _date(date) {
...
RelativeDateEdit::RelativeDateEdit(QWidget *parent)
    : ElidedPushButton<QPushButton>(parent), _date(), _popup(_date) {
    _popup.hide();
...
void RelativeDateEdit::wheelEvent(QWheelEvent *event) {
    if (event->angleDelta().x() == 0)
        return; // Vertical Orientation is 0
