...
EntryDelegate::EntryDelegate(EntryDelegateLayout *dLayout, QObject *parent)
    : QStyledItemDelegate(parent), layout(dLayout), _hiddenIcons(0) {
...
QSize EntryDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const {
    ...
    if (maxHeight < 0) {
        if (layout->displayMode() == EntryDelegateLayout::OneLine)
            maxHeight = qMax(QFontMetrics(layout->kanjiFont()).height(),
                             qMax(QFontMetrics(layout->kanaFont()).height(),
                                  QFontMetrics(layout->textFont()).height()));
        else
            maxHeight = qMax(QFontMetrics(layout->kanjiFont()).height(),
                             QFontMetrics(layout->kanaFont()).height()) +
                        QFontMetrics(layout->textFont()).height();
    }
...
void EntryDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option,
    ...
    style->drawItemText(painter, mainBbox, Qt::AlignBaseline, QApplication::palette(), enabled,
                        mainRepr);

    // Used for alternate writings and readings
    QString s = "  ";
    if (layout->displayMode() == EntryDelegateLayout::OneLine) {
        // The first reading is only needed if the main representation is part
        // of the writings
        if (!readings.isEmpty() && writings.contains(mainRepr))
            s += "(" + readings[0] + ")  ";
    } else {
    ...
    s = QFontMetrics(layout->kanaFont())
            .elidedText(s, Qt::ElideRight, wholeAreaRect.width() - mainBbox.width());
    ...
    if (entry->meanings().size() == 1) {
        ...
        for (int i = 0; i < entry->meanings().size(); i++) {
    ...
    if (layout->displayMode() == EntryDelegateLayout::OneLine) {
        ...
        s = QFontMetrics(layout->textFont())
                .elidedText(s, Qt::ElideRight,
                            wholeAreaRect.width() - (mainBbox.width() + readBbox.width()));
    } else {
        defRect.setBottom(defRect.bottom() - defDescent);
        defBbox = painter->boundingRect(defRect, Qt::AlignLeft | Qt::AlignBottom, s);
        s = QFontMetrics(layout->textFont()).elidedText(s, Qt::ElideRight, wholeAreaRect.width());
    }
    style->drawItemText(painter, defBbox, Qt::AlignLeft | Qt::AlignBottom, QApplication::palette(),
                        enabled, s);
