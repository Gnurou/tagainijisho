...
MultiStackedWidgetButton::MultiStackedWidgetButton(QAction *action, QAction *resetAction,
                                                   QWidget *parent)
    : ElidedPushButton<QToolButton>(parent), _action(action), _resetAction(resetAction) {
...
void MultiStackedWidgetButton::onResetActionChanged() {
    if (_resetAction->isEnabled())
        showAltWidget();
    else
        hideAltWidget();
}

void MultiStackedWidgetButton::onActionChanged() { setText(_action->text()); }

void MultiStackedWidgetButton::resizeEvent(QResizeEvent *event) {
    QToolButton::resizeEvent(event);
    if (!_altWidget)
        return;
    _altWidget->setGeometry((event->size().width() - 3 - ALTWIDGETSIZE),
                            (event->size().height() - ALTWIDGETSIZE) / 2, ALTWIDGETSIZE,
                            ALTWIDGETSIZE);
}
...
void MultiStackedWidgetButton::rewriteCurrentTitle() {
    if (_altWidget && _altWidget->isVisible()) {
        ...
        int nbSpaces = ALTWIDGETSIZE / metrics.horizontalAdvance(' ');
        QString spaces;
...
QPair<QAction *, QAction *> MultiStackedWidget::addWidget(const QString &label, QWidget *widget) {
    ...
    QAction *resetAction =
        new QAction(QIcon(":/images/icons/eraser-small.png"), tr("Reset this filter"), this);
...
void MultiStackedWidget::showWidget(QWidget *widget) {
    if (!_buttonMap.contains(widget))
        return;
    if (!widget->isVisible()) {
        foreach (QWidget *otherWidget, _buttonMap.keys())
            if (otherWidget->isVisible())
                _buttonMap[otherWidget].actions.first->toggle();
...
void MultiStackedWidget::onTitleChanged(const QString &newTitle) {
    ...
    if (!_buttonMap.contains(signalSender))
        return;
...
void MultiStackedWidget::hideAllExtenders() {
    foreach (QWidget *key, _buttonMap.keys()) {
        if (_buttonMap[key].actions.first->isChecked())
            _buttonMap[key].actions.first->trigger();
    }
}
