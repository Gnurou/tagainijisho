...
EntryPointer EntriesCache::_get(EntryType type, EntryId id) {
    ...
    _cacheMutex.lock();
    _cache << ret;
    while (_cache.size() > cacheSize.value())
        _cache.removeFirst();
    // Keep a weak pointer in the list of loaded entries - it is convertible to
    // a QSharedPointer but will not influence the reference count.
...
void EntriesCache::_removeAndDelete(const Entry *entry) {
    ...
    // From here we know that the reference counter of our entry will not be
    // changed Have we created a new reference to this entry by the meantime?
#if QT_VERSION >= QT_VERSION_CHECK(5, 14, 0)
    if (entry->ref.loadRelaxed() > 0)
        return;
#else
    if (entry->ref.load() > 0)
        return;
...
#ifdef DEBUG_ENTRIES_CACHE
    qDebug("Entry <%d,%d> deleted, %d in cache", entry->type(), entry->id(),
           _instance->_loadedEntries.size());
#endif
    delete entry;
}
