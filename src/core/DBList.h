...
template <class T> struct DBListEntry {
    ...
    /**
     * Returns the string that defines the columns of the data members in the
     * database.
     */
...
/**
 * Low-level database layer for persistent RB trees. Trees are stored in two
 * tables, one that stores all the nodes of the lists, another that associates
 * the list id to the index of its root node.
 *
 * The naming of tables is configurable at construction time, so it is possible
 * to store different kind of lists within different tables.
 */
template <class T> class DBList {
  ...
  public:
    ...
    /// Remove a given list. All nodes of the list should be deleted before
    /// calling this method!
...
template <class T>
DBList<T>::DBList(const QString &tableName, SQLite::Connection *connection)
    : _tableName(tableName) {
    prepareForConnection(connection);
}

template <class T> bool DBList<T>::createTables(SQLite::Connection *connection) {
    if (!connection->exec(QString("CREATE TABLE %1(rowid INTEGER PRIMARY KEY, leftSize INTEGER, "
                                  "red TINYINT, parent INTEGER, left INTEGER, right INTEGER, %2)")
                              .arg(_tableName)
                              .arg(DBListEntry<T>::tableDataMembers())))
        return false;
    if (!connection->exec(QString("CREATE TABLE %1Roots(listId INTEGER PRIMARY "
                                  "KEY, rootId INTEGER, label TEXT)")
                              .arg(DBList<T>::tableName())))
        return false;
    // Entry for root list (rowid = 0)
    // if (!connection->exec(QString("insert into %1Root values(0, 0,
    // \"\")").arg(DBList<T>::tableName()))) return false;
    return true;
}
...
template <class T> bool DBList<T>::prepareForConnection(SQLite::Connection *connection) {
    ...
    while (nbDataMembers-- > 0)
        dataHoldersList << "?";
    QString dataHolders(dataHoldersList.join(", "));

    if (connection) {
        if (!getEntryQuery.prepare(QString("select * from %1 where rowid = ?").arg(_tableName)))
            return false;
        if (!insertEntryQuery.prepare(
                QString("insert or replace into %1 values(?, ?, ?, ?, ?, ?, %2)")
                    .arg(_tableName)
                    .arg(dataHolders)))
            return false;
        if (!removeEntryQuery.prepare(QString("delete from %1 where rowid == ?").arg(_tableName)))
            return false;

        if (!newListQuery.prepare(
                QString("insert into %1Roots values(NULL, 0, \"\")").arg(_tableName)))
            return false;
        if (!getListQuery.prepare(
                QString("select * from %1Roots where listId = ?").arg(_tableName)))
            return false;
        if (!insertListQuery.prepare(QString("insert or replace into %1Roots values(?, ?, ?)")
                                         .arg(DBList<T>::tableName())))
            return false;
        if (!removeListQuery.prepare(
                QString("delete from %1Roots where listId = ?").arg(DBList<T>::tableName())))
            return false;
    }
    return true;
}
...
template <class T> quint32 DBList<T>::insertEntry(const DBListEntry<T> &entry) {
    if (entry.rowId == 0)
        insertEntryQuery.bindNullValue();
    else
        insertEntryQuery.bindValue(entry.rowId);
