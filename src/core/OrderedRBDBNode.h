...
template <class T> class OrderedRBDBNode : public OrderedRBNodeBase<T> {
  ...
  public:
    ...
    ~OrderedRBDBNode() {
        // This destructor should not remove the node from the database as it
        // would break data persistency. In order to permanently remove nodes,
        // removeNode() should be used instead.
    }
    ...
    /**
     * Called by OrderedRBDBTree for every node that has been marked as being
     * changed.
     */
    bool updateDB() {
    ...
    quint32 rowId() const { return e.rowId; }
 ...
 *
 * Instances of this class need to be connected to a suitable DBList instance in
 * order to fetch and store their data. They also need to be affected a list
 * identifier, from which the root of the tree can be fetched. The documentation
...
template <class T> class OrderedRBDBTree {
  ...
  public:
    ...
    void nodeChanged(Node *n) { _changedNodes << n; }

    void forgetNode(Node *node) { _changedNodes.remove(node); }
    ...
    quint32 listId() const { return _listInfo.listId; }
    ...
    bool setLabel(const QString &l) {
        ...
        if (!ret)
            _listInfo.label = oldLabel;
        return ret;
    }

    void setDBAccess(DBList<T> *ldb) { _ldb = ldb; }
     ...
     */
    bool removeList() {
        if (!_ldb->removeList(_listInfo.listId))
            return false;
