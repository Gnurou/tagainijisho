...
TatoebaPlugin::TatoebaPlugin() : Plugin("Tatoeba") { _instance = this; }
...
bool TatoebaPlugin::onRegister() {
    ...
    if (!Database::attachDictionaryDB(dbFile, "tatoeba", TATOEBADB_REVISION)) {
        qFatal("Tatoeba plugin fatal error: failed to attach main database %s!",
               dbFile.toLatin1().constData());
        return false;
    }
    foreach (const QString &lang, supportedLanguages()) {
        ...
        if (!dbFile.isEmpty()) {
            if (!Database::attachDictionaryDB(dbFile, "tatoeba_" + lang, TATOEBADB_REVISION)) {
                qWarning("Tatoeba plugin fatal error: failed to attach database %s!",
                         dbFile.toLatin1().constData());
                continue;
            }
            _dbLanguages << lang;
    ...
    if (!EntriesCache::instance().addLoader(TATOEBAENTRY_GLOBALID, loader))
        return false;
...
QString TatoebaPlugin::pluginInfo() const {
    return QString("<p><a href=\"http://www.tatoeba.org\">Tatoeba</a>, distributed under "
                   "the <a "
                   "href=\"http://www.creativecommons.org/licenses/by/2.0/\">Creative "
                   "Commons Attribution License, version 2.0</a>.</p>");
}
