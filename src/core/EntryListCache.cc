...
EntryListCache::EntryListCache() : _dbAccess(LISTS_DB_TABLES_PREFIX) {
    ...
    ownerQuery.prepare(
        QString("select rowid from %1 where type = 0 and id = ?").arg(_dbAccess.tableName()));
    goUpQuery.prepare(QString("select parent, leftSize, right from %1 where rowid = ?")
                          .arg(_dbAccess.tableName()));
    listFromRootQuery.prepare(
        QString("select listId from %1Roots where rootId = ?").arg(_dbAccess.tableName()));
}
...
EntryList *EntryListCache::_get(quint64 id) {
    ...
    if (!_cachedLists.contains(id)) {
        // We know we can only be called with list Ids that actually exist in
        // the DB, so no need to bother about non-existing entries
...
void EntryListCache::_clearListCache(quint64 id) { _cachedLists.remove(id); }
...
QPair<const EntryList *, quint32> EntryListCache::_getOwner(quint64 id) {
    if (!_cachedParents.contains(id)) {
        ...
        ownerQuery.exec();
        if (!ownerQuery.next())
            return QPair<const EntryList *, quint32>();
...
void EntryListCache::_clearOwnerCache(quint64 id) { _cachedParents.remove(id); }

void EntryListCache::_clearOwnerCache() { _cachedParents.clear(); }
