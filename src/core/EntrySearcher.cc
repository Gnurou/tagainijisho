...
void EntrySearcher::buildStatement(QList<SearchCommand> &commands, QueryBuilder::Statement &statement)
{
	...
	foreach(const SearchCommand &command, commands) {
		...
		if (command.command() == "study") {
			...
			if (interval.first.isValid()) statement.addWhere(QString("training.dateAdded >= %1").arg(interval.first.startOfDay().toSecsSinceEpoch()));
			if (interval.second.isValid()) statement.addWhere(QString("training.dateAdded < %1").arg(interval.second.startOfDay().toSecsSinceEpoch()));
		}
			...
			if (command.args().size() >= 2) s2 = command.args()[1];
			QPair<QDate, QDate> interval = timeInterval(s1, s2);
			if (interval.first.isValid()) statement.addWhere(QString("training.dateLastTrain >= %1").arg(interval.first.startOfDay().toSecsSinceEpoch()));
			if (interval.second.isValid()) statement.addWhere(QString("(training.dateLastTrain < %1 or training.dateLastTrain is null)").arg(interval.second.startOfDay().toSecsSinceEpoch()));
			...
			if (command.args().size() >= 2) s2 = command.args()[1];
			QPair<QDate, QDate> interval = timeInterval(s1, s2);
			if (interval.first.isValid()) statement.addWhere(QString("training.dateLastMistake >= %1").arg(interval.first.startOfDay().toSecsSinceEpoch()));
			if (interval.second.isValid()) statement.addWhere(QString("training.dateLastMistake < %1").arg(interval.second.startOfDay().toSecsSinceEpoch()));
