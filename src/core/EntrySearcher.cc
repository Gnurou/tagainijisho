...
EntrySearcher::EntrySearcher(EntryType entryType)
    ...
    QueryBuilder::Join::addTablePriority("tags", -55);
    validCommands << "study" << "nostudy" << "note" << "lasttrained" << "mistaken" << "tag"
                  << "untagged" << "score";
}
...
void EntrySearcher::buildStatement(QList<SearchCommand> &commands,
    ...
    foreach (const SearchCommand &command, commands) {
        ...
        if (command.command() == "study") {
            ...
            if (command.args().size() != 2 && command.args().size() != 1)
                continue;
            bool ok;
            ...
            if (command.args().size() == 1)
                statement.addWhere(QString("training.score = %1").arg(from));
            else
                statement.addWhere(QString("training.score between %1 and %2").arg(from).arg(to));
        } else if (command.command() == "note") {
            statement.addJoin(QueryBuilder::Join(QueryBuilder::Column("notes", "id"),
                                                 QString("notes.type = %1").arg(entryType()),
                                                 QueryBuilder::Join::Left));
            if (command.args().isEmpty())
                statement.addWhere(QString("notes.dateAdded not null"));
            else
            ...
            statement.addJoin(QueryBuilder::Join(
                QueryBuilder::Column("taggedEntries", "id"),
                QString("taggedEntries.type = %1").arg(entryType()), QueryBuilder::Join::Left));
            if (command.args().isEmpty()) {
                statement.addWhere(QString("taggedEntries.date not null"));
                allTagsHandled = true;
            } else
                foreach (const QString &arg, command.args())
                    // We filter the "*" tag as FTS3 does not support it - but
                    // since the condition is added, all non-tagged entries will
                    // be filtered which is the desired result anyway.
                    if (arg != "*")
                        tagSearch << "\"" + arg + "\"";
                    else if (!allTagsHandled) {
                        statement.addWhere(QString("taggedEntries.date not null"));
                        allTagsHandled = true;
                    }
    ...
    if (!notesSearch.isEmpty()) {
        statement.addWhere(QString("notes.noteId in (select docid from notesText "
                                   "where note match '%1')")
                               .arg(notesSearch.join(" ")));
    }
    if (!tagSearch.isEmpty()) {
        // Remove duplicates, case insensitively
        QSet<QString> tmpSet;
        foreach (const QString &string, tagSearch)
            tmpSet << string.toLower();
        tagSearch.clear();
        foreach (const QString &string, tmpSet)
            tagSearch << string;
        statement.addWhere(QString("taggedEntries.id in (select id from taggedEntries where type "
                                   "= %1 and tagId in (select docid from tags where tag match "
                                   "'%2') group by id having count(id) == %3)")
                               .arg(entryType())
                               .arg(tagSearch.join(" OR "))
                               .arg(tagSearch.size()));
        //		statement.setGroupBy(QueryBuilder::GroupBy("taggedEntries.id",
        // QString("count(taggedEntries.id) = %1").arg(tagSearch.size())));
    }
}

QueryBuilder::Column EntrySearcher::canSort(const QString &sort,
...
void EntrySearcher::setColumns(QueryBuilder::Statement &statement) const {
    ...
    statement.addJoin(QueryBuilder::Join(QueryBuilder::Column("training", "id"),
                                         QString("training.type = %1").arg(entryType()),
                                         QueryBuilder::Join::Left));
    statement.autoJoin();
