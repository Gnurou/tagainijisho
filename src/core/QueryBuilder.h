...
class QueryBuilder {
  public:
    ...
    class Column {
      ...
      public:
        Column(const QString &table, const QString &column, const QString &function = "")
            : _table(table), _column(column), _function(function) {}
    ...
    class Join {
      ...
      public:
        /**
         * Constructor for two-column join. Such join objects ignore the with
         * argument of the toString() method.
         */
        Join(const Column &column1, const Column &column2,
             const QString &additionalCondition = QString(""), Type type = Cross)
            : _type(type), _column1(column1), _column2(column2),
              _additionalCondition(additionalCondition) {}
        /**
         * Constructor for one-column join. The right part of the join is
         * determined later, using the with argument of the toString() method.
         */
        Join(const Column &column1, const QString &additionalCondition = QString(""),
             Type type = Cross)
            : _type(type), _column1(column1), _column2(Column("", "")),
              _additionalCondition(additionalCondition) {}
        /**
         * Returns true if that Join has been constructed with both a left and
         * right column.
         */
    ...
    class GroupBy {
      ...
      public:
        GroupBy(const QString &group = "", const QString &having = "")
            : _group(group), _having(having) {}
    ...
    class Limit {
      ...
      public:
        ...
        Limit(unsigned int start, unsigned int nbResults)
            : _active(true), _start(start), _nbResults(nbResults) {}
