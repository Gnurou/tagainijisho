...
Kanjidic2EntrySearcher::Kanjidic2EntrySearcher() : EntrySearcher(KANJIDIC2ENTRY_GLOBALID) {
    ...
    validCommands << "kanji" << "romaji" << "kana" << "mean" << "jlpt" << "grade" << "stroke"
                  << "radical" << "component" << "unicode" << "skip" << "fourcorner" << "kanjidic";
}
...
static QString buildTextSearchCondition(const QStringList &words, const QString &table) {
    ...
    static QString glossRegexpMatch(
        "{{leftcolumn}} in (select entry from kanjidic2_%2.meaning where "
        "FTSUNCOMPRESS(meanings) REGEXP '%1')");
    static QString globalMatch(
        "{{leftcolumn}} IN (SELECT entry FROM kanjidic2%3.%2 JOIN "
        "kanjidic2%3.%2Text ON kanjidic2%3.%2.docid = kanjidic2%3.%2Text.docid "
        "WHERE %1)");
    ...
    foreach (const QString &lang, langs) {
        ...
        if (!conds.isEmpty())
            globalMatches << globalMatch.arg(conds.join(" AND "))
                                 .arg(table)
                                 .arg(table == "meaning" ? "_" + lang : "");
        globalMatches += condsGloss;
    }
    return globalMatches.join(" OR ");
}

void Kanjidic2EntrySearcher::buildStatement(QList<SearchCommand> &commands,
    ...
    foreach (const SearchCommand &command, commands) {
        ...
        if (command.command() == "kanji") {
            ...
            if (command.args().isEmpty())
                statement.addWhere(QString("kanjidic2.entries.jlpt not null"));
            else {
                ...
                statement.addWhere(
                    QString("kanjidic2.entries.jlpt in (%1)").arg(levelsList.join(", ")));
            }
        } else if (command.command() == "grade") {
            if (command.args().isEmpty())
                statement.addWhere(QString("kanjidic2.entries.grade not null"));
            else {
                QStringList levelsList;
                foreach (const QString &arg, command.args()) {
                    bool isInt;
                ...
                statement.addWhere(
                    QString("kanjidic2.entries.grade in (%1)").arg(levelsList.join(", ")));
            }
        } else if (command.command() == "stroke") {
            if (command.args().size() == 1) {
                statement.addWhere(
                    QString("kanjidic2.entries.strokeCount = %1").arg(command.args()[0].toInt()));
            } else if (command.args().size() == 2) {
                statement.addWhere(QString("kanjidic2.entries.strokeCount between %1 and %2")
                                       .arg(command.args()[0].toInt())
                                       .arg(command.args()[1].toInt()));
            } else
                continue;
            ...
            if (command.args().size() > 0) {
                ...
                foreach (const QString &arg, command.args()) {
                    if (arg.size() != 1 || !TextTools::isKanjiChar(arg)) {
                        ...
                        radicalSearch << QString::number(TextTools::singleCharToUnicode(arg));
                }
            ...
            if (command.args().size() > 0) {
                ...
                foreach (const QString &arg, command.args()) {
                    if (arg.size() != 1 || !TextTools::isKanjiChar(arg)) {
                        ...
                        componentSearch << QString::number(TextTools::singleCharToUnicode(arg));
                }
                // Break command if one of the arguments were invalid
                if (!valid)
                    continue;
            ...
            if (codeParts.size() != 3)
                continue;
            bool ok;
            int t = codeParts[0].toInt(&ok);
            if (!ok && codeParts[0] != "?")
                continue;
            int c1 = codeParts[1].toInt(&ok);
            if (!ok && codeParts[1] != "?")
                continue;
            int c2 = codeParts[2].toInt(&ok);
            if (!ok && codeParts[2] != "?")
                continue;
            if (t || c1 || c2) {
                statement.addJoin(
                    QueryBuilder::Join(QueryBuilder::Column("kanjidic2.skip", "entry")));
                if (t)
                    statement.addWhere(QString("kanjidic2.skip.type = %1").arg(t));
                if (c1)
                    statement.addWhere(QString("kanjidic2.skip.c1 = %1").arg(c1));
                if (c2)
                    statement.addWhere(QString("kanjidic2.skip.c2 = %1").arg(c2));
            }
            ...
            if (topLeft != -1 || topRight != -1 || botLeft != -1 || botRight != -1 || extra != -1) {
                statement.addJoin(
                    QueryBuilder::Join(QueryBuilder::Column("kanjidic2.fourCorner", "entry")));
                if (topLeft != -1)
                    statement.addWhere(QString("kanjidic2.fourCorner.topLeft = %1").arg(topLeft));
                if (topRight != -1)
                    statement.addWhere(QString("kanjidic2.fourCorner.topRight = %1").arg(topRight));
                if (botLeft != -1)
                    statement.addWhere(QString("kanjidic2.fourCorner.botLeft = %1").arg(botLeft));
                if (botRight != -1)
                    statement.addWhere(QString("kanjidic2.fourCorner.botRight = %1").arg(botRight));
                if (extra != -1)
                    statement.addWhere(QString("kanjidic2.fourCorner.extra = %1").arg(extra));
            }
        }
        // Filter command
    ...
    if (!kanaSearch.isEmpty())
        statement.addWhere(buildTextSearchCondition(kanaSearch, "reading"));
    if (!transSearch.isEmpty())
        statement.addWhere(buildTextSearchCondition(transSearch, "meaning"));
    ...
    if (!radicalSearch.isEmpty()) {
        statement.addWhere(QString("kanjidic2.entries.id IN("
                                   ...
                                   "group by r1.kanji "
                                   "having uniquecount(r1.number) >= %2)")
                               .arg(radicalSearch.join(", "))
                               .arg(radicalSearch.size()));
    }

    if (!componentSearch.isEmpty()) {
        QString onlyDirectComponentsString("and ks1.isRoot = 1");
        QString qString;
        // TODO We should be able to control this - probably when we have a more
        // powerful command system based on subclasses
        if (0)
            qString = onlyDirectComponentsString;
        statement.addWhere(QString("kanjidic2.entries.id IN ("
                                   ...
                                   "ELSE NULL END) >= %2"
                                   ")")
                               .arg(componentSearch.join(", "))
                               .arg(componentSearch.size())
                               .arg(qString));
    }
}

QueryBuilder::Column Kanjidic2EntrySearcher::canSort(const QString &sort,
                                                     const QueryBuilder::Statement &statement) {
    QueryBuilder::Column res(EntrySearcher::canSort(sort, statement));
    if (res.column() != "0")
        return res;

    // Would be nice to have, but all meanings are grouped into the same string.
    // Maybe in a future database revision?
    /*
    if (sort == "matchPos") {
            foreach(const QueryBuilder::Join &join, statement.joins()) {
                    ...
                    if (t1 == "kanjidic2.meaning" || t2 == "kanjidic2.meaning")
    return QueryBuilder::Column("kanjidic2.meaning", "priority", "min");
            }
    }*/
    // TODO replace with frequency, once the sort order is changed
    else if (sort == "freq")
        return QueryBuilder::Column("kanjidic2.entries", "jlpt");
    else if (sort == "jlpt")
        return QueryBuilder::Column("kanjidic2.entries", "frequency");
    return res;
}
