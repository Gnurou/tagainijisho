...
Kanjidic2Plugin::Kanjidic2Plugin() : Plugin("kanjidic2") { _instance = this; }

Kanjidic2Plugin::~Kanjidic2Plugin() { _instance = 0; }

QString Kanjidic2Plugin::pluginInfo() const {
    return QString::fromUtf8("<p><a "
                             "href=\"http://www.csse.monash.edu.au/~jwb/"
                             "kanjidic.html\">Kanjidic</a> version %1, distributed under the "
                             "<a "
                             "href=\"http://creativecommons.org/licenses/by-sa/3.0/"
                             "\">Creative "
                             "Common Attribution Share Alike License, version "
                             "3.0</a>.</p><p><a "
                             "href=\"http://kanjivg.tagaini.net/\">KanjiVG</a> version %2, "
                             "distributed under the <a "
                             "href=\"http://creativecommons.org/licenses/by-sa/3.0/"
                             "\">Creative "
                             "Commons Attribution-Share Alike 3.0 license</a>.</p><p>JLPT "
                             "levels courtesy to the <a "
                             "href=\"http://www.jlptstudy.com/\">JLPT Study Page</a>, the <a "
                             "href=\"http://www.tanos.co.uk/jlpt/\">JLPT Resources Page</a>, "
                             "and lists provided by <a "
                             "href=\"http://www.thbz.org/kanjimots/jlpt.php3\">Thierry "
                             "Bézecourt</a> and <a "
                             "href=\"http://jetsdencredujapon.blogspot.com\">Alain Côté</a>.")
        .arg(kanjidic2Version())
        .arg(kanjiVGVersion());
}
...
bool Kanjidic2Plugin::attachAllDatabases() {
    ...
    if (dbFile.isEmpty()) {
        ...
        qFatal("kanjidic2 plugin fatal error: cannot find main database "
               "file!\n\nSystem path: %s",
               QDir(system_path).filePath("kanjidic2.db").toStdString().c_str());
        return false;
    }
    ...
    foreach (const QString &lang, Lang::preferredDictLanguages()) {
        ...
        if (dbFile.isEmpty())
            continue;
        if (!Database::attachDictionaryDB(dbFile, QString("kanjidic2_%1").arg(lang),
                                          KANJIDIC2DB_REVISION))
            continue;
...
void Kanjidic2Plugin::detachAllDatabases() {
    ...
    foreach (const QString &lang, _attachedDBs.keys()) {
        ...
        if (!Database::detachDictionaryDB(dbAlias))
            qWarning("kanjidic2 plugin warning: Cannot detach database %s",
...
bool Kanjidic2Plugin::onRegister() {
    ...
    if (!EntriesCache::instance().addLoader(KANJIDIC2ENTRY_GLOBALID, loader))
        return false;
