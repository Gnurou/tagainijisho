...
class Kanjidic2Entry : public Entry {
  ...
  public:
    class KanjiReading {
      ...
      public:
        KanjiReading(const QString &type, const QString &reading)
            : _type(type), _reading(reading) {}
    ...
    class KanjiMeaning {
      ...
      public:
        KanjiMeaning(const QString &lang, const QString &meaning)
            : _lang(lang), _meaning(meaning) {}
  ...
  protected:
    Kanjidic2Entry(const QString &kanji, bool inDB, int grade = -1, int strokeCount = -1,
                   qint32 kanjiFrequency = -1, int jlpt = -1, int heisig = -1,
  ...
  public:
    ...
    /**
     * Returns the root components, i.e. the minimum set of components that are
     * sufficient to cover as many strokes of this kanji as possible.
     */
...
class KanjiEntryRef : public EntryRef {
  public:
    ...
    KanjiEntryRef(const QString &kanji)
        : EntryRef(KANJIDIC2ENTRY_GLOBALID, TextTools::singleCharToUnicode(kanji)) {}
