...
const QString &KanjiComponent::repr(bool simplified) const {
    if (simplified && !original().isEmpty())
        return original();
    else
        return element();
}
...
Kanjidic2Entry::Kanjidic2Entry(const QString &kanji, bool inDB, int grade, int strokeCount,
                               qint32 kanjiFrequency, int jlpt, int heisig,
                               const QString &dictionaries)
    : Entry(KANJIDIC2ENTRY_GLOBALID, TextTools::singleCharToUnicode(kanji)), _inDB(inDB),
      _kanji(kanji), _grade(grade), _strokeCount(strokeCount), _jlpt(jlpt), _heisig(heisig),
...
/**
 * Returns the root components, i.e. the minimum set of components that covers
 * as many strokes as possible in this kanji.
 */
const QList<const KanjiComponent *> &Kanjidic2Entry::rootComponents() const {
    return _rootComponents;
    /*
    // Build a strokes coverage map associating each stroke to a "root"
    component QMap<const KanjiStroke *, const KanjiComponent *> strokesCoverage;
    foreach (const KanjiStroke &stroke, strokes()) strokesCoverage[&stroke] = 0;

    // Now set each stroke coverage to point to the component that contains it
    and features the most strokes const QList<KanjiComponent>
    &comps(components()); foreach (const KanjiComponent &component, comps) {
            foreach (const KanjiStroke *stroke, component.strokes()) {
                    if (strokesCoverage[stroke] == 0) strokesCoverage[stroke] =
    &component; else if (strokesCoverage[stroke]->strokes().size() <
    component.strokes().size()) strokesCoverage[stroke] = &component;
            }
    }
...
QStringList Kanjidic2Entry::readings() const {
    QStringList res;
    foreach (const KanjiReading &reading, kanjiReadings())
        res << reading.reading();
    return res;
}
...
QStringList Kanjidic2Entry::meanings() const {
    QStringList res;
    foreach (const KanjiMeaning &meaning, kanjiMeanings())
        res << meaning.meaning();
    return res;
}
...
QStringList Kanjidic2Entry::onyomiReadings() const {
    ...
    foreach (const QString &reading, readings()) {
        if (TextTools::isKatakana(reading))
            ret << reading;
    }
    return ret;
}
...
QStringList Kanjidic2Entry::kunyomiReadings() const {
    QStringList ret;
    foreach (const QString &reading, readings()) {
        if (TextTools::isHiragana(reading))
            ret << reading;
    }
    return ret;
}
