...
class PreferenceRoot : public QObject {
  ...
  public:
    PreferenceRoot(const QString &group, const QString &name)
        : QObject(), _group(group), _name(name) {}
...
/**
 * A template class that allows to set and access preference values quickly. The
 * current value of the preference is always cached. This class also provides a
 * way to reset a value to its default.
 */
template <class T> class PreferenceItem : public PreferenceRoot {
  private:
    T _value;
    T _defaultValue;

  public:
    PreferenceItem(const QString &group, const QString &name, const T &defaultValue,
                   bool persistent = false)
        : PreferenceRoot(group, name), _defaultValue(defaultValue) {
        _settingsMutex().lock();
    ...
    void setValue(QVariant newValue) { set(newValue.value<T>()); }
