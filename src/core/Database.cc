...
bool Database::createUserDB() {
    ...
    // Study table
    QUERY("CREATE TABLE training(type INT NOT NULL, id INTEGER SECONDARY KEY NOT "
          "NULL, score INT NOT NULL, dateAdded UNSIGNED INT NOT NULL, "
          "dateLastTrain UNSIGNED INT, nbTrained UNSIGNED INT NOT NULL, "
          "nbSuccess UNSIGNED INT NOT NULL, dateLastMistake UNSIGNED INT, "
          "CONSTRAINT training_unique_ids UNIQUE(type, id))");
    ...
    QUERY("CREATE TABLE taggedEntries(type INT, id INTEGER SECONDARY KEY, tagId "
          "INTEGER SECONDARY KEY REFERENCES tags, date UNSIGNED INT)");

    // Notes tables
    QUERY("CREATE TABLE notes(noteId INTEGER PRIMARY KEY, type INT, id INTEGER "
          "SECONDARY KEY, dateAdded UNSIGNED INT, dateLastChange UNSIGNED INT)");
    QUERY("CREATE VIRTUAL TABLE notesText USING fts4(note)");

    // Sets table
    QUERY("CREATE TABLE sets(rowid INTEGER PRIMARY KEY, parent INT, position INT "
          "NOT NULL, label TEXT, state BLOB)");
...
static bool update1to2(SQLite::Query &query) {
    ...
    // Fix invalid dates...
    QUERY("UPDATE oldtraining set dateLastTrain = null where dateLastTrain = "
          "4294967295");
    ...
    QUERY("CREATE TABLE training(type INT NOT NULL, id INTEGER SECONDARY KEY NOT "
          ...
          "nbSuccess UNSIGNED INT NOT NULL, dateLastMistake UNSIGNED INT, "
          "CONSTRAINT training_unique_ids UNIQUE(type, id))");
    QUERY("INSERT INTO training(type, id, score, dateAdded, dateLastTrain, "
          "nbTrained, nbSuccess, dateLastMistake) SELECT *, null from "
          "oldtraining");
    QUERY("UPDATE training SET dateLastMistake = (SELECT MAX(date) FROM "
          "trainingLog WHERE result = 0 and trainingLog.id = training.id and "
          "trainingLog.type = training.type)");
    ...
    QUERY("CREATE INDEX idx_training_type_id ON training(type, id)");
    QUERY("CREATE INDEX idx_training_score ON training(score)");
    QUERY("CREATE TRIGGER update_score after update of nbTrained, nbSuccess on "
          "training begin update training set score = (nbSuccess * 100) / "
          "(nbTrained + 1) where type = old.type and id = old.id; end;");
...
static bool update2to3(SQLite::Query &query) {
    ...
    QUERY("CREATE TABLE sets(parent INT, position INT NOT NULL, label TEXT, "
          "state BLOB)");
    QUERY("CREATE INDEX idx_sets_id ON sets(parent, position)");
...
static bool update4to5(SQLite::Query &query) {
    QUERY("CREATE TABLE lists(parent INTEGER REFERENCES lists, position INTEGER "
          "NOT NULL, type INTEGER, id INTEGER)");
...
static bool update6to7(SQLite::Query &query) { return true; }
...
static bool update7to8(SQLite::Query &query) {
    ...
    QUERY("SELECT oldLists.rowid, type, id, label FROM oldLists LEFT JOIN "
          "listsLabels ON oldLists.rowid = listsLabels.rowid WHERE parent is "
    ...
    do {
        ...
        while (query.next()) {
            ...
            // If type is zero, this means we have a sub-list - create it and
            // schedule it for migration
            else {
        ...
        QUERY(QString("SELECT oldLists.rowid, type, id, label FROM oldLists LEFT "
                      "JOIN listsLabels ON oldLists.rowid = listsLabels.rowid "
                      "WHERE parent = %1 ORDER BY position ASC")
                  .arg(next.first));
        listId = next.second;
    } while (true);
...
static bool update8to9(SQLite::Query &query) {
    ...
    QUERY("INSERT INTO newnotesText(docid, note) SELECT docid, note FROM "
          "notesText");
    ...
    QUERY("INSERT INTO notesText(docid, note) SELECT docid, note FROM "
          "newnotesText");
...
bool (*dbUpdateFuncs[USERDB_REVISION - 1])(SQLite::Query &) = {
    &update1to2, &update2to3, &update3to4, &update4to5,  &update5to6,
    &update6to7, &update7to8, &update8to9, &update9to10, &update10to11,
};
...
bool Database::updateUserDB(int currentVersion) {
    // The database is older than our version of Tagaini - we have to update the
    // database
    if (!_connection.transaction())
        return false;
    SQLite::Query query2(&_connection);
    for (; currentVersion < USERDB_REVISION; ++currentVersion) {
        if (!dbUpdateFuncs[currentVersion - 1](query2))
            goto failed;
        query2.clear();
    }
    // Update version number
    if (!query2.exec(
            QString("UPDATE versions SET version=%1 where id=\"userDB\"").arg(USERDB_REVISION)))
        goto failed;
    if (!_connection.commit())
        goto failed;
    return true;
...
bool Database::checkUserDB(QStringList &errors) {
    ...
    if (query.next())
        currentVersion = query.valueInt(0);
    else {
    ...
    if (currentVersion != -1) {
        if (currentVersion < USERDB_REVISION) {
            if (!updateUserDB(currentVersion)) {
                ...
                errors << tr("Error while upgrading user database: %1")
                              .arg(_connection.lastError().message().toLatin1().constData());
                return false;
            }
        }
        // The database is more recent than our version of Tagaini - there is
        // nothing we can do!
        else if (currentVersion > USERDB_REVISION) {
            errors << tr("Wrong user database version: expected %1, got %2.")
                          .arg(USERDB_REVISION)
                          .arg(currentVersion);
            return false;
        }
        ...
        if (!createUserDB()) {
            _connection.rollback();
            // Big issue here - start with a temporary database
            errors << tr("Cannot create user database: %1")
                          .arg(_connection.lastError().message().toLatin1().constData());
            return false;
        }
    }
...
bool Database::connectUserDB(QString filename, QStringList &errors) {
    ...
    if (!_connection.connect(filename)) {
        errors << tr("Cannot open database: %1")
                      .arg(_connection.lastError().message().toLatin1().data());
        return false;
    }
    if (!checkUserDB(errors))
        return false;
...
bool Database::init(const QString &userDBFile, bool temporary, QStringList &errors) {
    ...
    if (temporary || !_instance->connectUserDB(userDBFile, errors)) {
        if (!_instance->connectToTemporaryDatabase(errors)) {
            errors << tr("Temporary database fallback failed. The program will "
                         "now exit.");
            return false;
        } else if (!temporary) {
            errors << tr("Tagaini is working on a temporary database. This "
                         "allows the program "
                         "to work, but user data is unavailable and any change "
                         "will be lost "
                         "upon program exit. If you corrupted your database "
                         "file, please "
                         "recreate it from the preferences.");
        }
    }
    return true;
}
...
void Database::stop() {
    ...
    // The query must not live until the end of the method, as the connection is
    // uses will be deleted before.
    {
        SQLite::Query query(&_instance->_connection);

        // Remove unreferenced tags
        if (!query.exec("delete from tags where docid not in (select tagId from "
                        "taggedEntries)"))
            qWarning("Could not cleanup unused tags!");

        // VACUUM the database
        if (!query.exec("vacuum"))
            qWarning("Final VACUUM failed %s", query.lastError().message().toLatin1().data());
    }
    // Close the database
...
Database::Database() : _tFile(0) { sqlite3ext_init(); }
...
#define QUERY(Q)                                                                                   \
    ...
    if (!query.next())
        goto error;
    if (query.valueInt(0) != expectedVersion)
        goto errorDetach;
    ...
    foreach (DatabaseThread *dbThread, DatabaseThread::instances()) {
        if (!dbThread->connection()->attach(file, alias))
            goto errorDetachAll;
    }
    return true;
errorDetachAll:
