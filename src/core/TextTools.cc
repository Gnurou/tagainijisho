...
bool isKanjiChar(const QChar c, const QChar c2) {
    ...
    if (!c.isHighSurrogate())
        return isKanjiChar(c.unicode());
    // If we do, the second argument must be filled!
    else
        return isKanjiChar(QChar::surrogateToUcs4(c, c2));
}
...
bool isKanjiChar(const QString &s, int pos) {
    ...
    if (s.isEmpty() || pos >= s.size())
        return false;
    c = s[pos];
    if (c.isHighSurrogate() && pos < s.size() - 1)
        c2 = s[pos + 1];
...
bool isHiragana(const QString &string) {
    for (int i = 0; i < string.size(); i++)
        if (!(isHiraganaChar(string[i]) || string[i].isPunct()))
            return false;
    return true;
}
...
bool isKatakana(const QString &string) {
    for (int i = 0; i < string.size(); i++)
        if (!isKatakanaChar(string[i]) || string[i].isPunct())
            return false;
    return true;
}
...
bool isKana(const QString &string) {
    for (int i = 0; i < string.size(); i++)
        if (!isKanaChar(string[i]) || string[i].isPunct())
            return false;
    return true;
}
...
bool isKanji(const QString &string) {
    for (int i = 0; i < string.size(); i++) {
        ...
        if (!isKanjiChar(string, i) || c.isPunct())
            return false;
        if (c.isHighSurrogate())
            i++;
    }
...
bool isJapanese(const QString &string) {
    for (int i = 0; i < string.size(); i++) {
        const QChar &c = string[i];
        if (!isJapaneseChar(string, i) || string[i].isPunct())
            return false;
        if (c.isHighSurrogate())
            i++;
    }
...
bool isRomaji(const QString &string) {
    for (int i = 0; i < string.size(); i++) {
        if (!isRomajiChar(string[i]) || string[i].isPunct())
            return false;
    }
    return true;
}
...
int kanasTableRow(const QChar c) {
    for (unsigned int i = 0; i < KANASTABLE_NBROWS; i++) {
        if (c < hiraganaTable[i][0] || c > hiraganaTable[i][4])
            continue;
        for (int j = 0; j < 5; j++) {
            if (c == hiraganaTable[i][j])
                return i;
        }
    }
    return -1;
...
QString hiragana2Katakana(const QString &hira) {
    ...
    for (int i = 0; i < hira.size(); i++) {
        if (isHiraganaChar(hira[i]))
            kata[i] = hiraganaChar2Katakana(hira[i]);
        else
            kata[i] = hira[i];
    }
    return kata;
...
unsigned int singleCharToUnicode(const QString &chr, int pos) {
    if (chr.size() <= pos)
        return 0;
    if (!chr[pos].isHighSurrogate())
        return chr[pos].unicode();
    else if (chr.size() >= pos + 2 && chr[pos + 1].isLowSurrogate())
        return QChar::surrogateToUcs4(chr[pos], chr[pos + 1]);
    else
        return 0;
}
