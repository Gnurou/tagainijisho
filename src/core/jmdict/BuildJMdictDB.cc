...
#define BIND(query, val)                                                                           \
    {                                                                                              \
        if (!query.bindValue(val)) {                                                               \
            qCritical("BIND failed, line %d: %s", __LINE__,                                        \
                      query.lastError().message().toUtf8().data());                                \
            return false;                                                                          \
        }                                                                                          \
    }
#define BINDNULL(query)                                                                            \
    {                                                                                              \
        if (!query.bindNullValue()) {                                                              \
            qCritical("BINDNULL failed, line %d: %s", __LINE__,                                    \
                      query.lastError().message().toUtf8().data());                                \
            return false;                                                                          \
        }                                                                                          \
    }
...
#define EXEC(query)                                                                                \
    if (!query.exec()) {                                                                           \
        qCritical(" EXEC failed, line %d: %s", __LINE__,                                           \
                  query.lastError().message().toUtf8().data());                                    \
        return false;                                                                              \
    }
#define EXEC_STMT(query, stmt)                                                                     \
    if (!query.exec(stmt)) {                                                                       \
        qCritical(" EXEC_STMT failed, line %d: %s", __LINE__,                                      \
                  query.lastError().message().toUtf8().data());                                    \
        return false;                                                                              \
    }
#define ASSERT(cond)                                                                               \
...
class JMdictDBParser : public JMdictParser {
  public:
    JMdictDBParser(const QStringList &languages, QString sourceDirectory,
                   QString destinationDirectory)
        : JMdictParser(languages) {
...
bool JMdictDBParser::onItemParsed(const JMdictItem &entry) {
    ...
    foreach (const JMdictKanaReadingItem &kReading, entry.kana) {
        ...
        foreach (quint8 res, kReading.restrictedTo)
            restrictedToList << QString::number(res);
    ...
    foreach (const JMdictSenseItem &sense, entry.senses) {
        ...
        QStringList restrictedToList;
        foreach (quint8 res, sense.restrictedToKanji)
            restrictedToList << QString::number(res);
        AUTO_BIND(insertSenseQuery, restrictedToList.join(","), "");
        restrictedToList.clear();
        foreach (quint8 res, sense.restrictedToKana)
            restrictedToList << QString::number(res);
        AUTO_BIND(insertSenseQuery, restrictedToList.join(","), "");
        ...
        foreach (const QString &lang, languages) {
            ...
            const QStringList &glosses =
                jmf[lang][entry.id][idx].isEmpty() ? sense.gloss[lang] : jmf[lang][entry.id][idx];
            if (glosses.isEmpty()) {
...
bool JMdictDBParser::createMainDatabase() { return openDatabase("jmdict.db", "main"); }
...
bool JMdictDBParser::createMainTables() {
    ...
    EXEC_STMT(query, "create table posEntities(bitShift INTEGER PRIMARY KEY, "
                     "name TEXT, description TEXT)");
    EXEC_STMT(query, "create table miscEntities(bitShift INTEGER PRIMARY KEY, "
                     "name TEXT, description TEXT)");
    EXEC_STMT(query, "create table fieldEntities(bitShift INTEGER PRIMARY KEY, "
                     "name TEXT, description TEXT)");
    EXEC_STMT(query, "create table dialectEntities(bitShift INTEGER PRIMARY KEY, "
                     "name TEXT, description TEXT)");
    EXEC_STMT(query, "create table entries(id INTEGER PRIMARY KEY, frequency "
                     "SMALLINT, kanjiCount TINYINT)");
    EXEC_STMT(query, "create table kanji(id INTEGER SECONDARY KEY REFERENCES entries, "
                     "priority TINYINT, docid INTEGER PRIMARY KEY, frequency TINYINT)");
    EXEC_STMT(query, "create virtual table kanjiText using fts4(reading)");
    EXEC_STMT(query, "create table kana(id INTEGER SECONDARY KEY REFERENCES "
                     "entries, priority TINYINT, docid INTEGER PRIMARY KEY, "
                     "nokanji BOOLEAN, frequency TINYINT, restrictedTo TEXT)");
    EXEC_STMT(query, "create virtual table kanaText using fts4(reading, TOKENIZE katakana)");
    // Temporary table until we figure out how many pos, misc,... columns we
    // need
    EXEC_STMT(query, "create table sensesTMP(id INTEGER SECONDARY KEY REFERENCES "
                     "entries, priority TINYINT, pos TEXT, misc TEXT, dial TEXT, field "
                     "TEXT, restrictedToKanji TEXT, restrictedToKana TEXT)");
    EXEC_STMT(query, "create table kanjiChar(kanji INTEGER, id INTEGER SECONDARY "
                     "KEY REFERENCES entries, priority INT)");
...
static QVector<quint64> entityInsert(const QString &entities,
                                     const QHash<QString, quint16> &entityBitFields) {
...
bool JMdictDBParser::finalizeSensesTable() {
    ...
    // First figure out how many 64-bit integer columns we need to encode the
    // entities
    ...
    EXEC_STMT(query, QString("create table senses(id INTEGER SECONDARY KEY "
                             "REFERENCES entries, priority TINYINT, %1, %2, %3, "
                             "%4, restrictedToKanji TEXT, restrictedToKana TEXT)")
                         .arg(posStr)
                         .arg(miscStr)
                         .arg(dialStr)
                         .arg(fieldStr));
    ...
    query2.prepare(QString("insert into senses(rowid, id, priority, %1, %2, %3, %4, "
                           "restrictedToKanji, restrictedToKana) values(?, ?, ?, "
                           "%5%6%7%8?, ?)")
...
bool JMdictDBParser::finalizeMainDatabase() { return closeDatabase("main"); }
...
bool JMdictDBParser::createLanguagesTables() {
    foreach (const QString &lang, languages) {
        ...
        EXEC_STMT(query, "create table info(version INT, JMdictVersion TEXT)");
        EXEC_STMT(query, "create table gloss(id INTEGER SECONDARY KEY, docid "
                         "INTEGER PRIMARY KEY)");
...
void printUsage(char *argv[]) {
    qCritical("Usage: %s [-l<lang>] JMdict_file source_dir dest_dir\nWhere "
              "<lang> is a two-letters language code (en, fr, de, es or ru)",
              argv[0]);
}

bool buildDB(const QStringList &languages, const QString &JMdictFile, const QString &srcDir,
