...
JMdictEntryLoader::JMdictEntryLoader()
    : EntryLoader(), validEntryQuery(&connection), kanjiQuery(&connection), kanaQuery(&connection),
      sensesQuery(&connection), jlptQuery(&connection) {
    ...
    kanjiQuery.prepare("select reading, frequency from jmdict.kanji join jmdict.kanjiText on "
                       "kanji.docid == kanjiText.docid where id=? order by priority");
    kanaQuery.prepare("select reading, nokanji, frequency, restrictedTo from "
                      "jmdict.kana join jmdict.kanaText on kana.docid == "
                      "kanaText.docid where id=? order by priority");
    sensesQuery.prepare("select " + JMdictPlugin::dbColumns(JMdictPlugin::posMap(), "pos") + ", " +
                        JMdictPlugin::dbColumns(JMdictPlugin::miscMap(), "misc") + ", " +
                        JMdictPlugin::dbColumns(JMdictPlugin::dialMap(), "dial") + ", " +
                        JMdictPlugin::dbColumns(JMdictPlugin::fieldMap(), "field") +
                        ", restrictedToKanji, restrictedToKana from jmdict.senses where id=? "
...
Entry *JMdictEntryLoader::loadEntry(EntryId id) {
    ...
    while (kanaQuery.next()) {
        ...
        if (kanaQuery.valueBool(1) == false) {
            ...
            if (restrictedTo.isEmpty())
                ...
                for (int i = 0; i < restrictedTo.size(); i++) {
    ...
    foreach (const QString &lang, Lang::preferredDictLanguages()) {
        ...
        if (glossQuery.next()) {
            QStringList glosses(
                QString::fromUtf8(qUncompress(glossQuery.valueBlob(0))).split("\n\n"));
            for (int i = 0; i < glosses.size(); i++) {
                // Skip empty glosses
                if (glosses[i].isEmpty())
                    continue;
                // Do not load english if a preferred language is already loaded
                // and the corresponding option is set
                if (!Lang::alwaysShowEnglish() && lang == "en" &&
                    entry->senses[i].getGlosses().size() > 0)
                    continue;
