...
JMdictPlugin::JMdictPlugin() : Plugin("JMdict") { _instance = this; }

JMdictPlugin::~JMdictPlugin() { _instance = 0; }

QString JMdictPlugin::pluginInfo() const {
    return QString("<p><a "
                   "href=\"http://www.csse.monash.edu.au/~jwb/"
                   "jmdict.html\">JMDict</"
                   "a> version %1, distributed under the <a "
                   "href=\"http://creativecommons.org/licenses/by-sa/3.0/"
                   "\">Creative "
                   "Common Attribution Share Alike License, version "
                   "3.0</a>.</p><p><a href=\"http://www.kanji.org/\">SKIP "
                   "codes</a> "
                   "are developed by Jack Halpern and distributed under the <a "
...
bool JMdictPlugin::attachAllDatabases() {
    ...
    if (dbFile.isEmpty()) {
        ...
        qFatal("JMdict plugin fatal error: cannot find main database "
               "file!\n\nSystem path: %s",
               QDir(system_path).filePath("jmdict.db").toStdString().c_str());
        return false;
    }
    ...
    foreach (const QString &lang, Lang::preferredDictLanguages()) {
        ...
        if (dbFile.isEmpty())
            continue;
        if (!Database::attachDictionaryDB(dbFile, QString("jmdict_%1").arg(lang),
                                          JMDICTDB_REVISION))
            continue;
...
void JMdictPlugin::detachAllDatabases() {
    ...
    foreach (const QString &lang, _attachedDBs.keys()) {
        ...
        if (!Database::detachDictionaryDB(dbAlias))
            qWarning("JMdict plugin warning: Cannot detach database %s",
                     dbAlias.toUtf8().constData());
    }
    _attachedDBs.clear();
}

void JMdictPlugin::queryEntities(SQLite::Query *query, const QString &entity,
                                 QMap<QString, QPair<QString, quint16>> *map,
                                 QVector<QString> *shift) {
    QString queryString = QString("select bitShift, name, description from "
                                  "jmdict.%1Entities order by bitShift")
                              .arg(entity);
...
bool JMdictPlugin::onRegister() {
    ...
    if (query.next())
        _dictVersion = query.valueString(0);
    query.clear();
    ...
    if (!EntriesCache::instance().addLoader(JMDICTENTRY_GLOBALID, loader))
        return false;
...
QSet<QString> JMdictPlugin::shiftsToSet(const QVector<QString> &shift,
                                        const QVector<quint64> &bits) {
    QSet<QString> ret;
...
QString JMdictPlugin::dbColumns(const QMap<QString, QPair<QString, quint16>> &map,
