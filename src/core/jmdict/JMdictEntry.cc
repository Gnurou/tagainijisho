...
KanjiReading::KanjiReading(const QString &reading, quint32 info, quint8 frequency)
    : reading(reading), info(info), _frequency(frequency) {}

KanaReading::KanaReading(const QString &reading, qint32 info, quint8 frequency)
    : reading(reading), info(info), _frequency(frequency) {}

void KanaReading::addKanjiReading(int readingIndex) { kanjiReadings << readingIndex; }

Gloss::Gloss(const QString &lang, const QString &gloss) : _lang(lang), _gloss(gloss) {}

Sense::Sense(const QSet<QString> &partOfSpeech, const QSet<QString> &misc,
             const QSet<QString> &dialect, const QSet<QString> &field)
    : _partOfSpeech(partOfSpeech), _misc(misc), _dialect(dialect), _field(field) {}

void Sense::addGloss(const Gloss &gloss) { glosses << gloss; }
...
QString JMdictEntry::mainRepr() const {
    ...
    // the first sense has the 'uk' tag, in which case it becomes the first
    // reading
    if (senses.isEmpty() || kanas.isEmpty() || !writtenInKana())
...
QStringList JMdictEntry::writings() const {
    QStringList res;
    if (hasKanjiReadings())
        foreach (const KanjiReading &reading, getKanjiReadings())
...
QList<const Sense *> JMdictEntry::getSenses() const {
    ...
    QSet<QString> filter(JMdictEntrySearcher::miscFilterEntities() -
                         JMdictEntrySearcher::explicitlyRequestedMiscs());
