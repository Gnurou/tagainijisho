...
JMdictEntrySearcher::JMdictEntrySearcher() : EntrySearcher(JMDICTENTRY_GLOBALID) {
    connect(&JMdictEntrySearcher::miscPropertiesFilter, SIGNAL(valueChanged(QVariant)), this,
            SLOT(updateMiscFilterMask()));
    ...
    validCommands << "romaji" << "mean" << "kana" << "kanji" << "jmdict" << "haskanji" << "jlpt"
                  << "withstudiedkanjis" << "hascomponent" << "withkanaonly";
...
SearchCommand JMdictEntrySearcher::commandFromWord(const QString &word) const {
    ...
    else if (TextTools::isJapanese(checkString))
    ...
    // Then we are probably looking for romaji
        ...
        if (allowRomajiSearch()) {
            ...
            if (!kn.isEmpty())
                return SearchCommand::fromString(QString(":romaji=\"%1\"").arg(word));
        }
        return SearchCommand::fromString(QString(":mean=\"%1\"").arg(word));
    }
...
static QString buildTextSearchCondition(const QStringList &words, const QString &table) {
    ...
    static QString glossRegexpMatch("{{leftcolumn}} in (select id from jmdict_%2.glosses where "
                                    "FTSUNCOMPRESS(glosses) REGEXP '%1')");
    static QString globalMatch(
        "{{leftcolumn}} IN (SELECT id FROM jmdict%3.%2 JOIN jmdict%3.%2Text ON "
        "jmdict%3.%2.docid = jmdict%3.%2Text.docid WHERE %1)");
    ...
    foreach (const QString &lang, langs) {
        ...
        foreach (const QString &w, words) {
            if (w.contains(regExpChars)) {
                ...
                // there is no need for a regexp search
                if (wildcardIdx == w.size() - 1 && w.size() > 1 && w[wildcardIdx] == '*')
                    continue;
        ...
        if (!fts.isEmpty())
            conds.insert(0, ftsMatch.arg(fts.join(" ")));
        if (!conds.isEmpty())
            globalMatches << globalMatch.arg(conds.join(" AND "))
                                 .arg(table)
                                 .arg(table == "gloss" ? "_" + lang : "");
        globalMatches += condsGloss;
    }
    return globalMatches.join(" OR ");
}

void JMdictEntrySearcher::buildStatement(QList<SearchCommand> &commands,
        ...
        if (commandLabel == "mean" || commandLabel == "kana" || commandLabel == "kanji") {
            ...
            foreach (const QString &arg, command.args()) {
                if (arg.size() != 1 || !TextTools::isKanjiChar(arg)) {
                    ...
                    if (!hasKanjiSearch.contains(kNumber))
                        hasKanjiSearch << kNumber;
                }
            }
            ...
            foreach (const QString &arg, command.args()) {
                if (arg.size() != 1 || !TextTools::isKanjiChar(arg)) {
                    ...
                    QString kNumber = QString::number(TextTools::singleCharToUnicode(arg));
                    if (!hasComponentSearch.contains(kNumber))
                        hasComponentSearch << kNumber;
                }
            }
            // Break command if one of the arguments were invalid
            if (!valid)
            ...
            if (command.args().size() == 2) {
                ...
                if (!ok)
                    continue;
                scoreCondition =
                    QString("and training.score between %1 and %2").arg(minScore).arg(maxScore);
            } else if (command.args().size() != 0)
                continue;
            // Performance is very bad when using :train with this command.
            // changing the first jmdict.entries.id to training.id in that case
            // fixes the issue. We must keep track of the "id column" and use it
            // for this kind of reference
            QString condition(QString("{{leftcolumn}} in "
                                      "(select jmdict.entries.id from jmdict.entries "
                                      "join jmdict.kanjiChar on jmdict.entries.id = "
                                      "jmdict.kanjiChar.id and jmdict.kanjiChar.priority = 0 "
                                      "join training on training.id = jmdict.kanjiChar.kanji and "
                                      "training.type = 2 %1 "
                                      "group by jmdict.entries.id "
                                      "having count(jmdict.kanjiChar.kanji) = "
                                      "jmdict.entries.kanjiCount)")
                                  .arg(scoreCondition));
            // Check whether we should include kana only words as well
            if (allCommands.contains("withkanaonly")) {
                condition = "(" + condition +
                            QString(" or {{leftcolumn}} in "
                                    "(select id from jmdict.entries where "
                                    "kanjiCount == 0))");
            }
    ...
    if (!kanjiReadingsMatch.isEmpty())
        statement.addWhere(buildTextSearchCondition(kanjiReadingsMatch, "kanji"));
    if (!kanaReadingsMatch.isEmpty())
        statement.addWhere(buildTextSearchCondition(kanaReadingsMatch, "kana"));
    if (!transReadingsMatch.isEmpty())
        statement.addWhere(buildTextSearchCondition(transReadingsMatch, "gloss"));
    ...
    bool mustJoinSenses =
        filteredMisc | !posFilter.isEmpty() | miscFilter | dialectFilter | fieldFilter;
    if (mustJoinSenses) {
        ...
        if (miscFilter)
            statement.addWhere(QString("jmdict.senses.misc0 & %2 == %2").arg(miscFilter));
        if (dialectFilter)
            statement.addWhere(QString("jmdict.senses.dial0 & %2 == %2").arg(dialectFilter));
        if (fieldFilter)
            statement.addWhere(QString("jmdict.senses.field0 & %2 == %2").arg(fieldFilter));
        // Implicitely masked misc properties
        if (filteredMisc)
            statement.addWhere(QString("jmdict.senses.misc0 & %1 == 0").arg(filteredMisc));
    }

    // TODO it should be ensure that kanjisearch and componentsearch are applied
    // on the same writing
    if (!hasKanjiSearch.isEmpty()) {
        QString condition(QString("{{leftcolumn}} in "
                                  ...
                                  "having count(jmdict.kanjiChar.kanji) = %2)")
                              .arg(hasKanjiSearch.join(", "))
                              .arg(hasKanjiSearch.size()));
        // Check whether we should include kana only words as well
        if (allCommands.contains("withkanaonly")) {
            condition = "(" + condition +
                        QString(" or {{leftcolumn}} in "
    ...
    if (!hasComponentSearch.isEmpty()) {
        QString condition(QString("{{leftcolumn}} in "
                                  "(select distinct jmdict.kanjiChar.id from jmdict.kanjiChar "
                                  "join kanjidic2.strokeGroups on jmdict.kanjiChar.kanji = "
                                  "kanjidic2.strokeGroups.kanji "
                                  ...
                                  "ELSE NULL END) >= %2"
                                  ")")
                              .arg(hasComponentSearch.join(", "))
                              .arg(hasComponentSearch.size()));
        // Check whether we should include kana only words as well
        if (allCommands.contains("withkanaonly")) {
            condition = "(" + condition +
                        QString(" or {{leftcolumn}} in "
...
void JMdictEntrySearcher::updateMiscFilterMask() {
    ...
    foreach (const QString &str, miscPropertiesFilter.value().split(','))
        if (JMdictPlugin::miscMap().contains(str))
            _miscFilterMask << str;
}

QueryBuilder::Column JMdictEntrySearcher::canSort(const QString &sort,
    ...
    if (sort == "matchPos") {
        foreach (const QueryBuilder::Join &join, statement.joins()) {
            ...
            // if (t1 == "jmdict.gloss" || t2 == "jmdict.gloss") return
            // QueryBuilder::Column("jmdict.gloss", "sensePriority", "min");
            if (t1 == "jmdict.kana" || t2 == "jmdict.kana")
                return QueryBuilder::Column("jmdict.kana", "priority", "min");
            if (t1 == "jmdict.kanji" || t2 == "jmdict.kanji")
                return QueryBuilder::Column("jmdict.kanji", "priority", "min");
        }
    } else if (sort == "freq")
        return QueryBuilder::Column("jmdict.entries", "frequency");
    else if (sort == "jlpt")
        return QueryBuilder::Column("jmdict.jlpt", "level");
    return res;
}
